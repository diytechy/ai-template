...
Stack-agnostic, standard-library only. This is **Phase 2a** of the concurrency
restructure (docs/concurrency-restructure.md §2): the mechanical, provable half
of "specs are the registry". `docs/requirements/work-items.csv` remains the
single authoritative home — this script does **not** materialize `docs/work/`
in the repo. What it delivers is the converter plus the proof that the
conversion loses nothing, because the design's own rule is that *the converter
is proven by a round-trip before the CSV is demoted* (the 140-cell lesson: you
do not flip an authoritative registry on a conversion you have not measured).
...
def frontmatter_pairs(row, order):
    ...
    lists = dict(LIST_FIELDS)
    ints = dict(INT_FIELDS)
    for column in COLUMNS:
        cell = row.get(column) or ""
        if column in lists:
            if cell:
                pairs.append((lists[column], cell.split(SEP)))
        elif column in ints:
            if cell:
                pairs.append((ints[column], _int_or_text(cell)))
        elif column in scalars and column != "Title":
            if cell:
                pairs.append((scalars[column], cell))
        elif column == "Title":
            pairs.append(("title", cell))
