...
The active gate is **derived from the artifact states**, not hand-set: `docs/gate`
is generated by `scripts/derive_gate.py` (the repo is at gate G iff every in-scope
artifact meets G's bar), so closing a gate = **ratifying a batch of artifacts in a
reviewed commit** (`Status` `Draft`→`Planned`, or an SN out of its draft section),
never a manual bump (§7 "The active gate"; the model + parallel/series workflow:
...
**The active gate is derived, cached, and CI reads it.** The current gate lives on
the first non-comment line of `docs/gate` — now **generated** by
`scripts/derive_gate.py` from the artifact states, not hand-set (§4; the model:
