...
**Stages and gates — state vs. certified boundary.** **Stages are the tiers of
the decomposition: a repo is *in* one. Gates are the subset of stage boundaries
a human must certify: you *pass* one.** Keeping them one axis is what made "the
repo is at G1" unable to say whether G1 was ahead or behind. Six mechanized
...
stage 5  nothing in process
```

The boundaries that are **not** gates — 0→1 and 2→3 — are exactly the ones with
no sign-off row: you draft needs then requirements without a sitting between,
and designs then tests without one either. `G-Release` / `G-Final` are the named
boundaries *above* this ladder; no rung past 5 is mechanized, so a stage-5 repo
stays held to the G3 bar. **There is no `G0`** — the absence of having passed G1
is *stage 0*, which needs no apology; say "stage 0", never "at G0".

**The rule is uniform: `stage → gate` is the next gate you must pass** —
0,1→G1 · 2,3→G2 · 4,5→G3 — which is also the strictness the harness holds you
to, for a good reason: you are held to the bar you are trying to clear. The
stage is derived beside the gate and rides `docs/gate`'s `# basis:` line as
`stage=N`; it is the axis `[attestation] human_ratification_through` is compared
against (its own levels stay 0–4, the four *spine* tiers plus "nothing" —
implementation is not a ratification tier).

**But a gate is not a pure function of stage** — the mapping is a reader's
reconciliation, not a second source of truth. G1's bar includes non-goals
captured and a UX sign-off; G2's includes key runtime flows diagrammed. None of
that is derivable from which tier is in process, so never derive the gate from
the stage: that silently drops the human half.
...
bar the project is actually working toward** — a fresh scaffold deriving G1 is
...
Without a derived gate, CI would apply the end-state G3 bar from day one and
