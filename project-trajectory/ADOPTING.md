...
- **Set `docs/gate` to `DevStg-Reqs` honestly**, whatever the code's maturity — gates
  describe the *registry's* coverage of the product, and that coverage starts
  near zero. Claim DevStg-Tests/DevStg-Impl only when their criteria genuinely hold for the scope
...
The DevStg-Reqs bar is deliberately small (doc navigability). **The gate is derived, not
...
**approve artifacts in a reviewed commit** (`Status` `Drafted`→`Approved`, the same
field at every tier including SN) and regenerate — CI reads the cached value and raises the bar
