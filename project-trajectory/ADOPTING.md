...
- **`check_flows.py`** (the authored "Runtime flows" section, required from DevStg-Tests)
  ...
  **author** the section (write the sequence diagrams for the key scenarios and
  ...
  flow citing example ids — `--no-placeholders` (wired from DevStg-Tests) will flag it.
...
```
python scripts/derive_stage.py     # compute docs/stage (the effective stage) from the artifact states
python scripts/check.py            # runs every step at or above docs/stage (DevStg-Reqs to start)
python scripts/trace.py            # writes docs/test/report.md
```

The DevStg-Reqs selection is deliberately small (doc navigability). **The stage is
derived, not bumped** (process.md §7; process-options.md "Derived gate model"):
`docs/stage` is generated by `scripts/derive_stage.py` from the artifact states,
and it rises as you **approve artifacts in a reviewed commit** (`Status`
`Drafted`→`Approved`, the same field at every tier including SN) and regenerate —
CI reads the cached record and selects the higher steps with you. A fresh scaffold
ships the comment-only `stage.template` placeholder; the first `derive_stage.py`
run above turns it into a real record (until then the `derived-stage --check` step
passes with a note, so you upgrade without a red day).
...
**One standing rule in that table is overridden by the migrations.** The deviation
review classes a committed derived cache as *preserve* — its value is the repo's
own. Pack §3 inverts that twice, and in opposite directions:

- **`docs/gate` is now DELETE-at-re-sync, not preserve.** The whole three-value
  bar axis retired: nothing derives that file any more, no reader consults it, and
  no value in it is needed. `git rm docs/gate` along with `derive_gate.py`, the
  `derived-gate` step and `gate.template`.
- **`docs/stage` is the successor and the one that is kept** — but as a
  *generated* cache it is **regenerated**, never merged: run
  `python scripts/derive_stage.py` once and commit the result.

Take the pack entry's word over the class here; it is the more specific rule and
it says so in place.
...
| Gates `DevStg-Reqs`–`DevStg-Impl` | Technical review gates SRR / PDR / CDR / TRR (IEEE 15288.2) | Rough altitude match, not a 1:1 mapping: `DevStg-Reqs`≈requirements agreed (SRR), `DevStg-Tests`≈design/impl reviewed (PDR/CDR), `DevStg-Impl`≈verified (TRR). The kit's gates are lightweight self-reviews, not staffed milestone reviews. |
