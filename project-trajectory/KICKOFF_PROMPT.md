...
- `README.md` — the **human front door**: bootstrap lays down a skeleton (it
  ...
  needs are derived so DevBar-Reqs can check them against it. That tag is the purpose
...
- A runnable **check harness** — copy `scripts/check.py` (the reference runs
  ...
  fresh scaffold derives `DevBar-Reqs`) — never hand-set; `check.py` and CI read it. Run
...
- **DevBar-Reqs — Requirements, UX & constraints.** SN list complete with priorities,
...
- **DevBar-Tests — Decomposition & test coverage.** Every SR → ≥1 LLR (or marked
...
## End-user / edge-case lens (apply throughout, especially at DevBar-Reqs and the final read)
...
4. Run **DevBar-Reqs**: as Stakeholder, write SN-### (incl. edge cases); as UX, capture
