...
- `README.md` — the **human front door**: bootstrap lays down a skeleton (it
  ...
  needs are derived so DevStg-Reqs can check them against it. That tag is the purpose
...
- `docs/log.md` — the append-only history status.md points at: gate sign-off
  table, verdicts, approved decisions (copy `LOG.template.md`).
...
- A runnable **check harness** — copy `scripts/check.py` (the reference runs
  ...
  that runs the same command. The rung the repo is IN lives in `docs/stage`,
  **generated by `scripts/derive_stage.py` from the artifact states** (a fresh
  scaffold derives `DevStg-Reqs`) — never hand-set; `check.py` and CI read it and
  run every step declared at or above it. Run `python scripts/derive_stage.py`
  after approving artifacts (`--check` is the freshness gate).
...
`[attestation] human_approval_through` dial in `docs/process.toml`, the one home for every
process dial (default `attended`: pause for human approval at each
gate; the `single-approve`/`autonomous` levels and their mechanics:
`PROCESS.md` §4 + `PROCESS_OPTIONS.md` "Gate authority levels").

- **DevStg-Reqs — Requirements, UX & constraints.** SN list complete with priorities,
...
- **DevStg-Tests — Decomposition & test coverage.** Every SR → ≥1 LLR (or marked
  Analysis/Inspection/Attest); every SR and LLR has ≥1 TC; traceability reports
  **0 orphans**; the harness runs locally and in CI.
- **DevStg-Impl — Implementation (test-first).** Each DevStg-Tests TC becomes a *failing* test
...
## End-user / edge-case lens (apply throughout, especially at DevStg-Reqs and the final read)
...
2. **Recommend a gate-authority level** from the brief, risk-calibrated per the
   ...
   `human_approval_through` in `docs/process.toml` (non-default levels get the deviation
   register — `PROCESS_OPTIONS.md` "Gate authority levels").
3. Scaffold the artifacts and the check harness (empty registries + headers).
4. Run **DevStg-Reqs**: as Stakeholder, write SN-### (incl. edge cases); as UX, capture
...
- **Release cadence (one-off deliverable vs. versioned releases):** _(decides
  whether DevStg-Release + the release checklist apply)_
- **Gate authority (attended / single-approve / autonomous):** _(who accepts
