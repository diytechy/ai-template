...
def generate_status(docs, root, last_train=""):
    ...
    lines = [
        ...
        "- **Needs <human>:** see [open-items.html](open-items.html) if present.",
        "",
...
_STATUS_MAP_MARKERS = ("docs/status.md", "docs/requirements/open-items.csv")
_DISPOSITION_REGEN = (
    ...
    # WI-322: the owner decision surface is generated too, from the open-items
    # registry plus the spine's Draft/Modified rows. Same disposition contract as
    # its siblings — opt in by carrying either the registry or the rendered view.
    (
        "open-items",
        "gen_open_items.py",
        (),
        ("docs/requirements/open-items.csv", "docs/open-items.html"),
    ),
)
...
def _regenerate_pending(root, journal):
    """Best-effort refresh of the generated owner decision surface
    (docs/open-items.html) at a terminal decision — called AFTER run-state is
    written so a NEEDS-HUMAN ask is captured into the surface the owner reads.
    The projection is a pure view of durable state (blocked rows,
    refs/llm/conflict records, quarantined trains, the run-state ask), so
    regenerating here cannot lose data. A generator failure must NEVER crash the
    terminal path: it is journaled and the harness `open-items` freshness gate
    catches the staleness on the next run.

    WI-322 retargeted this from `gen_trajectory.py --status` (which spliced the
    projection into the retired docs/open-items.md) to `gen_open_items.py`, which
    renders it into the view. Vacuous when a repo carries neither the registry
    nor the view (a non-adopter)."""
    docs = Path(root) / "docs"
    if not (
        (docs / "requirements" / "open-items.csv").is_file()
        or (docs / "open-items.html").is_file()
    ):
        return
    gen = Path(__file__).resolve().parent / "gen_open_items.py"
    try:
        proc = _run_captured(
            [sys.executable, str(gen), "--root", str(root)], timeout=120
        )
