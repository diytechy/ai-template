...
(integrate.py) both stand on this layer:
  ...
  - the declared-surface reads — `read_declared`, `declared_config`,
    `pause_reason`, the WI-148 blackout window — and the stop banner +
...
# codes carry the outcome now.) NEEDS-JUDGEMENT's one-line ask is the stop
# banner's headline (WI-127).
#
# RENAMED from NEEDS-JUDGEMENT at P14 (decision D-6, WI-417 judgement 4): the word
# names the owed ACT, and who supplies it follows from the ratification
# boundary rather than from the label. `EXIT_NEEDS_JUDGEMENT` is still **7** —
# the number is a wire contract with every worker and launcher, and a rename
# that moved it would break scripts nobody in this repo can see.
END_STATES = ("DONE", "BLOCKED", "NEEDS-JUDGEMENT")
...
def read_declared(path, default):
    """Read a one-word declared STATE file — the first non-empty, non-comment
    line, or `default` when absent/empty.

    Its surviving callers are the STATE surfaces, not the configuration ones:
    `docs/gate` (generated by derive_gate.py) and friends. Every BEHAVIOUR dial
    moved to `docs/config.toml` at the P13 cutover and is read through
    `declared_config` below — the git hooks and check_privacy.py no longer apply
    this rule at all, so a new caller reaching for it should ask first whether
    what it wants is configuration."""
    try:
...
# --- the canonical configuration (the P13 cutover) ---------------------------
# `read_declared` above survives for the surfaces that are STATE, not
# configuration — docs/gate, docs/work/pause. Every BEHAVIOUR dial now comes
# through here, from the one validated document (contracts §1).


def declared_config(root, keys=()):
    """`(cfg, refusals)` — the repo's validated configuration and the refusal
    lines a caller must print before it stops.

    `refusals` is a list of ready-to-print strings and a NON-EMPTY LIST MEANS
    REFUSE. Three rungs feed it, and all three fail closed:

      - the loader's own findings (unknown key, wrong type, out-of-range value,
        absent or unsupported `schema`);
      - `unconverted_findings` over `keys` — the caller's OWN read set — which
        catches a repo whose retired one-word file still declares a dial that
        the canonical document leaves unset. That is the fail-open the cutover
        was built around: answering such a repo with a schema default obeys a
        policy nobody wrote.
      - `mixed_source_findings` over the same read set (wired at P14, with the
        deletions): a repo where BOTH the retired file and the canonical key
        declare the dial. Until P14 this had no call site on purpose — the
        migrating repo carried both — and turning it on is what makes the
        deletion final rather than optional.

    The two migration rungs are complementary halves of one question, and each
    is a refusal because the alternative is a silent precedence rule: unset +
    live retired file obeys a policy nobody wrote, set + live retired file lets
    the adopter read one source while the loop obeys the other.

    Passing the read set rather than the whole schema is deliberate: a refusal
    about a dial this caller never reads is noise, and noise is how a real
    ...
    findings = (
        list(findings)
        + config.unconverted_findings(root, keys, cfg=cfg)
        + config.mixed_source_findings(root, keys)
    )
    return cfg, config.refusal_lines(findings)


# The retired `docs/gate-policy` vocabulary, derived from the canonical boundary.
# NOT a second dial: `attestation.human_ratification_through` is the authority
# after the cutover, and these three words are the internal vocabulary the ruled
# tables still speak — the §A8 admission table (`dispatch._kind_action`), the
# adjudication flip (`intake.flip_verified`, ruled decision 2), the paging
# semantics (`plan_round.page_action`) and the escalation semantics
# (`agent_route.failure_action`). Those tables are RULED text with their own
# LLR rows (LLR-149, LLR-153, LLR-132, LLR-081); rewriting them in ordinals
# would be a policy change dressed as a cutover, so the cutover moves the DIAL
# and leaves the rulings alone.
#
# The inverse is READ OFF the converter's own tables rather than invented:
# `config_migrate.GATE_AUTHORITY` maps `attended` -> 3, and its `AMBIGUOUS` table
# names 1 and 2 as `single-ratify`'s candidate boundaries and 0 as
# `autonomous`'s. So each of the four boundary values has exactly one enum word,
# and `tests/test_config_cutover.py` pins this tuple against those two tables so
# a converter edit cannot leave the inverse behind.
BOUNDARY_LEVEL = ("autonomous", "single-ratify", "single-ratify", "attended")


def escalation_level(boundary):
    """The ruled gate-authority word for one ratification boundary.

    Refuses an out-of-range boundary by raising rather than clamping: clamping
    would route a whole tier to a machine the adopter meant to reserve for a
    human, which is the one direction this dial must never drift in. The loader
    already refuses 0..3 violations, so reaching this raise means a caller
    invented a number.
    """
    if isinstance(boundary, bool) or not isinstance(boundary, int):
        raise ValueError(
            "ratification boundary {!r} is not an integer".format(boundary)
        )
    if not 0 <= boundary < len(BOUNDARY_LEVEL):
        raise ValueError(
            "ratification boundary {!r} is outside 0..{}".format(
                boundary, len(BOUNDARY_LEVEL) - 1
            )
        )
    return BOUNDARY_LEVEL[boundary]


BOUNDARY_KEY = "attestation.human_ratification_through"


def ratification_level(root):
    """`(level, refusals)` — the ruled gate-authority word this repo's declared
    boundary implies, for the three admission sites that consume it.

    One helper rather than three copies of load-boundary-then-map: the three
    sites (`agent_loop`, `dispatch`, `intake`) must never disagree about what
    the dial says, and a shared refusal list means they cannot disagree about
    when to stop either.
    """
    cfg, refusals = declared_config(root, (BOUNDARY_KEY,))
    if refusals:
        return None, refusals
    try:
        return escalation_level(cfg.get(BOUNDARY_KEY)), []
    except ValueError as exc:
        return None, ["config: REFUSED - {} ({})".format(BOUNDARY_KEY, exc)]
...
# The declared `automation.blackout` window: `HH:MM-HH:MM` (UTC), active
# Mon–Fri. Inside the window the coordinator starts no new session (the
# in-flight one already wrapped, the same graceful semantic as docs/work/pause)
# — it waits out the window, then resumes automatically, so a single walk-away
# launch survives the blackout. An empty/malformed value, or `start == end`,
# disables it, and that is the DEFAULT: the retired `docs/blackout` template
# shipped a 12:00–19:00 window, which is a policy the kit chose for every
# adopter rather than one any of them declared. Deleted with the file at P14.
# (These parsers still take a raw line: the value is a string either way.)
...
def blackout_banner(
    window, resume_at, wake_seconds, policy_file="docs/config.toml automation.blackout"
):
    ...
    SOURCE (the retired `docs/blackout` was deleted at P14; sending an operator
    to a file that is not there is worse than saying nothing), the raw `HH:MM-HH:MM` UTC `window`, its weekday-only scope, the resume
...
def preflight(root, template, args):
    ...
    if code != 0:
        ...
        # The canonical dial since the P13 cutover. A refusal is a preflight
        # FAILURE, not a shrug: the loop is about to run unattended sessions
        # against a repo whose privacy posture it could not read.
        cfg, refusals = declared_config(root, ("policy.privacy_check",))
        failures.extend(refusals)
        enabled = not refusals and cfg.policy.privacy_check
        if enabled:
