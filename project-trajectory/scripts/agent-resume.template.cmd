...
REM docs/status.md, docs/gate + docs/config.toml - so resuming work
...
REM by filling the slot, declaring docs/config.toml, and running this.
...
REM with the policy.review_rounds reviewer dial - cross-provider dual review
REM is the recommended policy.review_rounds = 2 config), e.g.:
...
if not defined AGENT_CMD (
  ...
  echo in agent-resume.sh. Filling it ^(and declaring docs/config.toml^) is your
