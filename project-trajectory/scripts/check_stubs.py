#!/usr/bin/env python3
"""No-stub / substance detector: flag implementations that only *exist* (process.md §4 DevBar-Release).

The kit's gates verify *traceability + coverage + tests pass*; none of them, on
its own, asserts that an implementation has **substance**. TDD (DevBar-Release) mitigates
...
Analysis SRs have no automated test to fail. The DevBar-Release *no-stub* exit criterion
...
is the human/LLM inspection the DevBar-Release criterion names, and a legitimately tiny pure
...
def render_report(findings, n_files):
    lines = [
        ...
        "product-layer tripwire for the DevBar-Release no-stub criterion (process.md §4). A "
...
def main():
    ...
    if args.strict:
        ...
        sys.exit(1)
    # Warn-first: surfaced for the DevBar-Release Inspection, but never blocks on its own.
