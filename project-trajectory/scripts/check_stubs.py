#!/usr/bin/env python3
"""No-stub / substance detector: flag implementations that only *exist* (process.md §4 DevStg-Impl).

The kit's gates verify *traceability + coverage + tests pass*; none of them, on
its own, asserts that an implementation has **substance**. TDD (DevStg-Impl) mitigates
...
Analysis SRs have no automated test to fail. The DevStg-Impl *no-stub* exit criterion
...
is the human/LLM inspection the DevStg-Impl criterion names, and a legitimately tiny pure
...
def render_report(findings, n_files):
    lines = [
        ...
        "product-layer tripwire for the DevStg-Impl no-stub criterion (process.md §4). A "
...
def main():
    ...
    if args.strict:
        ...
        sys.exit(1)
    # Warn-first: surfaced for the DevStg-Impl Inspection, but never blocks on its own.
