...
def load_seam_modules(path):
    """`{LLR-###: Module}` from the design registry BESIDE the IF registry — the
    join a seam's provider derives through since WI-455 shed the derivable
    `Provider` cell. `{}` when that registry is absent, which reads as "every
    provider is whatever its own row states"."""
    return {
        (r.get("LLR-ID") or "").strip(): (r.get("Module") or "").strip()
        for r in spine_carrier.load(
            Path(path).parent / "low-level-requirements.toml", "LLR-ID"
        )
...
def _seam_edges(if_rows, llr_modules, known_norm):
    """The module<->module IF seams as `(src_node, dst_node, IF-###)` triples,
    provider -> consumer. A row with no provider, or one whose endpoint sits
    outside the map, draws nothing: a seam to a file or an external actor is a
    How-SW dashboard node, not a code-import edge."""
    edges = set()
    for r in if_rows or []:
        iid = (r.get("IF-ID") or "").strip()
        if not IF_ID_RE.fullmatch(iid) or iid.endswith("-000"):
            continue
        src_n = known_norm.get(
            _norm_module(_kitspine.seam_provider(r, llr_modules or {}))
        )
        if not src_n:
            continue
        for consumer in _kitspine.seam_consumers(r):
            dst_n = known_norm.get(_norm_module(consumer))
            if dst_n and dst_n != src_n:
                edges.add((src_n, dst_n, iid))
    return edges


def build_dependency_diagram(src_roots, if_rows=None, llr_modules=None):
    ...
    runs provider -> consumer, and the provider is resolved through
    `llr_modules` (`{LLR-###: Module}`) for every row that states none because
    its owner derives it (WI-455)."""
    files, internal_names = _module_files(src_roots)
    note = (
        "_Generated by `scripts/gen_arch_map.py` from the source tree (AST): "
    ...
    if_edges = _seam_edges(if_rows, llr_modules, known_norm)
...
def main():
    ...
    if args.mode == "files":
        ...
        diagram = build_dependency_diagram(
            src_roots,
            load_interfaces(args.interfaces),
            load_seam_modules(args.interfaces),
        )
