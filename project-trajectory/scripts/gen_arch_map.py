...
def build_flow(src_roots, entry):
    ...
    calls.sort()
    note = ("_Generated by `scripts/gen_arch_map.py --flow {0}` — the ordered "
    ...
    if summary:
        ...
        lines.append("")
    if not calls:
        lines.append("_(no internal calls found — is `{}` the orchestrator, and "
...
def main():
    ...
    for doc in docs:
        ...
        if flow is not None:  # FLOW markers are optional per doc
            updated = splice_region(updated, BEGIN_FLOW, END_FLOW, flow, doc,
                                    required=False)
        if args.check:
            if updated != current:
                stale = True
