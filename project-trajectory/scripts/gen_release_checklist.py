    ...
    for line in md_path.read_text(encoding="utf-8").splitlines():
        ...
        if header is None:
            if any("UN-ID" in c for c in cells):
                header = cells
                need_i = next((i for i, c in enumerate(header)
                               if "Need" in c), 1)
                acc_i = next((i for i, c in enumerate(header)
                              if "Acceptance" in c), None)
            continue
...
def main():
    ...
    ifs = [r for r in load_csv(docs / "requirements" / "interfaces.csv")
    ...
    L = ["# Release Checklist — {}".format(stamp),
         "",
    ...
    L += ["", "## 2. Human-verified requirements "
          "(Demonstration / Manual / Inspection)", ""]
    if human_srs:
        for r in human_srs:
            L.append("- [ ] **{}** [{}] — {} (AcceptanceCriteria of {})".format(
                r["SR-ID"], r.get("Verification", ""),
                r.get("Title", "").strip(), r["SR-ID"]))
    else:
    ...
    if manual_tcs:
        for r in manual_tcs:
            L.append("- [ ] **{}** [{}] — {} (verifies {})".format(
                r["TC-ID"], r.get("Tier", "") or "Manual",
                r.get("Method", "").strip(), r.get("Verifies", "")))
    else:
    ...
    if provided_ifs:
        ...
        for r in provided_ifs:
            L.append("- [ ] **{}** ({} {}) — {} still satisfies the published "
                     "contract (refs {})".format(
                         r["IF-ID"], r.get("Version", ""), r.get("Stability", ""),
                         r.get("Counterpart", ""), r.get("SR-Refs", "")))
    ...
    print("Release checklist -> {}  (UN={} human-SR={} manual-TC={} IF={})".format(
