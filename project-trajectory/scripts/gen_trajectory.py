...
How-SW views + panels), traj_status.py (the --status snapshot + pending
projection) — each re-exported here, so
...
# traj_views.py (the What/When/How-SW views + panels), traj_status.py (the
# --status snapshot + pending projection) — and this facade re-exports the
# consumer-read names so every existing `gen_trajectory.<name>` import keeps
# resolving (the seam, and its Contracts: line, stay HERE). These
