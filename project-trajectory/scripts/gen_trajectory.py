...
primitives, palettes, the drill renderer) — each re-exported here, so
...
# The pure layout/routing core lives in traj_graph.py, the parse/sources
# layer (registries, docs, git, the guarded `schedule` import's one home) in
# traj_parse.py, and the SVG/HTML primitives (palettes, svg wrappers, the
# drill renderer) in traj_render.py; this facade re-exports the consumer-read
# names so every existing `gen_trajectory.<name>` import keeps resolving (the
# seam, and its Contracts: line, stay HERE). These
