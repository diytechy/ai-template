...
sources + the guarded `schedule` import) — each re-exported here, so
...
# The pure layout/routing core lives in traj_graph.py and the parse/sources
# layer (registries, docs, git, the guarded `schedule` import's one home) in
# traj_parse.py; this facade re-exports the consumer-read names so every
# existing `gen_trajectory.<name>` import keeps resolving (the seam, and its
# Contracts: line, stay HERE). These
