...
| `commit-identity.template` | Per-repo **commit-identity policy** → copied to `docs/commit-identity`. One value: `inherit` (default, no constraint) or an email glob the commit author must match (e.g. `*@users.noreply.github.com` for a pseudonymous repo). `setup.{sh,ps1}` applies it per clone (repo-local git config, never `--global`); the pre-commit hook blocks a mismatch; `bootstrap.py --commit-identity` sets it at creation (process-options.md "Commit identity & anonymity"). |
