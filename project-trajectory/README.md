...
| `scripts/check_privacy.py` | **Secrets + privacy lint** (Python 3, stdlib only). Two layers: an always-on **secrets floor** (private-key headers + GitHub/Slack/AWS/`sk-…` shapes) that runs in *every* repo — opt out with `off` in `docs/secrets-scan` — and **identity-leak** classes (home-dir paths carrying an OS username, the current account/hostname, off-policy emails, the global-git-config identity) gated on a declared `docs/commit-identity` pattern. Staged-diff mode in the pre-commit hook, `--repo` sweep as a `check.py` process step, `--range` over outgoing history (diffs + commit messages) for the pre-push hook and the sync scrub's base pass; inline `privacy-ok` exempts a documented example line. A pattern floor, not a DLP product — deep secrets scanning stays external (gitleaks, trufflehog). Exits 0 fast only when both layers are off (`inherit` + `secrets-scan off`) (process-options.md "Commit identity & anonymity"). |
...
| `hooks/pre-commit` | Agent-neutral **process floor** → copied to `.githooks/pre-commit`; `setup.{sh,ps1}` enable it via `git config core.hooksPath .githooks`. One POSIX hook (works on Git for Windows too) running the fast, always-valid checks: code-map freshness + id integrity (`trace.py --strict-integrity`), the **secrets floor** on the staged diff (`check_privacy.py`; every repo unless `docs/secrets-scan off`) (+ ruff format on staged files if installed). Orphan strictness is gate-scoped — the hook never blocks a legitimate early-stage commit; the full gate bar stays in `check.py`/CI. |
| `hooks/pre-push` | **Secrets floor + anonymous-repo privacy backstop** → copied to `.githooks/pre-push` (same `core.hooksPath` wiring). Every repo: the deterministic **secrets floor** scans the **full outgoing range** (`check_privacy.py --range`), tooling-gated (a missing interpreter skips, never fail-closed). Under a declared anonymous `commit-identity` pattern it additionally lints the identity classes and invokes the **`REVIEW_CMD`** reviewer slot (env var or `git config privacy.reviewcmd`) over history — diffs *and* commit messages — **failing closed** when the reviewer can't run. An optional backstop, stated honestly: the review's primary home is the sync ritual's scrub step, because hooks are per-clone and `git push --no-verify` is git's own escape hatch (process-options.md "Commit identity & anonymity"). |
...
| `commit-identity.template` | Per-repo **commit-identity policy** → copied to `docs/commit-identity`. One value: `inherit` (default, no constraint) or an email glob the commit author must match (e.g. `*@users.noreply.github.com` for a pseudonymous repo). `setup.{sh,ps1}` applies it per clone (repo-local git config, never `--global`); the pre-commit hook blocks a mismatched author and (via `check_privacy.py`) patterned content leaks; `bootstrap.py --commit-identity` sets it at creation (process-options.md "Commit identity & anonymity"). |
