...
| `scripts/check_privacy.py` | **Privacy-leak lint** for anonymous repos (Python 3, stdlib only): when `docs/commit-identity` declares a pattern, scans for the deterministic leak classes — home-dir paths carrying an OS username, the current account/hostname, off-policy emails, the global-git-config identity in content, key/token shapes. Staged-diff mode in the pre-commit hook, `--repo` sweep as a `check.py` process step, `--range` over outgoing history (diffs + commit messages) for the pre-push hook and the sync scrub's base pass; inline `privacy-ok` exempts a documented example line. A pattern floor, not a DLP product — deep secrets scanning stays external (gitleaks, trufflehog). `inherit` repos skip at zero cost (process-options.md "Commit identity & anonymity"). |
...
| `hooks/pre-push` | **Anonymous-repo privacy backstop** → copied to `.githooks/pre-push` (same `core.hooksPath` wiring). Inert under the default `inherit` policy; under a declared pattern it lints the **full outgoing range** (`check_privacy.py --range`) and invokes the **`REVIEW_CMD`** reviewer slot (env var or `git config privacy.reviewcmd`) over history — diffs *and* commit messages — **failing closed** when the reviewer can't run. An optional backstop, stated honestly: the review's primary home is the sync ritual's scrub step, because hooks are per-clone and `git push --no-verify` is git's own escape hatch (process-options.md "Commit identity & anonymity"). |
