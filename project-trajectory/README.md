...
| `privacy-check.template` | Per-repo **privacy-check toggle** → copied to `docs/privacy-check`. One value: `false` (default — the privacy gate is off) or `true` to scan the commit author email and committed content/messages for PII / identity leaks. Identity (which account authors) is the user's own git config, not pinned here; the exempt-email allowlist (`EXEMPT_EMAILS`, default `*noreply*`) lives in `check_privacy.py`. The always-on secrets floor (`docs/secrets-scan`) runs regardless. `bootstrap.py --privacy-check` sets it at creation (process-options.md "Commit identity & anonymity"). |
