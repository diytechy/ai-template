<!-- DISPATCHER NOTES (stripped before the prompt is sent)
     ...
     THREE slots since C7 (docs/plans/2026-08-30-stall-guard-plan.md), all
     filled by `str.replace`: `{verdict}` (the path the verdict must land at),
     `{trunk}` (the primary checkout's branch — the integration trunk), and
     `{process_doc}` (docs/process.md in a scaffolded repo; the kit master in
     the meta-repo). An operator override file may carry the same slots.
