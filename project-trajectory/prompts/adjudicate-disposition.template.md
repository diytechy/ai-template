...
3. **The successor is MANDATORY (OI-73).** A PARTIAL or CANCELLED close MUST queue at least one successor — an open item ALONE no longer discharges it, and there is no third exit. Continuing partial work means MINTING A SUCCESSOR, never reviving a closed row — closed work items are never revived, and a scope definition never changes to mean something else. DRAFT it (do not create it; the machinery mints your draft at this row's own close) as a fenced `toml` block under a `## Dispositions` heading in THIS SESSION'S OWN SPEC — never in the verdict file — written as TOP-LEVEL `key = value` lines (no `[table]` header; intake refuses an unknown table key), carrying at minimum `title` (at most 120 characters — a concise name, rationale goes in the prose beside the block), `workstream`, `buildtier`, and `supersedes = "<the closed WI id>"` — a bare string for one id, or a TOML list (`supersedes = ["WI-1", "WI-2"]`) where the successor continues several rows — so the partial work keeps its thread across the id change. The mint REPLACES the superseded row's inbound hard `needs` edges with the successor, so nothing is left waiting on the terminal row.

4. **Is the answer human-owed?** Where a genuine part of what is left is the OWNER's to rule and you found no alternative route, add `open_item = "<the one-line question>"` to the successor's block. The machinery mints a `pending` open item (id from the watermark's OI space) and lands its id in the successor's `needs`, so the successor cannot become ready until the owner rules — the ruling gates the work, not your restraint. This is a TYPED DEPENDENCY of the successor, never a standalone exit. Then estimate `buildtier` (`quick` | `medium` | `strong`) from what is left to do — the remaining scope, not the frustration in the report — and set `planmode = "dual"` only when the remaining work is a genuine design fork rather than a build.
...
`OUTCOME` is YOUR ruling on what the close means, which may differ from the lane's claim; `successors` is the number of `## Dispositions` blocks you drafted (at least one for a PARTIAL or CANCELLED ruling). Commit that verdict file (an adjudication is a recorded verdict — its one home), ending that commit with the trailer `WI: {wi}` — the coordinator learns a judgement is recorded from that trailer and from nothing else, so a verdict committed without it leaves this row open. YOU DO NOT MOVE YOUR OWN SPEC: once your verdict is recorded, the machinery CLOSES this row for you (it fills a `## Deliverable`, clears `specref`, archives the spec into `complete/`, and mints your `## Dispositions` drafts into `queued/` at the merge). A disposition that recorded a verdict but drafted NO successor is REFUSED at that close and the run stops for a human — so draft the successor before you commit. The CLOSED ROW YOU JUDGED is untouchable — never move it, never edit its scope; and the only successors that exist are the ones you drafted in THIS row's `## Dispositions` section.
