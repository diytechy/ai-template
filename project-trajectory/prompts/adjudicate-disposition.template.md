...
`OUTCOME` is YOUR ruling on what the close means, which may differ from the lane's claim; `successors` is the number of `## Dispositions` blocks you drafted. Commit that verdict file (an adjudication is a recorded verdict — its one home), ending that commit with the trailer `WI: {wi}` — the coordinator learns a judgement is recorded from that trailer and from nothing else, so a verdict committed without it leaves this row open. THEN CLOSE YOUR OWN ROW (C6 — an adjudication lane whose spec stays in active/ is resumed forever, each resume a fresh judge finding nothing to judge): fill this spec's `## Deliverable` body (placed BEFORE `## Context` — after it parses as empty) with the `OUTCOME:` line and the verdict path; clear the spec's `specref` line; move the spec with `python scripts/spec_move.py` to docs/archive/work/complete/; and end that close commit with the same `WI: {wi}` trailer. The CLOSED ROW YOU JUDGED is untouchable — never move it, never edit its scope; the only spec you close is this adjudication row's own.
