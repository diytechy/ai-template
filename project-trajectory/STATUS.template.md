...
- **Stage:** 0 (needs in process) · next gate: **DevStg-Reqs** — Requirements, UX &
  constraints _(both derived from `docs/stage` — its `stage =` field is the rung
  the repo is IN, regenerated by `scripts/derive_stage.py`, never hand-set; keep
  this line matching it. A repo is IN a rung and PASSES the boundary a human
  signs; the harness runs every step declared at or above the current rung —
  process.md §4 "Stages and gates")_
- **Round:** 1
