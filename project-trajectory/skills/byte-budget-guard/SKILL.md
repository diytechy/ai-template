...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **162,324** as of 2026-07-30/WI-373 (+1,207 on the 161,117 stamp: the finding lifecycle — a finding is a claim its owner confirms by reproduction or refutes before code changes, refutation a recorded outcome — plus the re-review round scope, landed in the LLM-gate verdict protocol with a reviewer-independence pointer; the review-round era's measured fix-aimed recursion is the reason); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
