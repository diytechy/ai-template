...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **158,557** as of 2026-07-19/WI-237 (+463: the sanctioned owner dev-merge resync note in the crash-safety/reconcile section — a claim is read only from a train's novel, integration-unreachable commits); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
