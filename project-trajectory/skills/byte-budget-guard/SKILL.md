...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **166,123** as of 2026-08-01/WI-378 (+2,966 on the 163,157 stamp, of which **+846 was inherited unstamped, all of it WI-384's** (`e869770d` + `eb1b25f5`) — measured at each merge: 163,157 at wi-380, 164,003 at wi-384, 164,003 at wi-386, so WI-380 and WI-386 changed this file by **0 bytes** but likewise did not re-stamp, which is exactly the silent growth this row exists to catch — plus **+2,120** for the verdict-freshness paragraph under "The LLM-gate verdict protocol", carrying the staling census, the command that derives its population, and why `docs/work/` stays inside the freshness window); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
