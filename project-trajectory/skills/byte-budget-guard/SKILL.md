...
| `project-trajectory/PROCESS.md` | **watched** (baseline **64,319** as of 2026-07-31/WI-375 (+18 on the 64,301 stamp: the §4 coordinator sentence re-pointed from the deleted `docs/run-state` artifact to the stop banner + typed exit codes — a truth fix, not an expansion); keep flat — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report |
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **166,314** as of 2026-08-01/WI-378 (+3,157 on the 163,157 stamp, of which **+846 was inherited unstamped, all of it WI-384's** (`e869770d` + `eb1b25f5`) — measured at each merge: 163,157 at wi-380, 164,003 at wi-384, 164,003 at wi-386, so WI-380 and WI-386 changed this file by **0 bytes** but likewise did not re-stamp, which is exactly the silent growth this row exists to catch — plus **+2,311** for the verdict-freshness paragraph under "The LLM-gate verdict protocol", carrying the staling census, the command that derives its population, the post-ordering 2-in-11 figure, and why `docs/work/` stays inside the freshness window); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
