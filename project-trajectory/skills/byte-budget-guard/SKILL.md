...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **165,557** as of 2026-08-01/WI-378 (+2,400 on the 163,157 stamp, and only 1,554 of it is this row's: **+846 was inherited unstamped** — WI-380/384/386 all landed 2026-08-01 without re-stamping, which is exactly the silent growth this row exists to catch — plus **+1,554** for the verdict-freshness paragraph under "The LLM-gate verdict protocol", carrying the measured staling census and why `docs/work/` stays inside the freshness window); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
