...
Outcomes I want:
 ...
 Then make a plan to build out new stakeholder needs and all depdendencies.  (These may exist in some form already, how does it affect the current spine definition?) You may wish to use docs/architecture.md which for the most part should be up-to-date.  I want to ensure the entire build loop is properly mechanized, and that text templates being fed into LLM CLIs are easy to review and verify their prose are also correct, if that could be provided inside the plan as well for the review to understand how this may be influencing automation thus far.  This plan touches on recent changes around the handback system which -while fixing some issues- created a few others.  I want to allow the branch to make it's own judgements, while allowing the adjudicator to review and override that judgement.  In this way, work item scope definitions do not change in scope, only weather they were fully delivered. ==>
...
Seperately, a human will need at times to always require attestation at level 4 (After all test cases have been validated), just to give themselves an opportunity to review the full breakdown.  How does this get set?  This would likely change more frequently then the human attest level, but they are very closely related.
 ...
 1. First, the adjudicator will go through any handback documents and determine if they were displositioned properly, if they need to be moved to a different folder, and if needed will create work items to capture spine changes at the approriate attestation level to close any gaps identified by the handback, while ensuring any new work items do not conflict with current queued work items.  Note the adjudicator -when making work items- is in charge of estimating the agent model level required for the task at hand.  (duel plan, standard plan ,etc). After a work item is attempted and closed (completed / partial / cancelled) it SHOULD NOT BE REVIVED, and instead a new work item should be minteded.  Work items are effectively branches with a file reference attached noting the intended scope (weather that scope was achieved or not)
...
You are the supervisor of this repository's mechanized loop, on branch contract_split — the integrator's trunk is whatever HEAD is; stay on this branch, never merge to main, never push (push = "human"); commit is permitted. Read CLAUDE.md, the session-protocol skill, docs/status.md (RESUME HERE — the stall guard is LANDED), docs/decisions-for-review-2026-08-31.md (39 delegated decisions; 33–39 are the last sitting's), and the memory file unattended-run-2026-08-30-traps.md (its items 19–24 are the last sitting's traps).  Spin up opus agents where appropriate to perform edits or analysis to keep your context usage low.

State at handoff (trunk 62c9798d, 2026-08-30): WI-548 (the stall-guard plan C1–C7, docs/plans/2026-08-30-stall-guard-plan.md) is merged through the slot; the tracked docs/work/pause is STILL ARMED by my instruction; no lane is active; the wi508-architectural-remap-HELD-for-owner-verdict branch is held for me. The queue behind the pause: WI-549 (the sample-dial spot-check adjudication of WI-548's clean close — the first lane the new close ritual and the C2/C4/C5 machinery run on live), WI-547 (the amendment adjudication over the 17 delegated Rationale deletions), WI-545 (the debt owner's successor — its first commit moves the module-size ratchet pointer), WI-538 → WI-539, WI-535 → WI-540 → WI-541, WI-536; the scheduler's generated frontier is the order of record.
...
Do not touch the held wi508 branch, WI-543 (deferred), the approval dial, [policies], the TERRA reasoning-effort dial (kept on measurement, decision 36), or any ruled open item — those are mine.
Delegation for stops: the same as before — dispose NEEDS-HUMAN stops with the best decision the information supports, through the kit's own mechanisms, and record each with its alternative in docs/decisions-for-review-2026-08-31.md (continue the numbering at 40). Traps to expect: agent-resume must run from PowerShell by absolute path; a helper that prints a reviewer's stream needs agent_common._utf8_console(); the Bash tool mangles very long heredocs (write big patch scripts with the Write tool). End by writing the session fragment under docs/log.d/ (then trunk_step.py --compile-log on trunk once it is committed), updating RESUME HERE with a "for the owner's review" list, and stopping with the repo drained and quiet.