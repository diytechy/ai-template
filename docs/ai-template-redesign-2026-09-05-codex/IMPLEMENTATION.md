...
**Directory shape (adopted from the Claude plan after the second Fable review):** these responsibilities land in five packages with one-way downward imports — `spine/` (registries, tracer, stage, approval, harness; always shipped), `queue/` (work items, frontier, intake judgement; loop-independent), `loop/` (dispatch, lanes, sessions, review, integration; the opt-in layer), `surfaces/` (dashboard, owner briefs, status, log; reads all three, imported by none), `scaffold/` (bootstrap, resync, templates, guides). `model.py`/`intake.py`/`schedule.py` map to `queue/`; `run.py`/`review.py`/`integrate.py`/`git_adapter.py`/`agents.py` to `loop/`; `cli.py` stays at the package edge. `tests/test_import_layers.py` already enforces a layering and gets this one. The responsibilities above are the contract; the directory names are the shape.
