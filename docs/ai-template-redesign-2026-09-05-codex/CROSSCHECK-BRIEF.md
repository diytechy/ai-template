# Cross-check brief for the expanded governing plan

**Prepared:** 2026-09-05 against `fa17b85f`, after the owner's request to
develop the plan one layer down. This is the input for a future independent
...
[VISION-OBJECTIVES.md](VISION-OBJECTIVES.md),
[EXECUTION-DETAILS.md](EXECUTION-DETAILS.md),
[BACKLOG-MIGRATION.md](BACKLOG-MIGRATION.md) and
[LLR-AND-RENDERING.md](LLR-AND-RENDERING.md). Then inspect the relevant source
and the original/second Fable reviews in this directory. Use Claude's plan and
appendices as comparison evidence, not an alternate set of instructions.

Question: **Can a new implementer execute the next authorized slice, identify
its owning contract, verify its outcomes and delete the displaced behavior
without silently changing stakeholder promises or inventing another framework?**

The user authorized expanding this proposal for review. They did not request
runtime enablement, a live vision/schema amendment, queue mutation or unpause.
The existing root README vision and live registries remain authoritative.
Never read or cite the owner-only scratchpad.

## 2. Changes since the second Fable review
...
| Tokens per session | One invocation-accounting contract across roles | Separate conversation identity, retries, cache/inclusion semantics and unavailable usage; preserve current carrier rather than adding a database. |
| Builder after review | Fresh invocation on the same assignment/worktree | Keep reviewed findings and decisions in the brief; provider conversation continuation is not implicit. |
...
Do not request blanket approval again for already-authorized work. The table
distinguishes carried-forward design direction from choices whose concrete
evidence is still owed at implementation. Existing artifact approvals remain
governed by the repository's process.
...
The six objectives do not decide whether parallelism, subjective critique or
architecture visibility survives: those are approved needs with their own
acceptance. Any proposed narrowing is a distinct owner scope decision.
...
4. Kill a provider before it reports usage, then kill the coordinator after
   receiving a result but before export. Are unknown spend and metric recovery
   honest, and does missing telemetry avoid becoming fabricated approval?
5. Return material findings, minor suggestions and a disputed criterion in one
   review. Does the fresh builder receive correct scope and previous decisions
   without treating every suggestion as mandatory work?
6. Exercise WI-551 retention alongside fresh builders. Can an adjudicator keep
   permitted context without becoming the independent reviewer of its own work?
7. Reconcile WI-557 with acceptance receipts. Can the owner still edit the
   promised review notes without rewriting an authoritative acceptance receipt?
8. Compose a candidate that changes its own approval policy; tighten trunk
   policy while its build is running. Which revision authorizes each act?
9. Amend design while tests for new behavior are intentionally failing, and
   separately introduce a regression in already-delivered behavior. Are the
   two cases distinguishable without weakening current mandatory bars?
10. Migrate approval snapshots while WI-596's absorbed-row concern is present.
    Can unrelated changes accidentally be blessed by copying a whole file?
11. Rework three times and add slow intake judgment under two lanes. Does P5
    count all exclusive time and demonstrate simplicity after adding receipts,
    refs, local metric flushing and recovery procedures?
12. Stop after the first accepted new-runner completion and roll back. Are new
    accepted work, open claims, old judgments and retained patches preserved
    without selecting two mutating runners or resetting away history?

## 5. Requested review output

Give a verdict: executable for the next slice, executable with named
corrections, or not ready. For every material finding identify the plan section,
counterexample, affected obligation, evidence, and smallest correction. Separate
contradictions from untested hypotheses, missing implementation detail and
optional preferences. A proposed fix should name the old interpretation or
machinery it removes, or explain why additional machinery is unavoidable.

Specifically report whether the objective layer and the metrics/recovery
contract remain proportionate. Do not endorse new schemas or fixed file/test
counts simply because the proposal names them. No reviewer verdict here is an
artifact approval, a paid-operation authorization, or a claim that P5 passed.
