...
Selection must compare the complete proposed change to its recorded base, account for renamed/deleted files and shared inputs, and broaden on uncertainty. Do not base it only on the last commit. Local and CI invocation must use the same selection result; an always-running required job reports what ran and why a rendering suite was not applicable. Do not leave a required check pending through a workflow-level path skip, and do not report unrun cases as passing.
