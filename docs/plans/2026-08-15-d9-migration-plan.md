...
| 3 | **Build the `last_approved` snapshot mechanism — READER FIRST, ADVISORY** (the design doc §A–§B): new `scripts/baseline_snapshot.py` (`SNAPSHOT_DIR`, `SNAPSHOTTED`, `load_all`, `copy_live`, `is_drifted`, `unanchored_findings`); promote `check_trajectory._split_changed_cells` to `split_changed_cells`; `staged_snapshot_findings` (the mirror invariant) wired into the staged warn loop. **Nothing writes a snapshot yet and no rule is an ERROR yet** — the directory does not exist, so every reader is vacuous by absence | step 1 | mechanical |
| 4 | **`is_drifted` as a derived overlay, running ALONGSIDE `is_modified`** — `reattest_model` selects `is_modified(sr) OR sr_chain_drifts(sr)`; `gen_open_items` and `traj_status` the same. Delete `_attested_baseline`, `_changed_cells`, `declared_since` + its regex, the whole `--since` surface, and the check-the-baseline prose; `ratify_check` becomes a plain regenerate-and-compare. **Drift stays WARN-class; the UNANCHORED rule is not armed** | step 3 | mechanical |
| 4b | **Route the `amendment` adjudicator** — `adjudicate_brief._ASSEMBLERS` gains `amendment`; `{baseline}` = the snapshot stamp, `{rows}` = the drift model (design §C3) | step 4 | mechanical |
...
| 6 | **⟵ THE SIGNING ACT.** Owner rules each `Modified` row; `intake._apply_flips` writes `Approved`, then calls `baseline_snapshot.copy_live(root)` in the same act. **The FIRST snapshot is seeded HERE and only here** — `intake.py snapshot --seed`, in the owner's reviewed commit, after every `Modified` row is ruled and after step 5's rename, so the snapshot is born speaking the current vocabulary and blesses only text the owner just read. `--seed` is unreachable from every loop module (pinned) | brief regenerated post-step-5 | **SIGNING** |
| 7 | **Retire the transitional word**: delete `is_modified`; enum → `{Drafted, Approved, Founded}`; `test_rule_sync` gains the **negative** assertion (no predicate honours a retired word); resolve `intake.py:1591` into a refusal; **arm the UNANCHORED rule as an ERROR** on the `--strict-integrity` floor (a live row at `Approved`+ whose snapshot copy is absent or below `Approved`) — armed only now because before step 6 the snapshot either does not exist or speaks the pre-rename vocabulary, and either state would red every row. This is the successor to repo-lock D-9 consequence 1 | step 6 clears the last `Modified` | mechanical, gated on 6 |
