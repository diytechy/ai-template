...
| 3 | **The four package-wide B-05 properties** — SR-031 (policy readers agree) · SR-034 (stdlib + ledger) · SR-035 (stack-agnostic) · SR-114 (cross-OS): each is a property of EVERY delivered capability at once, so none fits a single one of B-05's five ruled buckets | **A (recommended):** the decomposition axis gains a declared sixth bucket, *package-wide property* — each stays ONE SR covering the whole package (the 13p invariant already reads "one SR per (need, crossing-or-**delivered-property**)"). **B:** no new bucket — each property re-stated inside the five capability groups, ~4×5 partial rows, the fragmentation the anti-duplication rule exists to refuse | §0.2 call 2 |
