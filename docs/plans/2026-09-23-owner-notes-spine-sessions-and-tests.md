...
| S2 | LLR → design expectation: prose now, prefix a separate later decision; no edits to old logs? (§1.2) | **DECIDED**: yes. Ships in the assumption-tier plan's terminology package (its §11 T), with Q9. **REOPENED 2026-09-24:** "expectation" already names needs in the kit's own needs template ("a failure-mode expectation") and in adopters; a different LLR name is proposed ([review pack](2026-09-24-owner-review-pack.md) A3). |
...
S2 and S4 are REOPENED (pack A3, A2); S9's mechanism has two open points
