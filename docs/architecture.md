...
**This file is RETIRING** (owner ruling 2026-08-13u, sitting-2 decision 8):
the authored narrative — "Shape of the product" and the Runtime flows — now
lives in [`runtime-flows.md`](runtime-flows.md), and the derived structure
below is being re-pointed to render straight from the registries and source
into [`PROJECT_STATE.html`](../PROJECT_STATE.html). The requirement spine
...
graph LR
    ...
    m_scripts_check_flows["scripts/check_flows — Design-time runtime-flow check: the DevStg-Test…"]
    ...
    m_scripts_check_vocab["scripts/check_vocab — Retired-vocabulary check: keep the retired `G*`…"]
    m_scripts_derive_gate["scripts/derive_gate — Derive the harness BAR from artifact states — t…"]
    ...
    m_scripts_adjudicate_brief --> m_scripts_baseline_snapshot
    ...
    m_scripts_adjudicate_brief --> m_scripts_trace
    ...
    m_scripts_gen_open_items --> m_scripts_baseline_snapshot
    ...
    m_scripts_trace --> m_scripts_baseline_snapshot
    m_scripts_trace --> m_scripts_check_trajectory
    ...
    m_scripts_baseline_snapshot -. IF-124 .-> m_scripts_adjudicate_brief
    m_scripts_baseline_snapshot -. IF-126 .-> m_scripts_gen_open_items
    m_scripts_baseline_snapshot -. IF-123 .-> m_scripts_intake
    m_scripts_baseline_snapshot -. IF-125 .-> m_scripts_traj_status
    ...
    m_scripts_check_trajectory -. IF-129 .-> m_scripts_baseline_snapshot
    ...
    m_scripts_prompts -. IF-113 .-> m_scripts_adjudicate_brief
    ...
    m_scripts_spine_carrier -. IF-128 .-> m_scripts_baseline_snapshot
    ...
    m_scripts_trace -. IF-127 .-> m_scripts_adjudicate_brief
...
(the DevStg-Impl arch-map-freshness step) fails if this block drifts from source;
...
Imports (internal): `agent_common`, `baseline_snapshot`, `dispatch`, `handback`, `prompts`, `spine_carrier`, `trace`
Contracts (interfaces): IF-113, IF-114, IF-115, IF-124, IF-127

| Public item | Summary | Implements |
|---|---|---|
| `declared_brief(row)` | The row's declared `Brief` cell, normalized; `""` when it declares none. |  |
| `verdict_refusal(brief, verdict_path)` | Why this adjudication's verdict is not acceptable evidence, or None. |  |
...
| `amendment_values(root, row)` | `({baseline, rows}, None)` for a spine row whose ratified text moved after | SN-029 |
...
| `session_body(root, worker, current_wi, session, sha, reviews_dir, templates)` | `(body, verdict_path, hold, brief)` for a NON-REVIEW session — the one | SN-026 |
...
| `stamp(root)` | `(short rev, date)` of the commit that last wrote the snapshot, or |  |
...
| `check_vision(docs, root)` | The root README must state the vision exactly once (process.md §4 DevStg-Reqs). |  |
| `check_inventory(docs, root, docs_dir)` | The root README honors the traceability spine (process.md §4 DevStg-Reqs). |  |
...
| `read_derived_phases(root)` | `{phase-label: bar-level-int}` parsed from the `# basis:` line of the |  |
| `phase_anchors(wis)` | `({(phase, level): wi}, [shape-warnings])` — the phase-anchor WIs parsed |  |
...
### `scripts/check_vocab`
_Retired-vocabulary check: keep the retired `G*` gate tags from growing back._
Contracts (interfaces): IF-118

| Public item | Summary | Implements |
|---|---|---|
| `is_exempt(rel)` | True when a repo-root-relative POSIX path is a carve-out. |  |
| `in_scope(path, root)` | `(rel, reason)` — the scope decision for one file, with the reason stated |  |
| `findings_for(text, rel)` | The retired-tag findings in one file's text, one per offending line. |  |
| `scan(root, list_scope)` | `(findings, scanned_count)` over the whole tree. |  |
| `main()` |  |  |

### `scripts/derive_gate`
_Derive the harness BAR from artifact states — the hybrid, cached selector._
Imports (internal): `spine_carrier`
Contracts (interfaces): IF-050, IF-051

| Public item | Summary | Implements |
|---|---|---|
| `bar_ord(name)` | The position of a bar name on BAR_ORDER — and it RAISES on an unknown value |  |
| `resolve_bar(value)` | A caller-supplied bar name, with any retired gate tag translated via |  |
| `load_csv(path)` |  |  |
...
| `maturity_bar(row)` | An LLR/TC caps the bar only when its own maturity is DRAFTED (`DevStg-Below` |  |
| `is_modified(row)` | The post-approval `Modified` state (WI-316, process.md §7): content changed |  |
| `sn_bar(sn_id, draft_ids, cited_ids)` | A Drafted SN is `DevStg-Below` — and that is the ONLY rung that fires on a |  |
...
Imports (internal): `baseline_snapshot`, `gen_trajectory`, `spine_carrier`, `trace`
...
| `render(root)` | The whole page. Deterministic: every input is sorted upstream. |  |
...
| Public item | Summary | Implements |
|---|---|---|
| `normalize_bar(value)` | A `bar:` cell as a canonical `DevStg-*` name ("" when blank). |  |
...
Imports (internal): `baseline_snapshot`, `check_trajectory`, `spine_carrier`, `trace_text`
...
| `sr_chain_drifts(sid, chain, snapshot)` | True when ANY row in this SR's chain has drifted from the snapshot. |  |
| `reattest_model(root, srs, llrs, tcs, snapshot)` | The STRUCTURED attestation model: one entry per SR owing a human act, |  |
| `newest_ratify_brief(root)` | The live re-attestation brief — the newest `docs/ratify/*.md` — or None. |  |
| `ratify_check(root, srs, llrs, tcs, out_path)` | `(code, message)` for `--ratify modified --check` — a plain |  |
| `reattest_lines(root, srs, llrs, tcs)` | Markdown for the re-attestation brief (`--ratify modified`, WI-316): one |  |
