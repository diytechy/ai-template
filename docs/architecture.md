...
| `check_vision(docs, root)` | The root README must state the vision exactly once (process.md §4 DevBar-Reqs). |  |
| `check_inventory(docs, root, docs_dir)` | The root README honors the traceability spine (process.md §4 DevBar-Reqs). |  |
...
### `scripts/check_flows`
_Design-time runtime-flow check: the DevBar-Tests reviewer reads diagrams, not CSV rows._
Imports (internal): `spine_carrier`
...
### `scripts/check_stubs`
_No-stub / substance detector: flag implementations that only *exist* (process.md §4 DevBar-Release)._
...
| Public item | Summary | Implements |
|---|---|---|
| `normalize_bar(value)` | A `bar:` cell as a canonical `DevBar-*` name ("" when blank). |  |
