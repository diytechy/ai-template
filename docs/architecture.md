...
```mermaid
graph LR
    m_scripts_agent_loop["scripts/agent_loop — Unattended coordinator: loop fresh agent driver…"]
    ...
    m_scripts_agent_loop --> m_scripts_plan_artifacts
    m_scripts_agent_loop --> m_scripts_plan_briefs
    m_scripts_agent_loop --> m_scripts_plan_coverage_step
...
Imports (internal): `agent_route`, `agent_session`, `plan_artifacts`, `plan_briefs`, `plan_coverage_step`, `plan_round`, `schedule`, `score_reviews`
Contracts (interfaces): IF-015, IF-037, IF-041, IF-055
