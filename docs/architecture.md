...
```mermaid
graph LR
    m_scripts_adjudicate["scripts/adjudicate — adjudicate.py — the authority that confirms or …"]
    m_scripts_admit["scripts/admit — admit.py — the ONE transaction that owns every …"]
    ...
    m_scripts_adjudicate --> m_scripts_config
    m_scripts_adjudicate --> m_scripts_intake
    m_scripts_adjudicate --> m_scripts_outcome
    m_scripts_adjudicate --> m_scripts_prompt_render
    ...
    m_scripts_intake --> m_scripts_attest
    ...
    m_scripts_admit -. IF-099 .-> m_scripts_check_trajectory
    ...
    m_scripts_wi_convert -. IF-100 .-> m_scripts_adjudicate
...
### `scripts/adjudicate`
_adjudicate.py — the authority that confirms or overrides a worker's_
Imports (internal): `config`, `intake`, `outcome`, `prompt_render`, `spec_move`, `wi_convert`
...
| `sampling_bucket(event)` | The event's position in `[0, 1)` — the deterministic sampling draw. |  |
| `safety_class_of(spec_row)` | The safety class a work item declares, read from either shape this tree |  |
| `evidence_gaps(outcome_event)` | The named gaps in one outcome event's scope/bar evidence, sorted. |  |
| `needs_adjudication(outcome_event, spec_row, config, *, review)` | Which triggers, if any, put this outcome in front of a dedicated |  |
| `find_event(root, ident, *, kind, ledger)` | `(event, refusal)` for one event id in the ledger that owns `kind`. |  |
| `dispositions_for(root, ident, *, ledger)` | Every disposition already recorded against one outcome event. |  |
| `effective_outcome(root, ident, *, ledger)` | `(outcome word, refusal)` — what the attempt's outcome IS, after |  |
| `adjudicate(root, event_id, verdict, *, by, rationale, ts, ledger, work)` | Record the disposition of one attempt: `(event, [refusals])`. |  |
| `read_lineage(spec_text)` | The lineage a candidate's frontmatter carries: `{supersedes, source_event}` |  |
| `candidate_for_event(root, source_event, work)` | The relpath of the candidate already drafted for one source event, or |  |
| `draft_successor(root, event_id, *, title, remaining, branch, labels, base, ledger, work, **cells)` | LLR-177 — draft the candidate carrying a Partial attempt's REMAINING | LLR-177, SR-151 |
| `draft_remediation(root, failure_event_id, *, title, effort, ledger, work, **cells)` | LLR-186 — draft the one repair candidate for one bar-failure event: | LLR-186 |
| `disposition_brief(root, event_id, evidence, *, ledger, work, cfg)` | Render the `adjudicate-disposition` brief for one outcome event: |  |
| `main(argv)` |  |  |

### `scripts/admit`
_admit.py — the ONE transaction that owns every move into the queue._

| Public item | Summary | Implements |
|---|---|---|
| `registry(root)` | Every work item under `docs/work/`, keyed by id. |  |
| `declaration(record)` | One registry record as the flat declaration `overlap_graph` reads — the |  |
| `overlap_graph(candidates, active, queued)` | LLR-179 — the mechanical overlap between candidates and work in flight, | LLR-179 |
| `spine_refs(data)` | The spine ids one declaration references, sorted and de-duplicated. |  |
| `spine_view(root, docs)` | `({id: (kind, row)}, {id: anchor digest})` for the whole current spine. |  |
| `spine_digest(root, refs, view)` | The 16-hex digest of the spine state a verdict was computed against. | LLR-178, SR-158 |
| `candidate_digests(root, spec_text, view)` | `(scope, spine)` for one spec's CURRENT text — the pair a verdict names |  |
| `ledger_path(root, ledger)` |  |  |
| `read_admissions(root, ledger)` | Every admission event in file order. A malformed line is a hard read |  |
| `latest_admission(root, wi, ledger)` | The NEWEST admission event for one work item, or None. |  |
| `admission_verdict(root, wi, verdict, scope, spine, *, overlaps, ts, ledger, **extra)` | Record the conflict ruling that permits (or refuses) one candidate: |  |
| `admit(root, candidate_path, *, verdict, ts, ledger)` | LLR-178 — validate a `draft/` candidate, move it to `queued/`, and record | LLR-178 |
| `seed(root, ts, by, ledger)` | Write a `pre-transaction` verdict for every queued row that has none. |  |
| `main(argv)` |  |  |
...
| `kind_of(rid)` | The spine tier an artifact id belongs to, read off its declared prefix. |  |
| `find_row(docs, rid)` | `(kind, row)` for the CURRENT text of one artifact, refusing an id that no |  |
| `verdict_action(kind, boundary, actor)` | `enact` or `recommend` — what `actor` may do about a verdict at `kind`'s |  |
| `enact(root, docs, rid, verdict, by, actor, accepted, note, ts)` | Record ONE verdict about an artifact's current text. |  |
...
Imports (internal): `admit`, `check_docs`
...
| `admission_verdict_findings(root)` | LLR-180 — a queued spec's admission verdict is present and CURRENT. | LLR-180, SR-152 |
...
Imports (internal): `agent_common`, `attest`, `check_trajectory`, `dispatch`, `schedule`, `wi_convert`
...
| `ledger_amendments(root, docs)` | The rows whose normative text has MOVED AWAY from what the attestation |  |
| `ledger_amendment_drafts(root, docs)` | Arm (a2)'s drafts: ONE adjudication row listing every moved spine row, |  |
