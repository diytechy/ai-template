...
| Every ANNOUNCED deferral resolves to an open item — the owner's single point of review (OI-41 ruled (e) + always-on, 2026-08-20) | Harness (ERROR) + Harness (advisory) + Reviewer | Three arms, each a FIELD or a COUNT rather than a phrase match, because a matcher's precision is a property of the wording it meets and cannot be bounded before it ships. **ARM 1, the only hard one:** `trace.provenance_allow_findings` — every `docs/provenance-allow` entry names an `OI-###` that resolves in `open-items.toml`, integrity-class on the always-on `--strict-integrity` floor (`tests/test_trace.py::test_an_allow_entry_names_the_open_item_it_defers`, `::test_an_unresolved_allow_entry_reds_the_integrity_floor`). It is hard at birth because a missing field has no false positives, and it makes the unrecorded deferral UNREPRESENTABLE on the declared-exception surface rather than merely detected. **ARM 2 (warn):** `gen_open_items.fragment_declarations` — a `docs/log.d/` fragment declares the ids its session deferred and each must name a pending row. RULED WITH ITS WEAKNESS ON RECORD: a session that defers silently and says nothing passes clean, and nothing here reaches that (`::test_a_session_that_defers_silently_passes_clean` pins the gap as a property rather than a discovery). **ARM 3 (warn):** `gen_open_items.deferral_findings` — zero pending rows while allow entries still defer is a contradiction between two artifacts, and the finding NAMES the entries (not "something, somewhere, defers", which a reader cannot discharge). **Substrate:** the layer is always-shipped process (`process.md` §5) and `check_docs`' S-3 warns on an ABSENT registry, so the rule means the same thing in every repo. Honest residue, stated by the ruling itself: always-on is NOT populated — this row's founding evidence is a registry that was perfectly fresh, rendered "the owner queue is empty", and was telling the truth while ten genuine decisions sat on other surfaces; and no mechanism here catches a decision deferred with no words at all, which is Reviewer-tier |
