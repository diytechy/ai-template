...
|---|---|---|
| Everything traces `SN→SR→LLR→TC`; 0 orphans before a gate | Harness | `trace.py --strict` (DevStg-Tests/DevStg-Impl step; pre-commit `--strict-integrity` floor) |
| No duplicate/malformed id; CSV structure; citation coherence | Harness | `trace.py --strict-integrity` (pre-commit) |
| "Every declared test case passed" is a HARNESS claim, never a cell or a hand-written file, and it expires with the tree it was measured on (OI-30 D2; WI-500) | Harness + Test (structural) | Was a VACUOUS guard — `DevStg-Release` was returned by nothing, so nothing could claim it falsely and nothing could claim it at all. Now the rung has exactly one input: `spine_stage(..., evidence_passed=)`, fed only by `kitlib.stage.evidence_verdict` over `docs/test/evidence`, which `record_test_evidence.py` writes only on a green harness exit (no flag records without running; no `outcome = fail` state; a partial `smoke` tier refused at the writer AND the reader). The claim is bound BY VALUE — a SHA-256 over the spine registries plus the declared `[paths]` source/test trees — so it cannot outlive its tree; and because the record is a declared stage input whose presence makes `stage.fingerprint` fold that same surface, a stale record reds `derive_stage --check` instead of sitting unread. The structural half is `tests/test_approval_level.py::test_the_RELEASE_rung_has_EXACTLY_ONE_PRODUCER_and_it_is_the_EVIDENCE_VERDICT`: exactly one Release return, guarded by the bare parameter, so a guard computed from Status cells is unrepresentable (driven against three mutants). **Honest residue:** this is not a signature — a determined author can compute a valid binding by hand. It defeats a green that outlived its tree, not forgery, and buying the stronger property would need a key and a verifier every adopter must manage |
| Code map is generated, never hand-edited | Harness | `gen_arch_map.py --check` (pre-commit + DevStg-Impl) |
| Trajectory dashboard / OKF bundle stay fresh | Harness | `check.py --run-step trajectory-map / okf` (pre-commit + DevStg-Impl) |
...
| Runtime flows diagrammed and current | Harness | `check_flows.py` (DevStg-Tests/DevStg-Impl) |
| No stub/placeholder at DevStg-Impl | Harness | `check_stubs.py` (DevStg-Impl) |
...
| No provenance citation in a LIVING registry cell — all four spine tiers, reason cells included (owner ruling 2026-08-18; repeals the rationale bullet's review/ruling/design-thread permission) | Harness (advisory) + Reviewer | `trace_text.provenance_advisories` over `SN`/`SR`/`LLR`/`TC`, with `if_note_advisories` and `off_spine_advisories` covering the IF/CMP/EXT reason cells (warn-only, never the exit code); the pre-existing gating rule — a work-item id or process-doc citation in an `SR`/`LLR`/`TC` normative cell under `--strict` — keeps its severity exactly, so nothing that passed before fails now. The reviewed exception list `docs/provenance-allow` is **token-scoped** (`<ROW-ID> <Cell> <token>`, and since OI-41 the reason must open with the `OI-###` the entry defers — see the deferral row above) after the review round measured the older row/cell key suppressing whole cells and hiding 67 unadjudicated tokens over 22 live rows (`tests/test_trace.py::test_the_provenance_allow_file_is_read_token_scoped`), and `::test_this_repos_own_provenance_allow_entries_all_still_bite` fails any entry matching no live token — a suppression nobody is checking. Honest residue: detection is a fixed token vocabulary, so a frame worded outside the shapes reads clean; and whether the durable REASON survived the frame's deletion is the reviewer's (the round's N5 caught two SN rewrites that deleted the argument with the citation — the named failure mode of the rule being applied) |
| Every ANNOUNCED deferral resolves to an open item — the owner's single point of review (OI-41 ruled (e) + always-on, 2026-08-20) | Harness (ERROR) + Harness (advisory) + Reviewer | Three arms, each a FIELD or a COUNT rather than a phrase match, because a matcher's precision is a property of the wording it meets and cannot be bounded before it ships. **ARM 1, the only hard one:** `trace.provenance_allow_findings` — every `docs/provenance-allow` entry names an `OI-###` that resolves in `open-items.toml`, integrity-class on the always-on `--strict-integrity` floor (`tests/test_trace.py::test_an_allow_entry_names_the_open_item_it_defers`, `::test_an_unresolved_allow_entry_reds_the_integrity_floor`). It is hard at birth because a missing field has no false positives. **What it enforces is PRESENT + RESOLVES TO A ROW, and no more** (wording corrected 2026-08-20 after the batch review read it as state-gating): an entry naming a row the owner has already RULED passes ARM 1, deliberately — ruled-but-unexecuted is a legitimate transient, and the count contradiction it can hide is ARM 3's, which names the same entries once rather than twice. So the state made unrepresentable is the *unnamed* deferral, not the stale one. Since 2026-08-20 a second integrity producer rides beside it, `trace.provenance_allow_parse_findings`: a declaring line the grammar cannot read is reported rather than dropped, because a dropped line still reads as a live exception to a human while counting as none — which is how a one-character separator typo made the ARM 3 count report an empty surface. **ARM 2 (warn):** `gen_open_items.fragment_declarations` — a `docs/log.d/` fragment declares the ids its session deferred and each must name a pending row. RULED WITH ITS WEAKNESS ON RECORD: a session that defers silently and says nothing passes clean, and nothing here reaches that (`::test_a_session_that_defers_silently_passes_clean` pins the gap as a property rather than a discovery). **ARM 3 (warn):** `gen_open_items.deferral_findings` — zero pending rows while allow entries still defer is a contradiction between two artifacts, and the finding NAMES the entries (not "something, somewhere, defers", which a reader cannot discharge). Two 2026-08-20 corrections ride here: an ABSENT registry with live allow entries is now itself a finding (it was the one state that produced nothing at all, and it is the founding class at its strongest), and the all-clear line prints the COUNTS it measured — entries parsed, unreadable lines, declarations read — instead of a hardcoded sentence that read identically whether the measurement found nothing or failed to run. ARM 2 gained the SCOPE rule in the same pass: a declaration inside a `### ` section speaks for that section, one in the fragment's top matter speaks for the file, and a multi-section fragment whose sections are mostly undeclared warns once — the day's own grind fragment carried twenty sections and one section-scoped `none`. **Substrate:** the layer is always-shipped process (`process.md` §5) and `check_docs`' S-3 warns on an ABSENT registry, so the rule means the same thing in every repo. Honest residue, stated by the ruling itself: always-on is NOT populated — this row's founding evidence is a registry that was perfectly fresh, rendered "the owner queue is empty", and was telling the truth while ten genuine decisions sat on other surfaces; and no mechanism here catches a decision deferred with no words at all, which is Reviewer-tier |
...
| A requirement is SINGULAR and UNAMBIGUOUS — one `shall`, no non-obligation modal, no unfalsifiable term, no open-ended clause, no actorless passive, no `shall` in an LLR | Harness | `trace_text.form_findings` (gating under `--strict`; process.md §3, "What a checker settles"). The half a checker settles without judgement. **NECESSARY, COMPLETE and FEASIBLE are deliberately unenforced** — they are the consistency review's, and no proxy metric is offered for them, which is the honest gap this row records rather than papers over |
| A requirement CONFORMS to the EARS statement pattern (process.md §3) | Harness (advisory) + Reviewer | `trace_text.ears_advisories` (warn-only, never the exit code): an SR `requirement` opening on a condition dressed in a keyword outside `When`/`While`/`If`/`Where` warns naming the keyword. Measured at 2 of 70 rows before shipping, both re-worded in the landing change, so it guards zero-to-zero. **Presence of an opening only** — whether a row IS event-driven or ubiquitous-with-a-qualified-response is a judgement about the obligation, and a condition buried AFTER the `shall` is not detected: the review's, by design |
...
| A spine row's Evidence-class citation (TC `Evidence`; LLR `Module`/`CodeSymbol`/`TestRefs`) names a FILE that exists | Harness + **Prose (accepted gap: the `::node` selector)** | Was nothing — an invented citation passed every strict gate at rc=0 while its row read `Automated=Yes, Status=Verified` (WI-394's driven evidence). Now: `check_doc_refs.py`'s registry tier (WI-394, owner ruling R2 2026-08-01, option (c)) checks the FILE half of each citation exists — warn-first, gating at the `[step:doc-refs]` `--strict` DevStg-Impl step, the WI-062 precedent. The `::node` test-id selector (and `CodeSymbol`'s symbol names) are ruled **prose by that same ruling**: a renamed-but-still-present test node is deliberately NOT detected — accepted for false-positive control (the original `::` guard's real question), recorded here so it is never implied as covered |
| A new or widened check has its FALSE-POSITIVE RATE measured on the live corpus before any claim about it, and the detector is a worklist rather than a definition of done | **Reviewer (accepted gap) + Test (negative cases only)** | process-options.md "Enforcement audit". Both halves failed in one round, measured 2026-08-18: the provenance detector's review-code arm was **100%** false-positive while its own landing commit reported "0 of 319", and the sweep that same detector graded stopped when the section went quiet, leaving 33 framed cells its token vocabulary could not see. Nothing mechanical can measure a detector's error rate against a corpus with no labelled ground truth, so the rate itself stays **Reviewer** — asking "run it and show me the misses" works, and it is the only thing that does. The mechanizable half is the negative cases, now pinned in `tests/test_trace_rules.py` (`test_a_living_cell_carries_no_citation_frame_but_never_gates_on_one` plus the off-spine and IF siblings) and in `tests/test_trace.py::test_the_provenance_allow_file_is_read_token_scoped`, with the liveness test on the row above failing a suppression that matches nothing. Recorded as a **stated reason** rather than implied as covered |
