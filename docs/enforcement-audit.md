...
| No provenance citation in a LIVING registry cell — all four spine tiers, reason cells included (owner ruling 2026-08-18; repeals the rationale bullet's review/ruling/design-thread permission) | Harness (advisory) + Reviewer | `trace_text.provenance_advisories` over `SN`/`SR`/`LLR`/`TC`, with `if_note_advisories` and `off_spine_advisories` covering the IF/CMP/EXT reason cells (warn-only, never the exit code); the pre-existing gating rule — a work-item id or process-doc citation in an `SR`/`LLR`/`TC` normative cell under `--strict` — keeps its severity exactly, so nothing that passed before fails now. The reviewed exception list `docs/provenance-allow` is **token-scoped** (`<ROW-ID> <Cell> <token>`) after the review round measured the older row/cell key suppressing whole cells and hiding 67 unadjudicated tokens over 22 live rows (`tests/test_trace.py::test_the_provenance_allow_file_is_read_token_scoped`), and `::test_this_repos_own_provenance_allow_entries_all_still_bite` fails any entry matching no live token — a suppression nobody is checking. Honest residue: detection is a fixed token vocabulary, so a frame worded outside the shapes reads clean; and whether the durable REASON survived the frame's deletion is the reviewer's (the round's N5 caught two SN rewrites that deleted the argument with the citation — the named failure mode of the rule being applied) |
