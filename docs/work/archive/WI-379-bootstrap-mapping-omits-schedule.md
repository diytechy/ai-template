...
Proven end to end rather than argued: the same fresh-scaffold probe that raised `ModuleNotFoundError: No module named 'schedule'` from integrate.py's claim refusal ladder now answers `integrate: claimed WI-001 on probe`. An AST audit of every MAPPED script's sibling imports established that schedule was the ONLY omission - 43 mapped before the fix and 44 after, against 46 kit scripts on disk, the two remaining unmapped being bootstrap itself and the deliberately kit-only gen_skills_index (44 + 2 = 46) -, so this is a complete fix rather than the first of a series.
