...
**CANCELLED 2026-08-08 — superseded by the mechanized-loop outcome contract (P6).**

This row asked for a dedup token for a return event. The program removes the
problem instead of tokenizing it: every attempted branch writes **one immutable
outcome event** carrying its own event id (plan §7), so a disposition is keyed to
an identity that exists rather than to one derived from an observer.

Both directions this row's two REVIEW-A rounds rejected are moot: there is no
derived token to collide, and `## Handback` — the section whose digest was the
second candidate — is retired outright (decisions D-4). The reviewer's third,
never-driven direction (state-based dedup against an open disposition row) is
also moot for the same reason, so it is recorded as untested rather than as a
missed opportunity.

Kept as audit history per the plan §13 rule: a cancelled row is evidence.
