...
`docs/reviews/wi-586-adjudicate-llr-207-llr-208/005-ADJUDICATE-3c7764c.md`,
governing line `OUTCOME: RETURN rows=4` over `LLR-207`, `LLR-208`, `TC-205`,
`TC-206` — the latest adjudication, which returned all four and re-drove every
...
1. `LLR-207.detail`, the `governing_identity` clause. It reads "composes
   ...
   Driven at refresh commit `c16246e0`: passing that refresh's own subject
   branch name `wi-585-adjudicate-llr-045-llr-140`,
...
VERDICT THIS CONTINUES:
`docs/reviews/wi-586-adjudicate-llr-207-llr-208/005-ADJUDICATE-3c7764c.md`,
governing line `OUTCOME: RETURN rows=4` — the latest adjudication. The FIRST
adjudication found `LLR-208` and `TC-206` ready on their text; the two
independent rereads since found one downward coverage gap that prevents either
row from being blessed, and the latest re-drove it directly (deleting the
`verdict-rollup` row from `trunk_step.REGEN_STEPS` left all four of `TC-206`'s
cited evidence nodes green, and the whole of `tests/test_trunk_step.py` green
at 16 passed).
