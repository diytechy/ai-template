...
[resolved design notes](parallel-dispatch-design-notes.2026-07-20.md) (non-normative — where
...
intersects the publish diff — **[WI-230](WI-230.2026-07-20.md)** amends the earlier
...
(**[WI-230](WI-230.2026-07-20.md)**). A manual update to
...
| Train claims a WI without its reservation ref | Quarantine that WI/train mismatch; do not recreate authority from cache alone. A "claim" is derived only from the train's NOVEL commits — those in `base..tip` and NOT reachable from `refs/heads/llm/integration`. A commit already reachable from the integration ref is integrated history, so an owner merging the development branch into a reserved train (a content-only sync to preempt conflicts) is not claiming the imported WIs; a genuine foreign claim in a novel commit still quarantines |
