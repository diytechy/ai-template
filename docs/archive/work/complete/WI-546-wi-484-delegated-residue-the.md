...
delegated unattended run (decision 31 of `docs/decisions-for-review-2026-08-31.md`):
the lane's four build sessions committed the work but never wrote the `WI:`
trailer the loop keys on, so no review round was scheduled and the row was
re-dispatched until the supervisor stopped it.
