...
|---|---|---|---|
| `docs/requirements/interfaces.toml` | `IF-###` | `Owner` (a path or `external:` party), `Requestors` / `Consumers` (exactly one), `interface_from_external` / `interface_to_external` | An id-shaped or multi-endpoint `Owner` joins the `--strict` failure set (OI-67), and so does a directional tie-back naming a crossing `external.toml` does not declare. A row whose owner AND far side are all `external:` parties joins it too — it has no in-tree endpoint, so no module whose header can hold our reading of the surface, and a crossing between two external parties is not a seam of this system. **Warn-only**: a row naming both far sides or neither; a module-shaped owner that no LLR `Module` names and whose header declares no `Implements:` (the seam reaches no requirement); an endpoint that resolves to nothing in the tree (WI-455: the cell is present only where `Owner`→LLR→`Module` cannot derive it). Citable from a TC's `Verifies`. The five RETIRED cells (`Contract`, `Provider`, `Req-Refs`, `Signal`, `SignalNote`) are found per REGISTRY by KEY PRESENCE through `spine_carrier.columns` — a TOML row that sets one, even to `""`, or a legacy CSV header that still declares the column — one `--strict` finding per key, naming the rows or the header column (OI-67 slice 6). The `Data` cell carries FIVE warn-first rules: the four FORM rules (no work-item id, no `D-<n>` citation, no rationale connective, a 160-character ceiling) plus a CONTENT rule — a `SCHED_*` / `Foo.bar` / `CONSTANT_NAME` token must resolve in the declared source surface and a named path must exist, vacuous where there is no surface (`[arch-map] mode = files`) and silent on a path `docs/declared-absences` declares. **The definition gate** is `check_trajectory.contract_body_findings` — WARN plain, ERROR under `--strict` (which `check.py` passes at or above `DevStg-Impl`; a direct run passes it any time), sharing `[checks] interfaces_check` — four shapes of ONE rule: a row its owner declares on its `Contracts:` marker but states no `Contract IF-###:` body for; a source whose header the contract grammar refuses (its declared rows count as unstated, the scan continues); an `external:`-owned row no far-side kit module states; a stray declaration on a source that is not the owner. An owner that declares NOTHING is the owner-exact reverse check's WARN, not the gate's. The optional `VerifiedBy` cell (a `TC-###` or an `LLR-###`; empty = "verified in its own right") is checked for RESOLUTION only, warn-first. |
