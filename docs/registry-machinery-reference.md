...
|---|---|---|---|
| `docs/requirements/interfaces.toml` | `IF-###` | `Owner` (a path or `external:` party), `Requestors` / `Consumers` (exactly one), `interface_from_external` / `interface_to_external` | An id-shaped or multi-endpoint `Owner` joins the `--strict` failure set (OI-67), and so does a directional tie-back naming a crossing `external.toml` does not declare. **Warn-only**: a row naming both far sides or neither; a module-shaped owner that no LLR `Module` names and whose header declares no `Implements:` (the seam reaches no requirement); an endpoint that resolves to nothing in the tree (WI-455: the cell is present only where `Owner`→LLR→`Module` cannot derive it). Citable from a TC's `Verifies`. The `Contract` cell carries FIVE warn-first rules: the four FORM rules (no work-item id, no `D-<n>` citation, no rationale connective, a 500-character ceiling) plus a CONTENT rule — a `SCHED_*` / `Foo.bar` / `CONSTANT_NAME` token must resolve in the declared source surface and a named path must exist, vacuous where there is no surface (`[arch-map] mode = files`) and silent on a path `docs/declared-absences` declares. The optional `VerifiedBy` cell (a `TC-###` or an `LLR-###`; empty = "verified in its own right") is checked for RESOLUTION only, warn-first. |
