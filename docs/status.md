...
- **Next action:** the queued frontier (scheduler-derived order):
  **WI-223** — decide the dual-plan regen fail-closed semantics (WI-220
  review finding 2); **WI-224** — quoted-path salvage hardening (finding 3,
  now unblocked); **WI-219** — the 390 px dashboard clipping/overflow repair
  + render-matrix rerun. The deferred backlog
