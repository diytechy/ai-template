...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-14** — One ruling in two parts - (A) which component boundaries exist, (B) what an interface row must say; recommendation is RULE A THEN B IN ONE SITTING, because A decides which rows exist and B what they must say
  - **OI-16** — the owner RULED the doctrine 2026-08-12 - bootstrap keeps its dependencies contained, shared code moves into one common file or themed library files, smallest total code wins - and only ONE narrow thing is still owed: step 2 (-import modules from bootstrap.py-) cannot be built as stated, because bootstrap.py is not shipped into a scaffold, so the recommendation is to INVERT it and let bootstrap import the common module instead
  ...
  - **OI-21** — confirm that -retire gate semantics for stage semantics- meant retiring gate-as-STATE while G1/G2/G3 keep their names as human-certified boundaries; recommendation is CONFIRM, because that reading is already shipped and the alternative is a second, larger rework
