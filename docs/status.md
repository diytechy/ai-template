...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  **stage unification program**, whose six ruled slices are all BUILT per
  ...
  in the frontier below (the evidence carrier, the cell repairs + cross
  check, the ratify-brief split, and the four open lanes). The spine is
  ...
  there is the orphan debt — seven undecomposed SRs plus one TC-less LLR,
  now fully assigned (owner-directed 2026-08-22): five ride fold-in notes
  on the queued rows that own their subjects, the two strays have their
  own decomposition row, so the grind pays the debt as a side effect and
  the ladder climbs as the mints approve. **ALL owner
...
- **Spine:** **SN=27 SR=74 LLR=171 TC=167** (2 drafts) · 129 seams · 4 components.
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-510** `P2` — Decompose the two stray orphan SRs no queued row owns: SR-160 (front-door launchers) and…
