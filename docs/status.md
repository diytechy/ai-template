...
- **Next action:** the queued frontier: **WI-234** — project pending owner
  actions (attestation blocks, conflict asks, quarantine reasons) into a
  generated open-items section (owner directive 2026-07-19; the OI-14 filing
  gap, mechanized); **WI-235** — declare the integrator's generated-artifact
  set in `stack.ini` instead of the hardcoded `GENERATED_ARTIFACTS` tuple;
  **WI-236** — weighted reviewer draws via `agents-enabled` annotations
  (owner-selected shape; the OPENCODE Kimi-K3/Grok-4.5 rows are filed
  definition-only in [agents.csv](agents.csv) pending CLI install + the
  stdin-contract verification recorded there); **WI-229** — the M-05 spine
  migration (attestation-blocked at stage 2; the
  brief is **OI-14** in [open-items.md](open-items.md) — a dispatcher launch
  re-quarantines it until ruled). The 2026-07-18 field-finding batch
  (WI-230..233) is landed — record in [log.md](log.md). The deferred backlog
