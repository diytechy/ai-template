...
- **Next action:** **WI-253** (queued, medium, SR-052;SR-053) — dashboard
  edge routing: wires still cross / pass through node boxes; acceptance =
  rubric anchor **T8** ([dashboard-usability](rubrics/dashboard-usability.md)),
  which holds the render critique at CHANGES-REQUESTED
  ([078-CRITIQUE](reviews/078-CRITIQUE.md), findings=1) until the render
  passes — building it re-fires the WI-243 staleness gate, so build + fresh
  critique together (spec: [specs/WI-253.md](specs/WI-253.md)).
