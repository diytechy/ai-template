...
- **Next action:** **WI-229 landed** 2026-07-19 (the M-05 spine migration:
  ...
  forced the manual path, **WI-240 landed** 2026-07-19 (park/quarantine details
  now carry the failing step via `agent_common._failure_tail`, not the first
  200 chars) and **WI-238 landed** 2026-07-19 (a blocked disposition now adopts
  an absent BlockRef column in-transaction — the WI-229 registry-extension
  precedent — or fails loudly naming it, ending the parked-error loop). The
  queued frontier is the remaining one: **WI-239** — a later completion trailer
  supersedes an earlier `Blocked-WI` so a cured blocker doesn't discard finished
  work.
