...
- **Stage:** **DevStg-Boundary** (stage 1 of 8, system boundary interfaces in work) · next bar: **DevStg-Reqs** (per-phase `1=DevStg-Below;3=DevStg-Tests;4=DevStg-Below;5=DevStg-Below`, derived current **phase=5**) — a repo is IN a stage and CLEARS a bar; the harness at that bar is the bar. [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) derives both, cached to [`docs/gate`](gate).
