...
- **The Phase 3 amendment window is CLOSED and the derived gate is G3**
  (sitting 2026-07-29, same-day — the close-windows-fast rule observed a
  third time): the 9 rows in
  [ratify/2026-07-29b-phase3.md](ratify/2026-07-29b-phase3.md) are
  owner-attested — SR-006/LLR-006 `Modified`→`Verified` (the existing
  evidence plus `tests/test_check_lane.py` verifies the amended lane
  clause) and SR-130/SR-131 + LLR-137/138/139 + TC-130/131
  `Draft`→`Verified` (TC evidence automated and green). `drafts=0
  modified=0`, so G3 is derived, not declared; the ruling is in
  [log.md](log.md)'s Decisions. **The spine being attested and the harness
  being green are different claims** — the second one is below.
