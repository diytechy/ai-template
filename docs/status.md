...
- **Spine:** **SN=27 SR=59 LLR=155 TC=150** (48 drafts) · 122 seams · 4 components.
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-30** — Three calls the migration cannot make for itself, each with a recommendation on record in docs/plans/2026-08-15-d9-migration-plan.md - rule them and the rest of the program is mechanical up to the signing act
