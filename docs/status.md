...
- **Queued (owner intake 2026-07-14** — triage + answers:
  ...
  **WI-147** (graceful pause) · **WI-148** (weekday blackout window) ·
  **WI-149** (lowest-gate-first advisory) · **WI-150** (planner-assigned
  BuildTier) · **WI-151** (throughline pointer). Their sitting-gate predecessor
  is satisfied (the 2026-07-14 sitting closed), so these are the loop's
  **actionable non-dependent backlog** — the loop works them in id order (the
  ratification-package WI shipped 2026-07-14, see [log.md](log.md)) unless the
  owner reorders `docs/next-wi` at the pending g2-close sitting._
...
- **Next action:** **WI-147** ([next-wi](next-wi)) — run-state **RUNNING**. The
  loop works the actionable owner-intake backlog (WI-147…151, id order) while
