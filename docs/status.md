...
- **THE INTERFACE ROW IS BEING RESHAPED — `OI-67` RULED (a), 2026-08-29.**
  ...
  **RESUME HERE:** (1) run the cross-family adversarial round the slice-6
  close owes — the slice-4 prompt re-targeted at the slice-6 fragment
  ([reviews/2026-08-29-oi67-slice4/PROMPT-ADVERSARIAL.md](reviews/2026-08-29-oi67-slice4/PROMPT-ADVERSARIAL.md)),
  the five deferred slice-4 findings riding with it (that round's README has
  the dispositions) — reproduce before acting, fold real findings as a
  correction commit; (2) the next worklist is the arms the split surfaced,
  listed in the slice-4 and slice-6 fragments; (3) the owner reads the two
  decision files (below) and rules on decision 4.1 (the harness-argv class)
  and 6.2 (the undeclared-owner case). The ruling record is
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-534** `P2` — The arms the split surfaced: nine rows minted and stated
