...
- **THE INTERFACE `contract` IS MEASURED, AND THE RELOCATION QUESTION IS ON THE
  ...
  [log.d/2026-08-25-wi516-if-contract-measure.md](log.d/2026-08-25-wi516-if-contract-measure.md);
  the durable per-row verdicts, one line per IF id, at
  [plans/2026-08-25-if-contract-verdicts.md](plans/2026-08-25-if-contract-verdicts.md)).
  All 108 non-CLI cells read per row, **none edited**. **The CLI family's
  87.7% does not generalize**: restatement measures **75.9%** on the 19 non-CLI
  `Provides` rows and **60.8%** on the 89 `Consumes` rows (64.3% combined), and
  the share of rows carrying an irreducible remainder rises from 40.7% to
  **65.7%**. What `OI-63` therefore asks wider than the ruling's one sentence:
  **15.3% of characters belong in NEITHER home** — bookkeeping and argument
  whose home is the once-used `rationale` field — and a **consumer-side
  obligation** class has no provider-side header to move to. The next act is
  the owner's ruling, not another pass. `OI-61`'s (c) stays deferred and was
  NOT re-raised, though the pass banked a second rot exhibit (`IF-117`,
  unreachable by any token grammar) beside `IF-055`.
...
- **The `wi508-architectural-remap` lane is OPEN with slices 1–4 landed**
  ...
  **ten keep-with-recorded-reason, two keep-with-the-reason-absent, zero
  ...
  rationale made it smaller: `WI-519` is filed** — carry the allow-file
  parse-honesty arm to the three declared-exception readers that drop a malformed
  line silently, each keeping its own grammar. Banked, not built: the duplication
  census reads 0/0/0 because it hashes function BODIES, so structural repetition
  is invisible to it by construction.
  **ON THE OWNER'S DESK: `OI-64`** — the zero-SR module both blind teams derived
  is a REQUIREMENT gap, not module work, and no module work was filed against it.
  NEXT in this lane: the dispersion families whose rationale has NOT been read
  (naming them without it is what the standing directive refuses), routing the 48
  fusion pairs to the size-ratchet debt rather than re-deriving them, and the two
  inherited items — the module-size ratchet's live debt owner, and M-06's four
  unsplit test monoliths.
...
- **Spine:** **SN=27 SR=75 LLR=186 TC=183** (4 drafts) · 135 seams · 4 components.
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  ...
  - **OI-64** — WI-508's blind architectural remap ran on two independent axes and both returned a module owning ZERO requirements, for the same subject: the shape of a finding, its severity class, strict-mode escalation, vacuity when an optional input is absent, declared carve-out markers, and how findings compose into an exit code. Eleven-to-thirteen SR rows each state a fragment of it as a secondary clause; no row takes it as its subject, so a new checker can violate it without failing anything.
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-519** `P2` — Carry the allow-file parse-honesty arm to the three declared exception readers that drop…
