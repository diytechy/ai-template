...
- **AN AMENDMENT WINDOW IS OPEN, and it is the biggest one this repo has
  ...
  **What the sitting owes.** Ratify or amend SN-028..032 and their
  decomposition (SR-137..146, LLR-155..164, TC-150..157); combine it with the
  21-row re-attest already owed from 2026-08-07; then `intake.py attest` the
  accepted set so the ledger anchors it (SN-029's whole point is that
  ratification stops being re-derived from git history). Until then the loop's
  ADJUDICATE routing, the ordinal, the attestation ledger and the outcome model
  are all live code answering unratified requirements — which is a legitimate
  state to be in, and a bad state to be in silently.
