...
- **RESUME HERE (2026-08-10) — the carrier migration, D-5.** The owner ruled
  ...
  **Steps 1 and 2 are DONE and step 3 — THE CUTOVER — HAS BEEN RUN BUT NOT
  LANDED** (2026-08-10). Every spine reader now goes through
  `scripts/spine_carrier.py`, the one home for the tier tables, the key→column
  vocabulary and both readers — an **owner ruling amending F5**, repo-lock
  **D-6**.

  **START THE NEXT SESSION BY APPLYING THE CUTOVER, NOT BY REBUILDING IT:**

      git apply -3 docs/plans/2026-08-10-carrier-cutover.patch
      python -m pytest -q -n auto     # expect ~76 red; drive them to zero

  The **`-3` is required, not optional** — three of the patch's files
  (`interfaces.csv`, `agent_loop.py`, `intake.py`, `traj_parse.py`) already
  carry the IF-109…IF-111 seams, which were landed separately once the cutover
  proved they were owed; a plain `git apply` refuses on those and a three-way
  merges them. `git stash apply stash@{0}` does the same thing and is
  equivalent, but the stash is **machine-local** and the patch is committed, so
  the patch is the portable route. The patch deliberately carries **only source,
  registries and tests** — every generated artifact
  (`PROJECT_STATE.html`, `docs/architecture.md`, `docs/gate`, `docs/okf/`,
  `open-items.html`) is regenerated by its own generator and would only
  conflict.

  It converts all four tiers and deletes the `.csv`/`.md` sources together. It
  was proven to work on the real registries — `trace --strict` reads TOML at
  `orphans=0 integrity=0`, `check_docs` at 0 broken links, the gating harness
  PASSES — and, most important, the **carrier-aware baseline read HELD across
  it**: the 25-row re-attest brief regenerated with zero "no baseline" cards,
  which is the one hazard D-5 says must not be forgotten. It is unlanded only
  because the suite ended at ~76 failures and a red trunk is worse than an
  unlanded migration.

  **Read repo-lock §2 D-5 "Step 3" before touching it** — it records the three
  readers the cutover CAUGHT that no amount of wiring against the old carrier
  could have surfaced (two of them fail-open, one reporting
  `SN=0 SR=0 LLR=0 TC=0 orphans=0` as a pass). The lesson generalizes: **the
  cutover is the DETECTOR, not the last step** — run it early, often, and
  against a throwaway tree.

  **Then steps 4–6:** `intake`'s writer becomes a TOML emitter ·
  `test_dogfood_sync`'s ordered-superset header rule, which has no meaning over
  TOML keys · the `registries/*.template.*` files and an ADOPTING migration
...
- **Spine:** **SN=29 SR=146 LLR=148 TC=145** (35 drafts) · 105 seams · 5 components.
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-426** — Tear down the duplication census (repo-lock D-7, owner-ruled 2026-08-10) and execute its…
...
- **Measure on a tree whose line endings match the index, or the measurement
  ...
  — D-7/WI-426 — but the EOL hazard it exposed is not, and the other two
...
- **Standing rules from the 2026-07-25 close:** **never revert a real fix, or
  ...
  this; it was torn down 2026-08-11, D-7/WI-426, and the rule is the half that
  survives.)
