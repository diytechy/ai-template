...
- **Next action:** the queued frontier is **WI-229** alone — the M-05 spine
  migration (attestation-blocked at stage 2; the brief is **OI-14** in
  [open-items.md](open-items.md) — a dispatcher launch re-quarantines it until
  ruled). WI-236 (weighted reviewer draws via `agents-enabled` annotations)
  **landed 2026-07-19** — the per-phase draw-weight grammar + deterministic
  weighted rotation; the OPENCODE Kimi-K3/Grok-4.5 rows stay filed
  definition-only in [agents.csv](agents.csv) pending CLI install + the
  stdin-contract verification recorded there. The 2026-07-18/19 field-finding batch
