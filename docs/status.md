...
- **Stage:** **DevStg-Boundary** (stage 1 of 8, system boundary interfaces in work) · next bar: **DevBar-Reqs** (per-phase `1=DevBar-Reqs;2=DevBar-Tests;3=DevBar-Tests;4=DevBar-Below;5=DevBar-Below`, derived current **phase=5**) — a repo is IN a stage and CLEARS a bar; the harness at that bar is the bar. [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) derives both, cached to [`docs/gate`](gate).
- **Spine:** **SN=27 SR=137 LLR=152 TC=147** (42 drafts) · 115 seams · 4 components.
