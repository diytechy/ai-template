...
- **Next action:** the queued frontier is **WI-229** alone — the M-05 spine
  migration, now **stage-3 dispatchable**: the owner ruled **ATTEST** on the
  frozen stage-1 plan 2026-07-19 (OI-14 ruled and closed — log Decisions; the
  record lives on the plan page itself, train commit `6905138`, and in
  [specs/WI-229.md](specs/WI-229.md)). The next dispatcher launch resumes the
  train into stage 3 — expect the ratified, honest **G3→G2 gate regression**
  mid-migration (pre-accepted; end state SR=108 LLR=96 TC=99 back at G3).
  The OPENCODE Kimi-K3/Grok-4.5 rows stay filed definition-only in
  [agents.csv](agents.csv) pending CLI install + the stdin-contract
  verification recorded there. The 2026-07-18/19 field-finding batch
  (WI-230..236) is landed — see [log.md](log.md). The deferred backlog
  remains as listed above; owner items OI-4 (license/public-private intent)
  and OI-7 (review cadence) stay open under `gate-policy: autonomous`
  (OI-3 ruled closed 2026-07-18, OI-14 ruled closed 2026-07-19 — log
  Decisions).
