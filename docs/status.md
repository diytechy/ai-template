...
- **Perceptual debt gates the two render trains.** The critique judges the whole
  ...
  never touched:
  - **WI-273** (SR-052) — its last failing anchor (A4, dark-theme hub contrast)
    is now **fixed on this branch**, so nothing in the rubric is outstanding.
    What remains is procedural: its registry `Status` is still `blocked`, and
    the train needs a fresh accessibility critique against the **composed**
    render before it can integrate.
