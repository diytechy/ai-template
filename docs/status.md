...
- **The concurrency-v2 design — [concurrency-v2.md](concurrency-v2.md) —
  ...
  **disposable commit**, because `log.md` is append-compiled and a stacked
  ...
  (`merged | cancelled | partial`, no fourth option, so no branch hangs — SN-031 replaced `handback` with the TERMINAL `partial/` plus an immutable per-close report); the
...
- **Spine:** **SN=29 SR=146 LLR=147 TC=144** (30 drafts) · 96 seams · 5 components.
- **Open items** _(pending rows of [requirements/open-items.csv](requirements/open-items.csv); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-12** — rule whether the four requirement tiers share ONE machine-parseable carrier - today SN is markdown prose and SR/LLR/TC are CSV, a split with no recorded rationale anywhere; recommendation is (b) one TOML carrier as the destination, sequenced as its own program after the repo is locked
...
- **Open-items policy** _(`OI-N` ids are stable and never renumbered;
  ratification history lives in [log.md](log.md) Decisions; the ratification level in
  [process.toml](process.toml) is **`autonomous`** — owner directive
  2026-07-15 — so the loop does **not** pause on the open items projected
  above; each item's depth is in
