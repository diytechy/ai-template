...
- **Supervised redesign execution is underway:** follow the scoped
  [execution record](ai-template-redesign-2026-09-05-codex/EXECUTION-RECORD.md)
  for current-runner repairs, verification and remaining control prerequisites.
  The unattended frontier stays paused during this work.
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  ...
  - **OI-85** — Two Approved needs were amended in place with no re-attestation act recorded anywhere a check reads: SN-007's acceptance (the declared bar replaces a green whole suite before every change) and SN-026's tags (legal and personal-data added, so the LEGAL and DATA-PROTECTION lenses now reach the need). The need tier has no snapshot-drift detector - SNAPSHOT_TIERS covers SR/LLR/TC only and the re-attestation brief renders no SN section - so this row is the only surface that names the debt. The same sitting owes a ruling on SN-024: its acceptance requires a family-heterogeneous critique session unconditionally, while Approved SR-154 scopes family diversity to unattended integration and Drafted SR-184 carries Critique acceptance for the attended path too. Does the heterogeneity clause bind attended Critique acceptance, or is it an unattended-only obligation the need should qualify?
