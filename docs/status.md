...
- **Open items** _(projected from [open-items.md](open-items.md) — each item's blast radius, options, and recommendation live there):_
  - **OI-3** — push — the branch is already remote-tracked, so the unpushed commits are pure durability risk (the main integration is a separate sitting).
...
- **Next action:** the build frontier is **WI-204** (spine-only traincar —
  amend SR-058 rule 1 so mid-run spine-serial WIs pack into ONE serial
  spine-only train, owner-directed 2026-07-17; strong tier, spine-touching —
  [specs/WI-204.md](specs/WI-204.md)). Ordering intent: it lands **before** the
  dispatcher-migration set (§14 SafetyClass/soft-edge audit + parallel-default
  flip + legacy-path retirement — discussed, not yet filed; when filed, those
  name WI-204 as a hard predecessor so repos migrate onto the amended rule).
  Queued behind it (owner-directed, same sitting): **WI-205** (backlog-staleness
  warn — [specs/WI-205.md](specs/WI-205.md)) and **WI-206** (whole-registry
  contradiction audit — [specs/WI-206.md](specs/WI-206.md)). Owner items
