...
- **RESUME HERE — THE FRONTIER IS PAUSED (tracked `docs/work/pause`); THE
  2026-08-31 RULINGS ARE LANDED AND THEIR ROWS ARE QUEUED.** `OI-70`..`OI-73`
  are ruled (the log's 2026-08-31 rulings entries); WI-552..WI-556 are
  filed — WI-552 re-scoped by `OI-73` (typed OI edges in `needs`, the
  mandatory successor, edges replaced at the mint) — WI-543 is re-scoped and
  queued (SR-163's owner, frontier head), WI-541 is re-pointed to WI-551,
  WI-545 sequenced behind WI-552/WI-553.
  Still open for the owner: `OI-74`/`OI-75` (the delegated-decision record
  surface and its routing doctrine — evidence in
  [knowledge/decision-routing.md](knowledge/decision-routing.md)), and the
  verdict-carrier design — plan of record
...
- **Supervisor prompt** (owner-authorized unpause — paste into a fresh session):
  ...
  State at handoff: OI-70/71/72/73 are RULED (log.md, the 2026-08-31 rulings
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-74** — Rule the mechanism for decide-and-record-for-affirmation — whether it is a per-sitting run report or a per-decision registry, where the owner's affirmation lands, what an overturn mints, and which altitudes it covers. A RECORD surface beside OI-70/OI-73's exits, never a third exit: work still routes only through a queued successor or a minted OI
  - **OI-75** — Rule how a session routes a decision among decide-silently / record-for-affirmation / escalate: what signal drives the routing (action properties and counts, never model self-confidence as a primary), how the recording obligation is made structural rather than instructed, and whether the owner's appetite lives in a process.toml dial or in the launch prompt
