...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  - **WI-446** `P2` — OI-19 execution (ruled 2026-08-13): ship the six-hat starting roster at docs/requirements…
  - **WI-449** `P2` — OI-20 execution (ratified as built, 2026-08-13): write the ruled CodeSymbol grammar into…
  - **WI-444** `P1` — The applied re-attest batch (owner's apply-now direction, 2026-08-13)
  - **WI-390** — PROGRAM CLOSE for concurrency-v2 (docs/concurrency-v2.md §A9 deletion ledger). NOT a swee…
  - **WI-440** `P1` — OI-14's two do-not-wait fixes, both partition-neutral: (1) correct the DIRECTION of check…
  - **WI-439** `P1` — OI-27's three do-not-wait defects, taken ahead of the re-sync pack: (1) the tarball adopt…
  - **WI-436** `P1` — OI-26 close residue: correct the repo-lock section 5 loose end that carried the refuted p…
  - **WI-437** `P1` — OI-25 execution: rename the live runtime label that collides with the retired gate_policy…
  - **WI-438** `P1` — OI-24 execution, the build half: declare the moment-to-tier table ONCE in a machine-reada…
