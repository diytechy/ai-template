...
- **RESUME HERE — THE FRONTIER IS PAUSED (tracked `docs/work/pause`); THE
  ...
  Still open for the owner: `OI-73` (typed OI dependencies in `needs` + the
  mandatory successor at every partial close — refines `OI-70`, re-scopes
  WI-552 **before it is worked**), and the verdict-carrier design — plan of record
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-73** — Rule whether a WI's needs list may carry OI ids as hard, gating edges, and whether every partial close must queue at least one successor whose inbound needs edges are REPLACED at the mint — tightening OI-70's exits from successor-and/or-OI to successor always, with the OI (when minted) a typed dependency of it
