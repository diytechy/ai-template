...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  **NINETEEN DRAFTED SPINE ROWS ARE LIVE AND THE OWNER SURFACE DOES NOT
  ...
  condition, `verified_by` sanctioned warn-first), and its execution row has
  since SHIPPED — what it left open is the bullet below. The grind over the
...
- **The interface `contract` cell's next question is (b), and it now has a
  number to answer it with.** The staged (a) pass thinned the 27 CLI-restatement
  ...
  prose was restatement**, and the open decision is whether to run the same
  reviewed pass across the **other 108 rows**, whose median cell is longer
  (374 chars, 34 over the ceiling). Two inputs are live and neither is owed by
