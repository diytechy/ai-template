...
- **Next action:** **WI-274** (queued, `scripts`, medium —
  ...
  `.venv` base 3.8 was then uninstalled, so recreating it is the next session's
  first act) + **B** single-home coordinator dials — `agent_loop` reads a
  declared config (IF-068 Proposed) so `AGENT_JOBS`/`AGENT_MODEL` stop living
  in all three `agent-resume.*` launchers. Beyond that, the owner
