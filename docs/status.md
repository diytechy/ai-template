...
- **Next action:** grinding the **repo-review-2026-07-21 backlog** in
  ...
  **adversarial review** before cherry-pick. Waves 1–3 are **done and
  ...
  weights, win-stay `next_primary` routing, status-map freshness-gate purity,
  blackout pause banner, Python floor 3.8→3.11) — see [log.md](log.md);
  integrated smoke 1063 passed. **Next (the last item):** **WI-267** (add a
  terminal `retired` Status to the work-item vocabulary + audit every Status
  consumer, solo). The render surface changed in waves 1–2 (edge-router
  lane-routing + cue/focus polish) — a consolidated perceptual critique is owed
  before the next green G3, scheduled at task close.
