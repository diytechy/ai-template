...
Contract IF-163: the forward-only blackboard, read as data by the kit's checks
    and by a resuming session. Markdown with `##` sections: `## Current State`
    is the section a stopping coordinator excerpts into its exit banner and the
    section holding the generated snapshot between its GENERATED STATUS marker
    pair, with the hand-authored intent outside that pair. Only what must happen
...
- **THE INTERFACE ROW IS BEING RESHAPED — `OI-67` RULED (a), 2026-08-29.**
  ...
  **RESUME HERE:** slice 6 remains, in `docs/work/queued/` — arm the gate (a
  declared seam with no body and a retired column become strict findings),
  plus the two things slice 3 could not place: the CSV loaders must skip a
  leading `#` header before `IF-031`'s owner can declare, and the three
  `external:`-owned rows need a rule for where our reading of an external
  surface lives; the split's own residue (the arms it surfaced, in its
  fragment) is the worklist after that. The ruling record is
...
- **Spine:** **SN=27 SR=75 LLR=187 TC=184** (6 drafts) · 154 seams · 4 components.
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-533** `P2` — Arm the gate: a missing contract body is a strict finding, retired columns are schema fin…
