...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-401** — SN coverage becomes a gate rung: an SN with no covering SR caps the derived gate (owner r…
  - **WI-402** — Phase is NUMERIC ONLY, and the next phase is a derived call (owner rulings 2026-08-01, di…
