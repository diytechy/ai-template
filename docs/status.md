...
- **Next action:** this branch (`dualplan-routing-fix`) carries the
  ...
  lookup, and factual coordinator/traceability contract corrections. **WI-219**
  is now the queued frontier: repair the reproducible 390 px dashboard hierarchy
  clipping/overflow affordance and rerun the 36-shot render matrix. The deferred
  backlog remains as listed above; owner items OI-3 (branch integration), OI-4
