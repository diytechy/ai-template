...
- **Next action:** the WI-220 review (log.md, 2026-07-18) filed three
  follow-ups now on the queued frontier (scheduler-derived order):
  **WI-222** — salvage committed round evidence on post-commit disposition
  resets (finding 1, the fix); **WI-223** — decide the dual-plan regen
  fail-closed semantics (finding 2); **WI-224** — quoted-path salvage
  hardening (finding 3, after WI-222); plus **WI-219** — the 390 px dashboard
  clipping/overflow repair + render-matrix rerun. The deferred backlog
