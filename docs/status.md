...
- **Next action:** the build frontier is **WI-208** (M1 — the §14
  dispatcher-migration audit + parallel-default flip:
  [specs/WI-208.md](specs/WI-208.md); ≺ WI-204 done, so it is actionable now),
  then **WI-209** (M2 — dual-plan auto-dispatch + quiet-park auto-page,
  [specs/WI-209.md](specs/WI-209.md)); **WI-210** (M3 — legacy-path
  retirement) waits deferred on its declared bake-period trigger. The
  WI-204…207 sitting evidence is in [log.md](log.md); the branch is pushed
  and in sync as of that close. Owner items OI-3 (now the `main`-integration
  residue) / OI-4 / OI-7 stay open under `gate-policy: autonomous`.
