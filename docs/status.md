...
- **Open perceptual findings (recorded, unfiled):** the fresh non-Anthropic
  critiques [115](reviews/115-CRITIQUE.md) (SR-054, APPROVE f=0),
  [116](reviews/116-CRITIQUE.md) (SR-052, f=1) and
  [117](reviews/117-CRITIQUE.md) (SR-053, f=7) leave **8 findings** with no WI
  yet — the dark-theme hub contrast (2.98:1, under AA), the U5 palette
  collisions, two U3 idiom splits, one U1 type-scale deviation. Filing them (and
  how finely to split them) is an **owner call**. Until that debt is paid, no
  render-surface train can close: the critique judges the whole document and
  APPROVE requires every anchor, so **WI-272** is held by U5 and **WI-273** by
  A4 — neither of which those WIs touch. Detail in [log.md](log.md).
