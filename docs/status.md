...
- **The `wi455-architecture-retirement` lane owns what the sitting does not:**
  the crossing-ownership re-key onto each named owner and B-04's half
  realization. The five untied `external:`-marked IF
  ...
  `interfaces.toml`'s columns were touched. The `external.toml` context view
  LANDED 2026-08-23 (`traj_context.py`, spliced above the derived structure in
  the How tab; `LLR-200` + `TC-196` mint `Drafted`), which is what satisfies the
...
- **The `wi448-common-module` lane is OPEN holding ONE item**, slices 1-4
  ...
  **`OI-48` is RULED (d) (2026-08-21) AND EXECUTED
  ...
  unblocked.
- **The `wi483-core-decomposition` lane is OPEN with the cycle and both bad
  edges GONE** (slices 1–3, 2026-08-20 / 2026-08-22 / 2026-08-23): the
  ...
  review's two DOCUMENTED bad edges are now cut rather than described — the
  `gen_trajectory` facade has **no importers at all**, asserted by a new
  equality ratchet in the same file. Four extractions did it: the lane-close
  terminal-outcome vocabulary and the per-close report's shape into
  `scripts/kitlib/station.py`, the registry-gap census into `scripts/census.py`,
  and the pending-owner-action read model into `scripts/pending.py`, each below
  every module that reads it. **What remains owed on the row is items 1, 3 and
  4 of its spec Context, in that order:** the layering (`integrate` still reaches
  up into `intake` for the post-merge mint, an edge that closes nothing but runs
  the wrong way, so `dispatch` is not yet the sole composer), then the engine
  splits (`trace.analyze`, `check.steps`, `agent_loop.main` and the attribute
  bags), then M-06's test-monolith splits, which ride along with them.
...
- **The `wi484-concern-refs-component-view` lane is OPEN with slice 3 landed**
  (2026-08-23): phases 0, 1, 2's backfill and **3** are done — the component
  view is GENERATED (`components.derived.toml`, freshness-gated by the
  `component-view` step) and `detail_doc` is retired. The row's spec Context
  lists what is still owed. **The next decision is phase 2's writer**, and it is
...
- **Spine:** **SN=27 SR=75 LLR=182 TC=179** (17 drafts) · 135 seams · 4 components.
