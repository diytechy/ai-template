...
- **Resuming in a new session — boot HERE, then the spec.** The working
  ...
  holds the per-phase records (Phases 0–3 done; 0–2 attested 2026-07-29,
  **Phase 3's amendment window is OPEN** — see below) and **Phase 4 is
  next** — the local integrator lane (the one-page serial merge queue) + the
  shrunken coordinator, with two `ordinary` WIs run end-to-end as the
  acceptance proof. Standing trap that
...
- **Spine:** **SN=25 SR=131 LLR=137 TC=127** (7 drafts) · 77 seams · 5 components.
...
- **The Phase 3 amendment window is OPEN** (2026-07-29): SR-006/LLR-006 →
  `Modified` (the work-branch freshness-skip clause) and SR-130/SR-131 +
  LLR-137/138/139 + TC-130/131 → `Draft` (the trunk step and the tracked
  pause), so the derived gate dropped (per-phase `1=G2 … 4=G0`) until the next
  sitting blesses them — the brief is
  [ratify/2026-07-29b-phase3.md](ratify/2026-07-29b-phase3.md), and
  [open-items.html](open-items.html) lists every row's before/after; the
  shorter the window, the less it can hide. The two
  prior sittings (2026-07-27: 140 rows; 2026-07-29: the 2c amendment —
  briefs in [ratify/](ratify/), rulings in [log.md](log.md)'s Decisions) are
  the precedent. **The spine being attested and the harness being green are
...
- **THE UNATTENDED LOOP IS PAUSED — the pause is now the tracked
  [work/pause](work/pause) file** (concurrency-restructure §5.6; Phase 3
  deleted the untracked `docs/pause` marker, and `pause_reason` honors both
  homes so the retired-in-place dispatcher cannot resume on the swap). The
  reason and date live in the file — read them there, and see the generated
  pending surface ([open-items.html](open-items.html)) where an open pause
  renders as an accruing cost. Do **not** launch `agent-resume`: the legacy
  dispatcher is retired-in-place and its replacement — the local integrator —
  is Phase 4's deliverable. Unpausing is a reviewed deletion commit, after
  Phase 4 lands. The SR-054 critique deadlock that originally forced this
  pause is **history**: SR-054 flipped to `Verification=Test` at the 2c
  amendment (RULING-5, attested 2026-07-29), the fail-closed perceptual gate
  is retired, and the full account — the two-WI train deadlock, the rejected
  `Exclusive` mutex, the twice-proven attended route — is in
  [log.md](log.md).
...
- **The owner decision surface is now [open-items.html](open-items.html)**,
  ...
  **OI-4 and OI-8 are ruled** (2026-07-25 — Apache-2.0, and hosted CI on every
  ...
  (keep per-slice review; WI-123 `retired`, spec archived). The owner queue
  now holds **the Phase 3 attestation** (the open window above — 9 rows,
  before/after rendered in [open-items.html](open-items.html)) plus the
  standing item the loop won't make: **merge-to-`main`** for
  `dualplan-routing-fix` (and the push before it, per `push-policy: human`).
  Per the 2026-07-27 sitting's own finding, the shorter the window, the less
  it can hide.
