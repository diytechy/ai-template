...
- **Resuming in a new session — boot HERE, then
  ...
  four phases**). **What's next is the queue**, through `integrate.py claim`
  in the generated Ready-frontier order below (WI-357..360 from Phase 4,
  WI-361..364 filed since, plus the surviving frontier).
...
- **Spine:** **SN=25 SR=132 LLR=123 TC=119** (0 drafts) · 78 seams · 5 components.
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  - **WI-365** `P1` — Split the three double-shall SRs the open window let through: SR-006/SR-130/SR-131 each c…
...
- **No amendment window is open** (all four 2026-07-29 sittings — 2c,
  ...
  and the harness being green are different claims**: the armed G3 bar
  carries three named pre-existing form findings (SR-006/130/131
  double-shall, let through by the open window at Phase 3) — **WI-365**
  (P1) owns the split, and it lands AT a sitting.
