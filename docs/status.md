...
- **The restructure design (this branch's purpose):**
  ...
  advisory critique; Phases 3 and 4 landed the same day — §7 has both
  records, and **the Phase 4 acceptance proof PASSED**: two ordinary WIs
  claimed, built in parallel worktrees, independently reviewed, and merged
  by `integrate.py`'s serial queue through the full composed-tree bar, audit
  clean, zero hand-rescues). Next: **Phase 5** (deletion — the train
  machinery, its tests, and the audit's approved retirements).
- **Resuming in a new session — boot HERE, then the spec.** The working
  ...
  holds the per-phase records (Phases 0–4 done; the Phase 4 amendment window
  is OPEN below) and **Phase 5 is next** — the train machinery, its tests
  and the dead surfaces leave; the audit's approved retirements execute with
  it; the WI-357..WI-360 findings register from the acceptance run is queued
  work, not blockers. Standing habit that
