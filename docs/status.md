...
- **Next action:** this branch (`dualplan-routing-fix`) carries two
  downstream-found **OpenAI-path** fixes — WI-215 (the routing-ON dual-plan
  crash + raw-transcript output) and WI-216 (session prompts delivered via
  stdin, so a large prompt survives the Windows `codex.CMD` 8191-char cmd.exe
  cap). Both surfaced in `C:\Projects\gilbert` and were confirmed latent here.
