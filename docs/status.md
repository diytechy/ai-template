...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-47** — rule the correction MECHANISM for the under-seeded B/REL id watermarks - B-08/REL-004 were allocated and cut (2026-08-13, ruling 13o) before external.toml existed, so the 2026-08-17n seeding probe (git log -S over that file) was structurally blind to them and seeded B = 7/REL = 3; the NEXT MINT in either space is exactly a spent id, hand-raising the marks was tried 2026-08-17w and REFUSED by trace.py's own integrity rule (a mark rises by allocating an id, never by hand - the first-seed exemption was consumed at the blind seed), and the only protection since is an unenforced prose block (the SPENT IDS header of external.toml); recommendation is (a) - make the spent-id record machine-readable so it both refuses the re-mint and justifies the one-time mark correction to B=8/REL=4
