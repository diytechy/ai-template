...
- **RESUME HERE — THE FRONTIER IS PAUSED (tracked `docs/work/pause`); THE
  2026-08-31 RULINGS ARE LANDED AND THEIR ROWS ARE QUEUED.** `OI-70`..`OI-75`
  are ruled (the log's 2026-08-31 rulings entries); WI-552..WI-557 are
  ...
  evidence [knowledge/decision-routing.md](knowledge/decision-routing.md)) —
  ...
  Still open for the owner: `OI-76`, the verdict-carrier design (now a
  registry row, no longer prose-only) — plan of record
...
- **Supervisor prompt** (owner-authorized unpause — paste into a fresh session):
  ...
  State at handoff: OI-70..OI-75 are RULED (log.md, the 2026-08-31 rulings
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-76** — Rule what artifact carries a lane's governing review verdict. The merge gate today requires docs/reviews/WI-<n>-REVIEW-A.md, which nothing in the kit writes (four of seven 2026-08-31 runs stopped on its absence), which paraphrases evidence that already exists, and which anyone with commit access could forge — plan of record docs/plans/2026-08-31-verdict-record-and-queue-blockers.md. Filed on the owner's question (this decision was living only in a plan file and a status.md line — the exact prose-invisible class OI-70 named — while the owner surface read 0 pending)
