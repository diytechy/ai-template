...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  **stage unification program**, whose six ruled slices are all BUILT per
  ...
  there is the orphan debt — **five** undecomposed SRs plus one TC-less LLR
  remaining, all assigned (owner-directed 2026-08-22): three ride fold-in
  notes on the queued rows that own their subjects, the two strays have
  their own decomposition row, so the grind pays the debt as a side effect
  and the ladder climbs as the mints approve. (The CI pair — the hosted bar
...
- **Spine:** **SN=27 SR=74 LLR=174 TC=170** (2 drafts) · 129 seams · 4 components.
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - _(+2 more ready — see the dashboard)_
