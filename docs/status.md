...
- **START HERE for the P0 approval sitting (current as of 2026-08-13):**
  ...
  [open-items.html](open-items.html) is the ruling surface (14 pending briefs,
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-14** — One ruling, three questions - (A) which component boundaries exist, (B) what an interface row must say, and whether G1 must require a declared system boundary first; rule A then B in one sitting
  - **OI-16** — RULED 2026-08-12 - bootstrap contained, shared code into one common or themed library files; ONE thing owed: step 2 is unbuildable because bootstrap.py is not shipped to scaffolds, so INVERT it and let bootstrap import the common module
  - **OI-17** — rule whether SN-034 (one-action dev-setup + agent-resume) and SN-035 (a run menu for this repo) stay needs or demote to requirements; recommendation updated to REFRAME - SN-034 becomes THE launcher need and SN-025's launcher clause moves into it - with keep-SN-035 standing either way
  - **OI-18** — rule whether the ten edge-case SNs retire into the requirements already carrying them; recommendation is DISSOLVE, inside the sitting's already-open amendment window, because eight of the ten already share their SR with a core need
  - **OI-19** — rule the declared expert-perspective roster SN-036 requires - its contents, its home and its injection point; recommendation is a six-hat starting roster in docs/requirements/hats.toml, injected by the decomposition brief builder and edited as owner text
  - **OI-20** — ratify the rule that an LLR's CodeSymbol/Module must bind to a real symbol before the row counts as discharged; recommendation is RATIFY AS BUILT, with the four unfounded rows left honestly unfounded
  - **OI-21** — RULED 2026-08-12: every G* tag converts to stage semantics on a new eight-rung ladder (0 vision/needs .. 7 release-ready) - the tags die purposefully, because gate numbers will always be conflated with stage numbers; open for ruling: the string-vs-int carrier, the historical sweep's attestation carve-out, the sign-off successor shape, the automation-dial keying, and the per-rung predicates
  - **OI-22** — rule whether the prepared exact-replacement text for 29 stakeholder needs and 17 system requirements is adopted; recommendation is ADOPT at the sitting alongside the 25-row re-attest, never piecemeal, so two re-blessing windows collapse into one
  - **OI-23** — rule the spine rows whose RATIFIED text describes a world that no longer exists after the carrier move, the dial re-homing and two singleton errors; recommendation is AMEND ALL AS ONE BATCH inside the sitting's already-open window
  - **OI-24** — rule SN-005's obligation that CI runs the SAME harness a human runs - true and shipped, yet pinned by no test; recommendation is BUILD THE CHEAP HALF (one string search) and narrow the need to what is checkable
  - **OI-25** — rule the gate_policy naming collision - a retired configuration enum and a live runtime label share the name, and the shipped options document still teaches the retired one; recommendation is RENAME the label and sweep the doc as one change
  - **OI-26** — the premise -a red TC cannot mint the WI that fixes it- is REFUTED on measurement: DRAFTED plus the adjudicator's ## Dispositions drafts plus intake's drafts-not-mints arm IS the mint path, built and tested; recommendation is CLOSE, with the corrected flow recorded here
  - **OI-27** — rule the method for re-syncing a repo on an older kit via LLM - no version was ever designated (the anchor is the write-only docs/kit-version SHA stamp), the mechanical paths are skip-all or clobber-all, and 34 migration recipes live as prose in two drifting homes; recommendation revised to (e) an LLM re-sync pack with SHA-anchored entries NOW, promoting to the (c) detector registry on a named trigger
  - **OI-28** — rule how the shipped template set ties back to requirements and how the LLM-in-a-direct-session actor is declared - measured: 2 of ~37 shipped templates appear in any LLR cell, and the session actor exists only as two accidental IF counterparts; recommendation is SEED AT THE BOUNDARY, executed with OI-14's rung-1 work
