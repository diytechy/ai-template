...
- **Deferred backlog** _(first-class `deferred` rows; each carries its reason in
  ...
  WI-187 · WI-271** in [work-items.csv](requirements/work-items.csv)
...
- **Next action:** **WI-275** first (queued, `quality`, quick, **Priority 1** —
  ...
  living in all three `agent-resume.*` launchers. Beyond that, the owner
  must decide whether to merge `dualplan-routing-fix`, rule OI-4/OI-7, or
  explicitly activate one of the deferred rows. The five archive-anchored
  rows **WI-060 / WI-061 / WI-062 / WI-063 / WI-082** also need per-row
  re-specification versus the now-available `retired` disposition.
