...
- **Next action:** the queued frontier: **WI-229** — the M-05 spine
  migration (staged; its stage-2 owner-attestation stop is declared in the
  spec and blocks any spine edit — the parallel run's worker correctly
  quarantines there on every launch until the owner attests); plus the
  2026-07-18 parallel-run field findings, dispatcher hardening in dependency
  order: **WI-230** — publication proceeds under dirty tracked paths disjoint
  from the publish diff (owner-selected); **WI-231** — the integrator
  regenerates generated artifacts on conflict instead of parking + the
  WI-keyed registry union merge; **WI-232** — make `needs-re-review`
  actionable (dispatch the focused re-review or page NEEDS-HUMAN; soft on
  WI-231). The deferred backlog remains as listed above; owner items OI-3
  (branch integration), OI-4 (license/public-private intent), and OI-7 stay
  open under `gate-policy: autonomous`.
