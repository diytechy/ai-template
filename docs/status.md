...
- **Next action:** the review-18 open items are consumed
  (repo-review-2026-07-18.md §6); the C901 ratchet is live at baseline 52.
  The queued frontier (scheduler-derived order): **WI-226** — the dispatcher
  decision/effect decomposition (H-02 slice B, absorbs the L-04 test split;
  each stage re-stamps the ratchet downward); **WI-227** — the run-menu
  trailing-argument contract (M-06); **WI-228** — the live-orphan taxonomy +
  ratchet (M-07); **WI-219** — the 390 px dashboard clipping/overflow repair
  + render-matrix rerun; **WI-229** — the M-05 spine migration (staged; its
  stage-2 owner-attestation stop is declared in the spec and blocks any
  spine edit). The deferred backlog remains as listed above; owner items
  OI-3 (branch integration), OI-4 (license/public-private intent), and OI-7
  stay open under `gate-policy: autonomous`.
