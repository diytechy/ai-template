...
- **Next action:** the **repo-review-2026-07-21 eleven-item backlog is fully
  built, reviewed, and integrated** — each item ran a parallel worktree build →
  an independent opus **adversarial review** → cherry-pick (see [log.md](log.md)
  for per-item deliverables + verdicts; delivered: edge-router round 3, G3-SR
  unification, integrator verdict-gate unanimity, repair-round independence,
  dashboard cue/focus polish, cross-train draw weights, win-stay `next_primary`
  routing, status-map freshness-gate purity, blackout pause banner, the Python
  floor 3.8→3.11 bump, and the terminal `retired` work-item Status). **Owed
  before the next green G3:** a fresh consolidated perceptual **dashboard
  critique** (the fail-closed perceptual-re-fire staleness gate — the render
  surface changed under the edge-router round-3 + cue/focus work). **Open owner
  calls / follow-ups** filed from the reviews: a requirements-change WI to bump
  the Python-floor **requirement spine** (SN-011 / SR-034 / SR-035 + the
  `architecture.md` overview + SR-035's CI-matrix narrative) to match the kit's
  new 3.11 floor; the per-row triage of the five archive-anchored deferred rows
  **WI-060 / WI-061 / WI-062 / WI-063 / WI-082** (re-spec vs the now-available
  `retired` disposition); plus the minor robustness notes recorded in the log.
