...
- **THE TARGET — the SR/LLR/TC RE-TIER CAMPAIGN against the locked depth-0
  ...
  **WHAT REMAINS is the SITTING, and only the sitting.** The campaign's closing
  ...
  (`WI-464`, preconditions WI-461/462/463) executes the one-decision tiering
  model through its six slices, and THEN
...
- **Spine:** **SN=27 SR=60 LLR=155 TC=150** (49 drafts) · 122 seams · 4 components.
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-462** `P3` — The dispatch trio reds on Windows MAX_PATH only under xdist: test_a_needs_human_worker_ha…
  - **WI-463** `P3` — Declare the traj_status->derive_gate seam: check_trajectory reports 'cross-component impo…
  - **WI-461** `P2` — The CRLF-relay conviction is red on the declared floor: tests/test_integrate.py::test_a_w…
