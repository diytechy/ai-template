...
- **Next action:** the build frontier is **WI-207** (registry coherence
  amendments from the first whole-registry audit — the five
  [reviews/108-AUDIT.md](reviews/108-AUDIT.md) findings, headline: SR-040 still
  keys routing on the SR-059-deleted `docs/run-phase`; strong, spine-touching —
  it will ride a spine-only train under the WI-204 rule). WI-204/205/206 all
  landed this sitting (spine-only traincar + backlog-staleness warn +
  contradiction audit; evidence → [log.md](log.md)). Still to file: the
  **dispatcher-migration set** (§14 SafetyClass/soft-edge audit +
  parallel-default flip + legacy-path retirement + dual-plan auto-dispatch),
  naming WI-204 as hard predecessor. Owner items OI-3/OI-4/OI-7 stay open under
