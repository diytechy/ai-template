...
- **START HERE (current as of 2026-08-13, post-charge-through):**
  ...
  perspective-coverage record. **Sitting 1 of 2 EXECUTED 2026-08-13** (the
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-442** `P2` — Mint external.toml and slim interfaces.toml
...
- **THE AMENDMENT WINDOW ROTATED AT SITTING 1 (2026-08-13): `drafts=27
  modified=51`, gate derived DevBar-Reqs, stage DevStg-Boundary.** The SN tier
  is fully ratified (27 core needs, zero draft — the 2026-08-08 program's
  needs included) and its SRs moved `Planned`; what stays open is
  **deliberate** — the `Modified` SR set re-opened by the ruled rationale
  sweep and gate-policy corrections, plus the LLR/TC drafts — and it closes at
  sitting 2, after the SR re-statement/retier against the agreed boundaries.
  Ratified needs with no SR children read `uncovered=8` until that pass
  provides their first coverage (the account is in [log.md](log.md)'s
  2026-08-13d entries). **The spine being attested and the harness being green
  stay different claims:** the full unfiltered suite is green and `check.py`
  runs at the derived gate; the SR tier's attestation completes at sitting 2.
...
- **The owner decision surface is now [open-items.html](open-items.html)**,
  ...
  **20 SRs to `Modified`** and dropping the derived gate **DevBar-Release → DevBar-Tests**
  (`ex-draft=DevBar-Tests` — amendment pressure, not new drafting). **That
  re-attest was consumed by sitting 1 of 2 on 2026-08-13** (with the rest of
  the batch; the split and rulings are in [log.md](log.md#decisions-log)) —
  what remains owed is sitting 2, queued in the START HERE bullet above.
  Alongside it stands the standing item the loop won't make:
