...
- **Queued (ready frontier)** _(activated 2026-07-22 from the repo-review-2026-07-22
  backlog triage; build order):_ **WI-274** (per-module coverage floors for the
  security/process boundaries — highest value: guards the H-1 boundary
  `agent_session.py` 74% and `subagent_gate.py` 40%) → **WI-272** (dashboard
  status fidelity — the state surface mislabels `deferred`/`blocked` as `queued`)
  → **WI-273** (dashboard ARIA tabs; soft-gated behind WI-272) → **WI-276**
...
- **Deferred backlog** _(first-class `deferred` rows; each carries its reason in
  ...
  WI-275 · WI-277 · WI-278** in [work-items.csv](requirements/work-items.csv)
  (WI-097/WI-123/WI-278 are the OI-4/OI-7/OI-8 subjects above; WI-275 is the
...
- **Retired 2026-07-22:** **WI-082** (decompose `bootstrap.py main()`) —
  superseded by WI-275, which now owns it as a concrete first slice.
...
- **Next action:** the ready frontier above is queued; start **WI-274**. The
  owner still owns three calls: rule **OI-4** (LICENSE — the review's H-3 legal
  blocker), rule **OI-8** (branch integration / CI-on-branch), and decide
  merge-to-`main` for `dualplan-routing-fix`. OI-7 (review cadence) stays parked
  by its own recommendation. The remaining archive-anchored rows
