...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  Four of the six queued briefs RULED in one owner message (2026-08-21;
  record: [log.d/2026-08-21-owner-rulings-oi48-52.md](log.d/2026-08-21-owner-rulings-oi48-52.md);
  execution rows in the generated frontier below). Still with the owner:
  **`OI-53`** (the stale Approved `CodeSymbol` batch — the sitting's act)
  and **`OI-51`'s confirmation** (the why-would-tests-run question is
  ANSWERED on the row; the re-tag ruling itself stays open).
...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-497** `P2` — intake._gate_moved reads the static header comment, so tier_signal's strong adjudication…
