...
- **Next action:** this branch (`dualplan-routing-fix`) carries the
  downstream-found OpenAI-path fixes (WI-215/216/217), the
  [review-17c fixes](repo-review-2026-07-17c.md),
  WI-218's behavior-preserving coordinator split, and the confident remediation
  from the [2026-07-18 deep repository review](repo-review-2026-07-18.md):
  immutable/least-privilege GitHub Actions, the batched documentation-staleness
  lookup, and factual coordinator/traceability contract corrections. The queued
  frontier (scheduler-derived order): **WI-220** — dispatcher hardening from the
  2026-07-18 gilbert crosscheck (their kit bugs #5/#6/#7: the diverged-dev
  reconcile must page NEEDS-HUMAN instead of silently publishing over `main`;
  all three registry-editing disposition/integration commits regenerate the
  present okf/trajectory artifacts so a freshness-gated hook can't reject them;
  error paths salvage round evidence before reset — spec
  [WI-220](specs/WI-220.md)); **WI-219** — the 390 px dashboard
  clipping/overflow repair + render-matrix rerun; **WI-221** — raise
  `DEFAULT_ROUND_BUDGET` 10→14 (field-proven; zero fallback headroom bit
  gilbert twice — spec [WI-221](specs/WI-221.md)). The deferred backlog
  remains as listed above; owner items OI-3 (branch integration), OI-4
  (license/public-private intent), and OI-7 stay open under
