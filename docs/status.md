...
- **Resuming in a new session — boot HERE, then
  ...
  so the frontier holds exactly one thing: the **H-2 decomposition
  program**, promoted out of `deferred` on 2026-07-31 and named in the
  generated Ready frontier below (what a claimer needs before taking it is
  in the Deferred-backlog bullet further down; the scope is in the rows).
