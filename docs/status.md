...
- **What the sitting still has to RULE.** The two below, **plus the ten
  `OI-31`…`OI-40` briefs minted 2026-08-18 and projected below (item 2 is now
  `OI-38`)** — the rest is ruled, countersign-only ([log.md](log.md)).
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-31** — rule whether the nine regenerate-and-compare freshness gates read the STAGED tree instead of the working tree - today an author regenerates, watches the hook pass, and commits without staging, and the hook was honest while the committed tree is stale; 3b8d306d is the measured instance; recommendation is (b) a separate staged-vs-worktree divergence step, warn-first
  - **OI-32** — rule whether docs/cmp/ is materialized as the durable component-design home the shipped CMP schema already points at, or detail_doc retires - the column ships, the directory has never existed, and three living spine rows point into the archive instead because there is nowhere else to point; recommendation is (a) materialize and promote the five documents already identified
  - **OI-33** — rule how a work item leaves its component's spec CORRECT - at the right level of detail, in the right location, with superseded decisions removed, and without restating its own scope - because a WI archives at close and a component spec outlives it, and nothing today connects the two; recommendation is (a) a mandatory component-update deliverable at close with the anti-restatement rule as an acceptance condition, instrumented by (c)
  - **OI-34** — rule how a derived-requirement label's RATIFICATION STATE is carried - 18 live SR rows assert the label in rationale prose, 17 of them with a hand-written PROVISIONAL, unsigned parenthetical that nothing mechanical holds, so deleting the words would silently promote 17 unsigned claims to ratified ones; recommendation is (a) one schema cell
  - **OI-35** — rule whether the no-provenance-in-a-living-cell rule extends to a registry file's # header block, or headers are declared documentation-ABOUT-the-registry and out of scope - three headers carry 45 history tokens today, four of them of the shape that GATES under --strict when it appears in a cell; recommendation is (a) extend, warn-first
  - **OI-36** — rule whether the 49 pre-existing work-item/decision citations in IF Contract cells and the 8 remaining citation frames in IF/LLR/TC cells are swept under the 2026-08-18 provenance ruling or scoped out with a stated reason - today they are outside it by a log sentence that nothing pins; recommendation is (a) sweep the 8 now, (b) hold the 49 for the interface lane
  - **OI-37** — rule the contradiction between SR-043's fail-open-on-any-error clause and the bound SN-006's text now promises - a gate error currently DROPS the ceiling the need states as normative, and all three resolutions are behavior changes only the owner may pick; recommendation is (b) a narrower independently-enforced ceiling
  - **OI-38** — rule SR-040's resume-surface size tripwire, whose carrier was deleted with the serial resume driver and whose SUBJECT no longer exists either - the nearest live look-alike was examined and refused on measured evidence, so the row states an obligation nothing satisfies; recommendation is (b) STRIKE, with the two hat lenses answered explicitly rather than silently
  - **OI-39** — rule whether the LLR CodeSymbol anchor rule - the computation that decides when decomposition is FINISHED - is owed a stating SR, or stays a decision-registry rule with a code carrier; today it is normative only in OI-20's ruling, SR-147 states the carrier and SR-158's acceptance names only the verdict shape; recommendation is (a) mint it
  - **OI-40** — rule the design/test half of SR-040's retired tripwire - both rows were corrected on 2026-08-18 against the live code after LLR-037 was found publishing a DELETED symbol and TC-040 was found Approved while claiming a clause no test performs; the pair follows OI-38 and cannot settle before it; recommendation is (b) correct shut, tracking OI-38's strike
