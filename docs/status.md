...
- **The calls the sitting still makes — reconciled `2026-08-17l`, re-pruned
  ...
  **What remains before the brief regenerates is the pre-brief work pair
  alone** — the `SR-140` split and the `SR-173` wording. The brief WAS
  regenerated `2026-08-18a` (the EARS pass amended `SR-043`/`154`/`155` and
  added `LLR-178`/`TC-173`, so the freshness check went red); that pair is
  still owed and the brief regenerates once more when it lands. The
  `SR-140` split is now also a **standing `trace.py` form finding** (3
  `shall` in one row), which the quality bar §3 states makes visible.
...
- **The amendment window closes at that sitting, not before** — deliberately.
  Gate `DevBar-Reqs`, stage `DevStg-Boundary` (1/8). Sitting-3's **§0.3 ledger
...
- **Spine:** **SN=27 SR=70 LLR=160 TC=156** (70 drafts) · 123 seams · 4 components.
