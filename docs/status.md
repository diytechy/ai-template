...
- **Spine:** **SN=25 SR=66 LLR=76 TC=76** (0 drafts) · 64 seams · 5 components.
...
- **Next action:** this branch (`dualplan-routing-fix`) carries four
  downstream-found **OpenAI-path** fixes — WI-215 (the routing-ON dual-plan
  crash + raw-transcript output), WI-216 (session prompts via stdin vs the
  Windows `codex.CMD` 8191-char cap), and WI-217 (ALL claude sessions via stdin
  too, since ~70 K briefs blow the 32 K CreateProcess cap; + codex sessions read
  back `--output-last-message` since codex echoes the prompt into stdout). All
  surfaced in `C:\Projects\gilbert` and were confirmed latent here.
