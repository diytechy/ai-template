...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  **stage unification program**, whose six ruled slices are all BUILT per
  ...
  pinned by its own test, plan §2 amended); PB-002 is re-measured on the live
  pair. The stale-Approved-cell batch is RULED onto the tracked-repair route
  (`OI-53` (b)+(d), 2026-08-22; the repair row and the warn-first
  ...
  recommendation): the terminal-WI archive move, staleness headers,
  session continuity, the consolidation doctrine, and the
  bootstrap-bundle question.
- **The `wi455-architecture-retirement` lane owns what the sitting does not:**
  ...
  **49 held provenance citations** (`OI-36` ruled 2026-08-19) — the hold's
  ...
  that same precondition satisfied and are now UNBLOCKED: the
  `owner`-reading question is ruled (`OI-54` (a), 2026-08-22 — owner names
...
- **The `wi448-common-module` lane is OPEN with slice 1 landed** (2026-08-20):
  ...
  slice. **`OI-48` is RULED (d) (2026-08-21)**: one owning component plus a
  declared-kernel seam exemption, generalised into a reuse provision (a
  declared LIST with per-entry reasons, never a kitlib hardcode). The
  execution row is queued in the frontier below and unblocks the remaining
...
- **The `wi483-core-decomposition` lane is OPEN with slice 1 landed**
  ...
  than joining `LLR-181`'s four-way tag, precisely so `OI-48` stayed free to be
  ruled either way — it now is ((d), 2026-08-21), and the ruling's execution
  row reconciles this tag choice with the ruled ownership.
...
- **In stage:** **DevStg-Arch** (stage 4 of 8, architecture (partition) in work) (per-phase `1=DevStg-Arch;3=DevStg-Arch;4=DevStg-Arch;5=DevStg-Arch`, derived current **phase=5**) — the rung this repo is IN, derived over its settled spine. [`derive_stage.py`](../project-trajectory/scripts/derive_stage.py) derives it, recorded in [`docs/stage`](stage).
- **Spine:** **SN=27 SR=73 LLR=168 TC=164** (0 drafts) · 129 seams · 4 components.
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-55** — the owner (2026-08-22): move complete/cancelled/partial work items to archive so WI history does not get conflated with active work space - docs/work/ currently holds ~480 terminal specs beside the ~15 live ones, so every agent that lists the registry wades through history to find the frontier, which is exactly the stale-doc churn the owner wants prevented
  - **OI-56** — the owner (2026-08-22): should generated items carry a header telling LLM agents not to reference them (they could be stale - point at the toml/generator instead), and should archive documents carry a history-only header - both serving one goal, agents must not churn on documentation that could be very old
  - **OI-57** — the owner (2026-08-22) wants the prose templates handed to the adjudicator vs the working session reviewed side by side, and wants to know whether a session/adjudicator can survive THROUGH work items - resume from standing state and automatically restart itself when its context exceeds ~66% - so long-running lanes do not die with a context window; the templates live at project-trajectory/prompts/ (worker/reviewer/critique + five adjudicate-* forms), indexed by the generated CATALOG.md, with changes readable via git log -- project-trajectory/prompts
  - **OI-58** — the owner (2026-08-22): CLAUDE.md/AGENTS/PROCESS prose that minimizes CHANGES scopes agents to the current edit and thereby preserves duplication - where two flows are 0->B and 0->D, consolidate to 0->A->B and 0->A->D; where outputs A/B/C exist and B is an overlap of A and C, restructure; the goal is minimum total behavior (calls, not lines), which points at an architectural remapping exercise and at how this meshes with the antidote skill (C:/Projects/antidote - root-cause-over-band-aid enforcement, 'delete the broken path; one clean fix replaces a dozen guards')
  - **OI-59** — the owner (2026-08-22): if the template is a bundle, doesn't bootstrap.py need to ship with the bundle even if it only runs once at repo initialization? - today bootstrap.py is deliberately absent from its own MAPPING (it never copies ITSELF into the scaffold), which WI-498's recovery hit concretely: the migration recipe told adopters to run a file their repo does not have
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-494** `P3` — The declared-kernel seam exemption with a reuse provision (OI-48 ruled (d), 2026-08-21)
  - **WI-498** `P3` — The stage unification program: one axis, one vocabulary, one owner (OI-51 ruled 2026-08-2…
  - **WI-501** `P2` — The stale-Approved-cell repair batch: the CodeSymbol dozen plus the seven post-unificatio…
  ...
  - **WI-495** `P2` — The interface-rework exception dossier for the owner's ratifying commit (OI-49 ruled (b),…
  - **WI-500** `P2` — The test-evidence carrier: the input that makes DevStg-Release reachable (owed by the rul…
  - **WI-390** — PROGRAM CLOSE for concurrency-v2 (docs/concurrency-v2.md §A9 deletion ledger). NOT a swee…
  - **WI-503** `P3` — The re-attestation brief splits: a regenerated CURRENT.md plus dated briefs that are immu…
  - **WI-496** `P2` — Re-tier smoke to fit its 60 s ceiling everywhere, then enforce the seconds at the commit…
