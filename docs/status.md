...
- **RESUME HERE (2026-08-11) — the carrier migration is LANDED; batch-2 is
  ...
  **What is still owed on §8.1, and why it is sequenced rather than skipped:**
  `docs/requirements/interfaces.csv` waits on **OI-14** (which rewrites what a
  `Contract` cell may hold) and `docs/requirements/components.csv` waits on the
  components ruling (which is *about* CMP rows) — converting either first means
  converting it twice.
...
- **Stage:** **0** (needs in process) · next gate: **G1** (per-phase `1=G2;2=G3;3=G3;4=G0;5=G0`, derived current **phase=4**) — a repo is IN a stage and PASSES a gate; the harness at that gate is the bar. [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) derives both, cached to [`docs/gate`](gate).
- **Spine:** **SN=37 SR=147 LLR=149 TC=146** (46 drafts) · 113 seams · 5 components.
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-14** — rule what an IF row's Contract cell must contain, may contain and must not - the owner ruled the DIRECTION 2026-08-12 (interfaces carry INTERFACES ONLY, each signal typed discrete vs variable), and a re-measurement the same day found the drift this item was raised about has ACCELERATED rather than held: 113 live rows now, requirement voice up from 1 to 7 percent, repo-lock citations up from 11 to 14, median cell length 260 to 325; recommendation is DECLARE the contract and BUILD THE SCHEMA TIER now, split the existing cells gradually, and rule it in one sitting with the component partition
  - **OI-15** — rule the component partition - the owner ruled its DIRECTION 2026-08-12 (components define the boundaries system requirements are written against), but the partition itself is unruled and it GATES OI-14, the interfaces.csv carrier conversion, and SN-037; recommendation is to score candidate partitions against a declared objective (the method is now researched) and bring the shortlist to a sitting, because a partition an agent chose alone is the failure mode the owner named
  - **OI-16** — rule whether kit scripts may import a declared common sibling, inverting the F5 standalone-copy default - the owner re-posed this 2026-08-12 (-basically moot given how things have grown-) and a fresh AST census agrees: 32 of 55 scripts already import a sibling and the standalone-required list reduces to ONE file; recommendation is to invert the default with an explicit exception list, executed AFTER the lock
  - **OI-17** — rule the tier of SN-034 (one-action dev-setup + agent-resume) and SN-035 (a run menu for this repo) - the owner's intake words them as needs, an adversarial review argues both are single-requirement items that this sitting's own precedent would demote, and SN-035 additionally proposes REVERSING the recorded no-product-launchers non-goal; recommendation is DEMOTE SN-034, KEEP SN-035 as the need that carries the non-goal amendment
  - **OI-18** — rule whether the ten edge-case SNs retire, their obligations dissolving into the requirements already carrying them - measured, EIGHT of ten already share their SR with a core need so the dissolution is mostly dropping an id from a ref list, and only two need a re-anchor judgement; recommendation is DISSOLVE, in the sitting's already-open amendment window
  - **OI-19** — rule the declared expert-perspective roster SN-036 requires - the CARRIER is already ruled (TOML, per the owner) but the CONTENTS, the file's home and the injection point are not; recommendation is a six-hat starting roster in docs/requirements/hats.toml injected by the decomposition brief builder, with each hat naming the failure class it exists to catch
  - **OI-20** — ratify the rule that an LLR's CodeSymbol/Module must bind to a real symbol before the row counts as discharged - built under a -proceed-, gating HARD under --strict today, and the LAST agent decision from the 2026-08-11 batch still owed a yes or no; recommendation is RATIFY as built, with the four unfounded rows left honestly unfounded
  - **OI-21** — confirm that -retire gate semantics for stage semantics- meant retiring gate-as-STATE while G1/G2/G3 keep their names as human-certified boundaries - that reading is EXECUTED and shipped, so a different reading is a second and larger rework; also confirms six smaller execution calls; recommendation is CONFIRM, and say so explicitly if the gate vocabulary was meant to go entirely
  - **OI-22** — rule whether the prepared exact-replacement text for 29 stakeholder needs and 17 system requirements is adopted - it is written, twice adversarially reviewed and waiting, and it should be worked WITH the 25-row re-attest so two re-blessing windows collapse into one; recommendation is ADOPT at the sitting, never piecemeal
  - **OI-23** — rule the spine rows whose RATIFIED text describes a world that no longer exists - a column-count clause after the move off CSV, six dial names after the toggles moved into process.toml, a citation to an archived spec path, and a docstring claim a later work item proved false; recommendation is AMEND ALL AS ONE BATCH inside the sitting's open window
  - **OI-24** — rule what to do about a stakeholder need whose central obligation - CI runs the SAME harness a human runs - is TRUE, shipped and working, yet has no test behind it because every existing test reads triggers, pinned actions and job names rather than the command; recommendation is BUILD THE CHEAP HALF (one string search proving one definition of passing) and decline the rest explicitly
  - **OI-25** — rule a naming collision that has already produced false findings: gate_policy is BOTH a retired configuration enum AND a live runtime label, and the shipped options document still instructs through the retired vocabulary at ten distinct token lines that no test can see; recommendation is RENAME the runtime label and sweep the doc, both as one small change
  - **OI-26** — rule what a red test case is allowed to PRODUCE - the adjudicator that examines one can today answer only DRAFTED or NEEDS-JUDGEMENT, and neither is -mint a work item to plug the gap-, which is the outcome the owner expects; recommendation is ADD THE VERDICT, since the discovery half already works and only the disposition is missing
...
- **Bar (per commit):** `python -m pytest -q -n auto -m smoke` (~3.3 min) +
  ...
  G3-only `lint` and `--require-verified`, the `--strict` trajectory
...
- **AN AMENDMENT WINDOW IS OPEN, and it is the biggest one this repo has
  carried: `drafts=33 modified=21`, gate derived G1.** That is not drift — it
  ...
  **record** at the end rather than as the vehicle. Those rows are `Draft`
  ...
  it for the P0 sitting; a Draft SN reads G0, so the gate dropped. The drop IS
...
- **The owner decision surface is now [open-items.html](open-items.html)**,
  ...
  **20 SRs to `Modified`** and dropping the derived gate **G3 → G2**
  (`ex-draft=G2` — amendment pressure, not new drafting). **One re-attest sitting
