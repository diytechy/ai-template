...
- **The frontier is OPEN and the grind runs in series** (owner directive
  ...
  **stage unification program**, whose six ruled slices are all BUILT per
  ...
  OPEN for its CLOSE, which owes the owner four things named in its spec — a
  disposition for the superseded product-floor row (WI-473); the APPROVED spine
  cells whose design prose still describes the deleted module (re-authoring them
  is ratification-bearing, not a worker's act); a re-decision on whether
  `docs/process.toml` belongs in the stage's `DECLARED_INPUTS`, now that the
  close MEASURED the over-inclusion's real cost as a red commit bar after any
  policy-dial edit rather than the "milliseconds" its comment claims; and a
  re-measurement of PB-002, whose `fig:` provenance names a command that no
  longer exists. Still with the owner:
