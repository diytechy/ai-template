...
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-293** — Dashboard dark-theme hub contrast fails WCAG AA (116-CRITIQUE A4)
...
- **Perceptual debt gates the two render trains.** The critique judges the whole
  ...
  never touched:
  - **WI-273** (SR-052) is held by **A4 alone** — [WI-293](requirements/work-items.csv).
    A1/A2/A3 already pass on the current render, so that one contrast fix is the
    whole distance to an APPROVE. Its own registry `Status` is still `blocked`.
