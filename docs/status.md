...
- **Stage:** **0** (needs in process) · next gate: **G1** (per-phase `1=G2;2=G2;3=G2;4=G0;5=G0`, derived current **phase=4**) — a repo is IN a stage and PASSES a gate; the harness at that gate is the bar. [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) derives both, cached to [`docs/gate`](gate).
