...
- **Next action:** **WI-257** (queued, medium, SR-052;SR-053) — edge-router
  round 3: lane-route backward edges (they hide under their own endpoint
  boxes — 080's strongest letter-passing finding) + the two 111-REVIEW-A
  MINORs (outboard-stub span residual; dense-overlap perf); SpecRef
  [111-REVIEW-A](reviews/111-REVIEW-A.md). Then **WI-258** (queued, quick,
  SR-054) — clip-edge marker + focus-outline colour check
  ([080-CRITIQUE](reviews/080-CRITIQUE.md)). Both touch the render surface —
  bundle a fresh critique with the build (the render is green now:
  080-CRITIQUE APPROVE f=0, T1–T8 all pass).
- **Standing floors just armed** (2026-07-20, WI-251/252/254 — record in
  [log.md](log.md)): rule **R-F** holds `docs/specs/` to open-WI specs only
  (fail-closed at G2/G3); `test_dogfood_sync` walks `bootstrap.MAPPING`
  against the declared-omissions list; the plan/build cadence and the
  trajectory layer are mutually exclusive; pre-commit sweeps `.coverage*`
  residue.
