...
- **Next action:** **WI-229 landed** 2026-07-19 (the M-05 spine migration:
  ...
  200 chars). The queued frontier is the remaining two: **WI-238** — the blocked
  disposition survives a BlockRef-column-less registry (extend in-transaction
  or fail loudly); **WI-239** — a later completion trailer supersedes an
  earlier `Blocked-WI` so a cured blocker doesn't discard finished work (soft
  on WI-238).
