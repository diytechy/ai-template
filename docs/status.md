...
- **Next action:** **WI-255** (queued, medium, SR-052;SR-053) — edge-router
  hardening: the `_detour_d` port-stub-corridor fail-open + detoured
  `sw_graph` label anchoring (the two render-surface 110-REVIEW-A MINORs;
  SpecRef [110-REVIEW-A](reviews/110-REVIEW-A.md)). Then **WI-256** (queued,
  quick, SR-053;SR-054) — desktop clip affordances from the
  [079-CRITIQUE](reviews/079-CRITIQUE.md) follow-ups. Both touch the render
  surface, so each build re-fires the WI-243 staleness gate — bundle a fresh
  critique with the build (T8 itself is green: 079-CRITIQUE APPROVE f=0).
