...
- **START HERE if you are picking up after 2026-08-01:**
  ...
  **cancellation**, so the refutation is a trunk fact rather than a deleted
  ...
  have no id yet.
- **RESUME HERE (2026-08-10) — the carrier migration, D-5.** The owner ruled
  **OI-12: one TOML carrier**, and brought it **forward of the P0 sitting** so
  the remaining schema work lands on the new format once. Read
  [repo-lock.md](repo-lock.md) **§2 D-5** for the ruled shape, the guardrails
  and the state; **§5 step 6a** for the order of what is owed. Two things a
  fresh session must not rediscover: the migration is **carrier-only** —
  `Status` keeps `Modified`, or the sitting's 38 rows are laundered (Q11) — and
  the cutover **must** carry a carrier-aware baseline read, or the 25-row
  re-attest brief silently reports "no baseline" for every row. The converter
  is shipped and proven lossless; **no registry has moved yet**.
  **Steps 1 and 2 of §5 step 6a are DONE** (2026-08-10): the carrier-aware
  baseline read on both sides of the two-tree scan, and
  `scripts/spine_carrier.py` — the one home for the tier tables, the
  key→column vocabulary and both readers, an **owner ruling amending F5**
  (repo-lock **D-6**). Nine readers go through it. **What the cutover still
  needs first:** `intake` (which also *writes* — step 4), `agent_loop`,
  `agent_common`, `check_docs`, and every reader of the **SN tier**, whose
  shape differs and which the cutover converts. The full owed list, and the
  three things the wiring cost, are in repo-lock **§2 D-5 "State"**.
...
- **Active gate:** derived **G1** (per-phase `1=G2;2=G3;3=G3;4=G0;5=G0`, derived current **phase=4**) — the harness at the derived gate is the bar; [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) computes it, cached to [`docs/gate`](gate).
- **Spine:** **SN=29 SR=147 LLR=149 TC=146** (35 drafts) · 106 seams · 5 components.
- **Open items** _(pending rows of [requirements/open-items.csv](requirements/open-items.csv); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-13** — rule what Status means across the six registries that carry one - the word names six different vocabularies today, one of them (interfaces.csv) undeclared, self-overlapping, read by nothing mechanical, and fed verbatim to LLM planning briefs; recommendation is to reserve Status for ratifiable-artifact maturity and rename the rest, executed WITH OI-12 because both rewrite registry definitions
  - **OI-14** — rule what the Contract cell must contain, may contain, and must not - measured over all 95 live rows it carries design narrative and history rather than a pinned specification (1 percent requirement voice, 13 percent a callable signature, 27 percent a WI id), and nothing validates any of it because --strict-schema covers SR/LLR/TC only; recommendation is DECLARE now and SPLIT gradually, never a 95-row rewrite
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-422** — Measured dead-symbol sweep across project-trajectory/scripts: nothing in the kit catches…
  - **WI-423** — Fold the six remaining check-enablement toggles into docs/process.toml, or rule that they…
  - **WI-424** — Route each ADJUDICATE session to its own adjudicator brief
  - **WI-425** — Repoint the ~71 live source citations of the three SN ids the 2026-08-10 sitting retired…
