...
- **Next action:** the M1–M3 dispatcher migration is **complete** (WI-208/209/
  210 done — one engine, one selection path; evidence in [log.md](log.md)).
  The registry frontier is the deferred backlog above plus two recorded
  residuals worth triage as WIs: the dual-plan round files children without a
  `SafetyClass` (they quarantine until audited — WI-209 finding), and the
  `schedule.py ready --explain` text renderer's duplicate-`reasons` formatting
  defect (WI-208 finding). Owner items OI-3 (the `main`-integration residue —
  now also carrying three landed migration commits) / OI-4 / OI-7 stay open
