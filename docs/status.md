...
- **Next action:** this branch (`dualplan-routing-fix`) now also carries
  WI-220's fail-closed divergence guard, disposition freshness regeneration,
  and round-evidence salvage. The queued frontier (scheduler-derived order):
  **WI-219** — the 390 px dashboard clipping/overflow repair + render-matrix
  rerun; **WI-221** — raise
