...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  ...
  - **OI-17** — rule whether SN-034 (one-action dev-setup + agent-resume) and SN-035 (a run menu for this repo) stay needs or demote to requirements; recommendation is DEMOTE SN-034, KEEP SN-035, which carries a non-goal reversal a requirement cell cannot hold
  ...
  - **OI-26** — rule what a red test case is allowed to PRODUCE - the adjudicator can answer only DRAFTED or NEEDS-JUDGEMENT, neither of which schedules work; recommendation is ADD THE VERDICT, since the discovery half already works and only the disposition is missing
  - **OI-27** — rule the method for re-syncing a repo on an older kit via LLM - no version was ever designated (the anchor is the write-only docs/kit-version SHA stamp), the mechanical paths are skip-all or clobber-all, and 34 migration recipes live as prose in two drifting homes; recommendation is a shipped migration registry with per-entry detectors - state first, SHA range as fallback
