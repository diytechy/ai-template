...
- **Perceptual debt gates the two render trains.** The critique judges the whole
  document and APPROVE requires **every** anchor, while the WI-243 gate needs a
  CRITIQUE APPROVE at the reviewed head — so a render train cannot close while
  any unrelated anchor fails, including anchors it never touched:
