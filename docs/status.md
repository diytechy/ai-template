...
- **Active gate:** derived **G2** (per-phase `1=G2;2=G3;3=G3;4=G2;5=G2`, derived current **phase=5**) — the harness at the derived gate is the bar; [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) computes it, cached to [`docs/gate`](gate).
- **Spine:** **SN=32 SR=158 LLR=171 TC=168** (0 drafts) · 99 seams · 5 components.
...
- **Open-items policy** _(`OI-N` ids are stable and never renumbered;
  ratification history lives in [log.md](log.md) Decisions; the derived
  escalation level is **`autonomous`** (`attestation.human_ratification_through`
  = 0 — owner directive 2026-07-15, carried across the P13 cutover) — so the
  loop does **not**
