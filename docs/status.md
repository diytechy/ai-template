...
- **Spine:** **SN=25 SR=135 LLR=127 TC=124** (0 drafts) · 82 seams · 5 components.
- **Open items** _(pending rows of [requirements/open-items.csv](requirements/open-items.csv); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  - **OI-11** — rule whether concurrency-v2 B2's second sentence is STRUCK or RESTATED - a decision about the DESIGN TEXT only; WI-391, the row that would have built it, is already cancelled under either answer
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  - **WI-381** — RULED 2026-07-31 (docs/concurrency-v2.md §A4)
  - **WI-392** — Filed 2026-08-01 by WI-378 out of WI-386's REVIEW-A, whose reviewer disclosed the bias hi…
  - **WI-393** — Rehome the LINK-AWARE SPEC-ARCHIVAL RITUAL that Phase 5 deleted
  - **WI-394** — A Verified spine row can cite evidence that has NEVER EXISTED
  - **WI-395** — A `blockref` is a LABEL, not a SUBSCRIPTION
  - **WI-396** — `check_doc_refs` silently cannot see a SUFFIXED reference into `project-trajectory/`
  - **WI-398** — A RED BAR'S REFUSAL MUST CARRY THE FAILING STEP'S OWN OUTPUT
  - **WI-399** — CONTAINMENT IS OWED WHERE A MODULE IS ADDED, NOT WHERE THE STATION REGENERATES THE INVENT…
  - **WI-400** — THE UNLOAD MUST DISTINGUISH DECLARED TOOL-RESIDUE FROM EVIDENCE
