...
- **Active gate:** derived **G2** (per-phase `1=G2;2=G3;3=G3;4=G2`, derived current **phase=4**) — the harness at the derived gate is the bar; [`derive_gate.py`](../project-trajectory/scripts/derive_gate.py) computes it, cached to [`docs/gate`](gate).
- **Spine:** **SN=27 SR=136 LLR=137 TC=135** (0 drafts) · 91 seams · 5 components.
- **Ready frontier** _(dependency-ready WIs in build order — generated from the scheduler; a closed WI drops out automatically, so this list is never stale and never names a `done` id):_
  ...
  - **WI-416** — dispose: WI-413 handed back (e56f4e2c201a)
  - **WI-415** — Process-tab polish: the ff-wording one-string fix and the 390px legibility observation (W…
  - **WI-417** — The handback reason string does two unrelated jobs and one of them is a layering violatio…
  - **WI-418** — Derived prose injected at claim can ANCHOR the agent it briefs
...
- **Open-items policy** _(`OI-N` ids are stable and never renumbered;
  ratification history lives in [log.md](log.md) Decisions; `gate_policy` in
