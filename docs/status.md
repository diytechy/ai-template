...
- **THE INTERFACE `contract` CELLS ARE CLEANED, AND THE PLACEMENT QUESTION IS
  ...
  **What is owed the owner next is now CARRIED BY A ROW — `OI-65`, filed
  ...
  1. **The placement re-ask** — does the irreducible remainder move to a
...
- **The `wi508-architectural-remap` lane is OPEN with slices 1–4 landed**
  ...
  **12 severity words for four dispositions** and six escalation-flag
  ...
  entangled with `OI-65` above: the owner's direction puts the mint target at
  the interface tier, and where interface contracts live is what that row
  decides — so `OI-65` recommends being ruled first, and nothing in it depends
  on this one. Also recorded on the row: of the four restatement families it
  bundles, the fourth — *every degrade is named, never silent* — is the one the
  measurement did NOT sweep.
...
- **Open items** _(pending rows of [requirements/open-items.toml](requirements/open-items.toml); each item's blast radius, options and recommendation render in [open-items.html](open-items.html), the generated owner surface):_
  ...
  - **OI-65** — OI-63's re-ask, FILED so that it has a row. Ruling (d) cleaned the cells - 36 rows changed, contract over the 108-row population 43,995 -> 37,859 characters, rows carrying a rationale 1 -> 37 - and deferred placement until the cells held only crossing facts. They now do. This row re-asks placement with the owner's file-far-side correction and OI-64's one-definer rule in hand, and carries the two sub-decisions the cleanup could not take: ten rows of non-crossing content rationale's grammar refuses, and an IF rationale cell that went from 1 user to 37 with no lint arm reading it.
