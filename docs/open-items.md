...
- **WI-273** blocked — attest/ratify `SR-084 family-heterogeneous CRITIQUE dispatch (preferred not mandated) — SR-052 (Verification=Critique) needs a fresh non-authoring-session critique against docs/rubrics/dashboard-accessibility.md; a genuinely different (non-Anthropic) provider is preferred for stronger corroboration; a fresh same-family session is a legal DEGRADED substitute`, then unblock the registry row.
