...
BASELINE = {
    ...
    "trace.py": 3917,  # +239 2026-08-14: WI-442 — the depth-0 FRAME joins the checker: three tiers loaded off external.toml, their required-field/enum schema rows, the entity/crossing/tie-back resolution rules (`frame_findings` + `tieback_findings`, a --strict failure class of their own), SN-037's SR->boundary rule at its two severities (`sr_boundary_findings`: resolution hard, coverage a summary advisory) and the report section that reads them out (reviewed bump, reason in the log); earlier +221 2026-08-13: WI-443 — the IF/CMP schema tier (required fields, closed vocabularies), the four IF Contract negative rules, and the untagged-endpoint classifier, all warn-first; +8 the adversarial round's refutation recorded in schema_advisories' docstring (reviewed bumps, reasons in the log)
