...
the real decomposition (WI-275) is deferred.
...
- A baselined module grew, or a NEW module crossed THRESHOLD without a baseline:
  the fix is DECOMPOSITION (WI-275), not a baseline bump. A deliberate bump is a
...
This is a growth SENSOR, not an approval of the current sizes. WI-275 is the
...
# as-found `6a752b4`. Re-stamp DOWNWARD as WI-275 decomposes them.
BASELINE = {
...
def test_module_sizes_exactly_match_the_committed_baseline():
    ...
    if grew:
        message.append(
            "module(s) grew past baseline — decompose (WI-275), do not bump "
