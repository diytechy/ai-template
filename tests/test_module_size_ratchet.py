...
BASELINE = {
    ...
    "agent_loop.py": 2610,  # +2 (2608 -> 2610 SLOC) 2026-09-04, WI-580 review A: the fix for the brief calling an unclosed row `built` is a CONSOLIDATION that costs one line — `lane_completion` is now the one place the lane's completion predicate lives (trailer AND spec out of `active/<branch>/`), read by both `current_assignment_wi` (which loses two lines to it) and `assignment_block` (which had derived doneness from the trailer alone, the defect). One added line is the third display state the honest predicate makes visible, `started, not closed`; the other passes `scripts_dir(root)` into the worker template's `{scripts}` slot, reusing the runtime path source `reviewer_prompt` already reads so both meta-repo and scaffold prompts emit executable close commands. Earlier: +36 (2572 -> 2608 SLOC) 2026-09-04, WI-580: the two brief-evidence blocks the shipped templates gained — `assignment_block` (every row a batch lane was claimed with, with its walk state) and `reviewed_rows_block` (`{wis}`, the rows a review round covers), plus `_row_title`, the missing-row placeholder both share. REVIEWED BUMP, not a decomposition, and the reason is which module they belong to: both read `worker["rows"]`/`worker["assigned"]` and one reads `train_evidence` against the lane base — the same three facts `worker_prompt`, `reviewer_prompt` and `current_assignment_wi` already read here, so a new sibling module would export the loop's own session state to import it straight back. Compacted first (45 -> 36 SLOC) per the phantom-overage rule; the standing reduction of this module is owned by WI-545, which declares WI-580 among its needs. Earlier: -3 (2575 -> 2572 SLOC) 2026-09-04, the MINOR-only routing + unchanged-rework pass: two features landed NET NEGATIVE by decomposing OUTWARD rather than bumping — the round's routed verdict moved to `score_reviews.merged_routing_verdict`, the has-a-verdict-already-named-this-tree question and its operator sentence to `kitlib.verdict` (both modules far under THRESHOLD) — plus three consolidations inside the arms they touch: `page_human` for the page prologue the review-escalation and critique paths held in duplicate, `committed_build_rounds` for the three-call sequence the BUILD and DESIGN-CHECK arms held in duplicate, and `absorb_review_verdict`'s twin failure arms (one consequence, two messages). Ratcheted DOWN, no licence. Earlier: -12 (2587 -> 2575 SLOC) 2026-09-04, MERGE of three downward moves measured on the merged tree with check_complexity.module_sloc: WI-589 deleted the duplicate REVIEW_PHASES tuple (-1), the cognitive-complexity decomposition of worker_endstate (-3), and the batch-lane defect pass moving trunk_name/default_base out to agent_common and the branch-tree read out to integrate.claimed_ids_on_branch (-8 on its own base). DOWNWARD on every side, no licence.
    ...
    "integrate.py": 1426,  # +65 (1361 -> 1426 SLOC) 2026-09-04, the station settles its own [generated] conflicts and ignored build residue stops refusing the unload — the two mid-run defects of that day's queue drain, each measured three times. UNAVOIDABLE HERE, not a preference: the kit's scripts may not import one another (the never-import-a-sibling rule the check.py/kitlib.stage split records), so a helper for a station behaviour has one home short of kitlib — this module. The delta is `_generated_table` (the [generated] rows read as {path: kind}; `_generated_paths` becomes two lines over it, so the parser is not duplicated), `_declared_generated_kind`, `_resolve_generated_conflicts` and its wiring in `refresh` (take the TRUNK side of a conflicted declared-generated path and carry on into the trunk step that regenerates it, refuse otherwise and NAME what was settled), plus `_is_build_residue`/`_is_build_residue_dirt` and the symlink arm in the shed. Roughly half is the recorded WHY the ratchet's hatch exists for, and it is the half a successor would otherwise re-litigate: why `linecounts` — THIS file — is the one declared kind held back (its rows are hand-stamped data, so both sides of a conflict carry a reviewed reason), and why a `.venv` is never followed when it is a symlink. Six of the lines are a DECOMPOSITION, not the feature: the merge arm moved out to `_merge_trunk_in` because the conflict handling took `refresh` C901 10 -> 11, so tests/test_complexity_ratchet.py is untouched by this change and the cost sits here instead. Reviewed bump, reason in docs/log.d/2026-09-04-station-generated-conflicts-and-unload.md. Earlier: +7 (1354 -> 1361 SLOC) 2026-09-04, MERGE measured on the merged tree: the census pass shed 2 (`_verdict_gate` -> `_legacy_window_refusal`, the merge-base stripped once) and the batch-lane defect pass added 9 (`claimed_ids_on_branch`, the sibling of `finished_branches` that reads which claimed specs are still under active/<branch>/ on the BRANCH tree - the one home the walk and the merge slot now share). The bump pays for lines agent_loop shed; reviewed at the supervisor merge, reason here.
