...
BASELINE = {
    ...
    "agent_loop.py": 2613,  # +3 (2610 -> 2613 SLOC) 2026-09-05, supervisor: `_predecessor_lines` extracted OUTWARD from `worker_prompt` after WI-580's assignment block pushed it 30 -> 31 over the cognitive ceiling at the DevStg-Impl gate; the loop body moved verbatim, the three new lines are the sibling's def, its return and one docstring line, and the cognitive row for worker_prompt is deleted (it now sits under the ceiling). Reviewed at the supervisor's gate run, reason here.
