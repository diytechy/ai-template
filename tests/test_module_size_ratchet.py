...
BASELINE = {
    ...
    "derive_gate.py": 1526,  # NEW ENTRY 2026-08-20 — the module CROSSED THRESHOLD (1495 -> 1526) at the batch-close iterate pass, and the crossing is disclosed rather than trimmed around. What crossed it is ROUND-SOL MAJOR-6: `maturity_bar` passed `default=APPROVED`, so EVERY unrecognized spine status derived a finished bar — a one-character typo (`Approvd`) read exactly like an approval on the axis the automation dial reads. The fix names the retired spellings instead (`SPINE_TRANSITIONAL` + `SPINE_MATURITY_READ`, ~20 lines incl. the per-word reasoning), so the documented migration tolerance is unchanged in both directions and the typo hole closes. The executable delta is two constants and one lookup swap; the rest is why each retired word reads as it does — the part a successor would otherwise collapse back into a blanket default. This module is a TABLE module (four maturity ladders + the bar arithmetic) rather than a coordinator, so a decomposition would split ladders from the arithmetic that reads them; the honest response to the crossing is this entry, and a re-stamp downward if the stage-axis half is ever extracted. Reviewed bump, reason in the log.
