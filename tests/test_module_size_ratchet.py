...
BASELINE = {
    ...
    "derive_gate.py": 1523,  # RE-ENTERED 2026-08-21, WI-498 slice 3: +56 (1467 -> 1523), and it is COMMENT MASS on a slice that DELETED executable lines. The Impl->Release cell discriminator is gone (two lines out, one in), and what grew is three records the next reader of this module needs where the mechanism is: (a) `spine_stage`'s docstring now carries the OI-30 D2 rule itself — "a Status cell may never claim the test evidence passed" — because the stage axis is where that guard is now enforced, by the top rung having no producer at all; (b) the `_RELEASE_CEILING` block carries the old->new MAPPING (rule unchanged; stage half deleted; bar half KEPT while docs/gate is still written for slice 4's detectors; exit condition NOT claimed, since the harness driver has not landed) so nobody reads the surviving ceiling as vestigial and lifts it; (c) the fall-through records the POLARITY INVERSION, since a reader who does not know rung 6 used to mean the opposite will misread the new arm as a tightening. NOT TRIMMED TO FIT: the +56 was measured after the fact, and cutting prose to clear a threshold is the ratchet inverted (slice 1's own near-miss note). Re-stamped deliberately, reason in docs/log.d/2026-08-21-wi498-stage-unification.md. THIS IS THE THIRD ENTER/DELETE CYCLE IN THREE SLICES (1503 in, deleted at 1467, back at 1523), which says the threshold sits exactly where this module oscillates while it holds two axes; the resolution is still slice 5's DELETION of the bar half with docs/gate, not a bump here.
    # (the deleted slice-2 tombstone, kept because the prediction it records is
    # the reason this entry exists at all:)
    # derive_gate.py: ENTRY DELETED 2026-08-21, WI-498 slice 2 — it fell to
