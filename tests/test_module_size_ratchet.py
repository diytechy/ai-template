...
BASELINE = {
    ...
    "agent_loop.py": 2579,  # -8 (2587 -> 2579 SLOC) 2026-09-04, the batch-lane defect pass, DOWNWARD and with new behaviour in it. Two moves OUT and one thin reader in: `trunk_name`/`default_base` went to `agent_common` (re-exported here beside their siblings) because the worker PREFLIGHT there must ask the same "where does this branch's own work begin?" question and a second merge-base rule would be a second answer; and the branch-tree read behind `claimed_on_branch` went to `integrate.claimed_ids_on_branch`, beside the `finished_branches` that asks the same tree the same question. What is left here is the loop's own: resolve the current branch, and let `current_assignment_wi` count a row BUILT only when its `WI:` trailer is committed AND its spec has left `active/<branch>/`. A batch row whose session committed the trailer and ran out before the close ritual satisfied one reader and not the other, so the walk stepped past it forever and the branch never counted as finished (measured 2026-09-03, lane `wi-589-…`, four rows on one branch). Re-stamped DOWNWARD in the same commit, per this file's rule; reason in docs/log.d/2026-09-04-batch-lane-fixes.md. Earlier: +4 (2583 -> 2587 SLOC) 2026-09-03, owner ruling after the WI-579 lane: a committing DESIGN-CHECK arms the review round itself (`build_bookkeeping`, the three-statement arm mirroring the BUILD one) - the BUILD session that ran after every design-check only to re-arm the round cost 13 sessions / 5.1 h on one lane. Reviewed bump, reason in docs/log.d/2026-09-03-design-check-arms-the-round.md. Earlier: +3 (2580 -> 2583 SLOC) 2026-09-03, WI-579 (REVIEW-A round 034): the round-033 rework inverted the C2 resume contract, and the three lines are the fix. Making the evidence the only trigger left `review_owed_by_evidence` answering `[]` for two different states - `read it, nothing owed` and `could not read it` - so a resumed run whose `default_base` merge-bases to HEAD (the single-checkout shape its own docstring names) scanned an EMPTY range, read every assigned WI as unbuilt, and dropped a genuinely parked round to run another BUILD. The derivation now answers None for `cannot say`, and `resume_owed_round` re-ASKS it at the base the marker carries rather than answering for it: one condition, one binding, one re-ask, and no line is compactable further (the ratchet was re-measured after ruff-format at each shape). That is round 033 finding 2's own antidote applied one level down - the two-states-one-falsey-value defect it deleted in `schedule_review_round` was left in the function that FEEDS it. Reviewed bump, reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: -3 (2583 -> 2580 SLOC) 2026-09-03, WI-579 (REVIEW-A round 033, findings 1 and 2): DOWNWARD, and no bump was taken for the two behaviours added. `resume_owed_round` stops consulting the untracked owed-marker as a TRIGGER (the evidence decides alone, which is all `write_review_owed` ever claimed for it) and `schedule_review_round` takes its phases as a REQUIRED argument, so "the evidence owes nothing" and "the caller named nothing" stop sharing one falsey value - both are re-orderings and a deleted fallback, not new lines. `complete_review_round` gains a one-line stderr note when a completed round derives NO attestation, which is the only place that absence can be reported (a missing trailer is never a refusal), and it fits inside the SAME hunk because the `commit_telemetry` call it sits beside compacted from seven lines to one. Reason at the entry per the ratchet rule; reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: +4 (2579 -> 2583 SLOC) 2026-09-03, WI-579 (REVIEW-A round 030, finding 2): `dispositions_drafted` asks `agent_common.authoritative_spec` which of the branch's spec copies governs instead of walking the two homes in its own order, which was the OPPOSITE of the merge gate's. The four lines are the collection the selection needs — the candidates are gathered before one is chosen, where the old shape returned on the first file it happened to reach. The precedence itself went OUTWARD to `agent_common`, beside the dial these two callers already share, so what is left here is again only the loop's own read of its own working tree. Reviewed bump, reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: +1 (2578 -> 2579 SLOC) 2026-09-03, WI-579 (round-023 rework): `review_owed_by_evidence` answers with the review PHASES still owed instead of a yes/no, and `resume_owed_round` binds that answer to hand it to `schedule_review_round` — the ONE new line. The behaviour it buys is a wedge closed: the merge slot had just been taught to demand every phase the dial declares while this derivation still read any verdict at the tree as the whole round served, so a run killed between REVIEW-A and REVIEW-B (the phase queue is in-memory) resumed scheduling nothing against a gate refusing the phase nobody would draw. Naming the missing phases is also what keeps the resume from redrawing a served one, which the gate would read as a reroll-until-green. The rule itself went OUTWARD, to `kitlib/verdict.py` (`declared_phases`/`phases_owed`), so what is left here is again irreducibly the loop's. Reviewed bump, reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: +59 (2519 -> 2578 SLOC) 2026-09-02, WI-579 (the 2026-09-02 restructure plan §3, absorbing WI-559 DW2 and WI-560 DW1): `schedule_adjudication_round` + `dispositions_drafted` (a committing ADJUDICATE schedules its round exactly as a committing BUILD does, under the dial), `review_verdict_trailer` (the round's machine half, written by the COORDINATOR — a trailer a reviewer could stamp on its own commit attests nothing), and the tree-identity rewrite of `review_owed_by_evidence`. DECOMPOSED FIRST, and most of the row went outward: the identity, the trailer grammar and the round/session-log join are `kitlib/verdict.py`, a new leaf under THRESHOLD that opens no entry, and the dial's reader is `agent_common`'s. What is left here is irreducibly the loop's — which session owes a round, and what the branch's own tree says about it. The phase deliberately STAYS in `NON_BUILD_PHASES` (a judgement is not a build; widening the set would have bought the round by asserting something false), which is why this is a new arm rather than a one-line set edit. Reviewed bump, reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: +53 (4047 -> 4100) 2026-08-30, WI-535 (telemetry first, dial off): `family_context_telemetry` reads the session id and context occupancy/window/percent straight off the process's own JSON result, per family — no mint, no resume, no adapter — and `session_meta` gains the four columns it feeds. NEW BEHAVIOUR: today's one-shot ANTHROPIC calls already carry enough (`session_id`, `usage`, `modelUsage`) to compute this without any launch change; OPENAI/OPENCODE stay blank until a later WI's per-family adapter lands. Reviewed bump, reason in docs/log.d/WI-535-adjudicator-telemetry-first.md. Earlier: +425 (3622 -> 4047, post ruff-format + the composer extractions the policy/complexity guards asked for) 2026-08-30, WI-548 (the stall-guard plan C1/C2/C4/C5/C7): NEW BEHAVIOUR, not drift - the route-aware stall split (note_review_draw_failure + the judging arm), the C2 review-owed exit and its marker trio (write/clear/stop), the C4 liveness probe pair (probe_route/select_with_probe, kept beside the draw it guards), the C5 relaxed rung and its recording, and the C7 brief-slot resolvers (process_doc_path/trunk_name). Roughly half the bump is docstrings stating WHY each guard exists (the WI-521 incident); the engine gains no second responsibility. Reviewed bump, reason in docs/log.d/WI-548-stall-guard.md. Earlier:+8 (3614 -> 3622) 2026-08-29, WI-530 (OI-67 slice 3): DOCSTRING ONLY — the `Contract IF-###:` bodies this module owns moved out of the registry cells into its header, the one home the ruling names, and its `Contracts:` marker was trimmed to exactly the rows the registry owns to it. No executable line changed. Reviewed bump, reason in docs/log.d/2026-08-29-wi530-cell-pass.md.
    ...
    "bootstrap.py": 1660,  # +2 (1658 -> 1660 SLOC) 2026-09-02, WI-579 (OI-76): two MAPPING rows — `kitlib/verdict.py` and `gen_verdict_rollup.py`. Data, not logic: the must-be-whole rule this list states in its own header (a scaffold missing a module its shipped importer needs ImportErrors on the first merge, and a missing generator halts `trunk_step --regen` before every LATER family). Reviewed bump, reason in docs/log.d/WI-579-verdict-carrier-and-adjudication-review.md. Earlier: +1 (1657 -> 1658 SLOC) 2026-09-02, the backlog-restructure plan §1.6: ONE scaffold row, `docs/archive/work/restructured` — the fourth terminal status directory, so a repo bootstrapped after this date has the folder tracked instead of discovering it on the first consolidation. Not decomposable: it is one entry in the directory manifest, which is the one home for what an adopter's tree contains. Reviewed bump, reason in docs/log.d/2026-09-02-restructured-vocabulary.md. Earlier: +5 SLOC (1652 -> 1657) 2026-09-01, WI-572: one MAPPING row for `prompts/adjudicate-first-approval.template.md`, the fifth adjudicator brief. The manifest is the one home for what ships, so a new shipped file costs this module a row by construction — the same declaration shape WI-483, WI-520 and WI-521 each took here. Reviewed bump, reason in docs/log.d/WI-572-approval-act-adjudicator-only.md. Earlier: +52 SLOC (1600 -> 1652) 2026-09-01, WI-543 REVIEW-A round-2 rework: delivery_inventory independently walks every physical kit source and classifies static, conditional and generated deliveries against the reasoned mapping-source-exclusions carrier, so deleting a MAPPING row cannot delete the source from observation. Reviewed bump, reason in docs/log.d/WI-543-sr163-verification-tc.md. Earlier: +29 SLOC (1571 -> 1600) 2026-08-31, WI-543 (SR-163, OI-72): the tolerant MAPPING reference cell — a row may carry a requirement reference as an optional third element and `mapping_entries()` normalizes every row to `(src, dst, ref|None)`, so a bare pair keeps working (it is by definition an unmapped-entry warning) and the burn-down IS the migration with no flag day. New behavior (the warn-to-gate reference join SR-163 owns), not monolith drift; reviewed bump, reason in docs/log.d/WI-543-sr163-verification-tc.md. Earlier: +13 (3153 -> 3166) 2026-08-29, WI-530 (OI-67 slice 3): DOCSTRING ONLY — the `Contract IF-###:` bodies this module owns moved out of the registry cells into its header, the one home the ruling names, and its `Contracts:` marker was trimmed to exactly the rows the registry owns to it. No executable line changed. Reviewed bump, reason in docs/log.d/2026-08-29-wi530-cell-pass.md. Earlier: +7 (3146 -> 3153) 2026-08-25, WI-521 slice 1: one MAPPING row for `scripts/acceptance_record.py` plus the six comment lines recording why the scaffold cannot skip it — `check_trajectory.py` imports it UNGUARDED and joins its findings to the failure set, so a copy without it cannot run the checker at all, which is a harder dependency than the optional `gen_arch_map` beside it. The same declaration shape WI-483 slices 3 and 4 and WI-520 each took here; the manifest is the one home for what ships, so a new sibling costs this file a row by construction. Reviewed bump, reason in docs/log.d/2026-08-25-wi521-slice1-acceptance-record.md. Earlier: +8 (3138 -> 3146) 2026-08-25, WI-520: one MAPPING
    ...
    "intake.py": 1357,  # +14 (1343 -> 1357 SLOC) 2026-09-02, round 3 of the adversarial review: the non-dependent skip in `_replace_inbound_edges` was decided per ROW over the whole absorbed group, so a successor hard-naming the row it absorbs kept a dead edge and a row that absorbed a DIFFERENT row of the group was exempted from tokens it never absorbed; now decided PER TOKEN (`_repointed_needs` drops a token the row absorbs, replaces the rest; `_token_targets` is the extraction that keeps it under the cognitive ratchet), the re-point announcement names only the edges the row held (`_announce_repoint`), the authoring grammar refuses a joined string inside a list, and the dead `_apply_supersede` is deleted. Reviewed bump, reason in docs/log.d/2026-09-02-backlog-restructure.md. Earlier: +38 (1305 -> 1343 SLOC) 2026-09-02, round 2 of the same adversarial review found the ACCUMULATE arm this file's previous entry describes to be both unsound and a guess, and it is replaced by SET-BASED re-pointing at the mint. Unsound: `siblings` excluded the successor being minted, so a successor whose own `needs` hard-named an earlier sibling appended ITSELF - a wait nothing can clear and no validator reports. A guess: "already names an earlier successor" is equally true of a row that named it on purpose, so an unrelated row silently gained a hard edge and the mint announced a re-point that never happened. The mint now resolves the WHOLE verdict at once - `_apply_supersedes` inverts `[(successor, absorbed)]` to absorbed -> its successors and re-points each group in ONE write, after every draft is on disk, so a dependent gets the full successor list and a successor is recognisable by its OWN written `supersedes` cell rather than by the shape of its `needs`. `_pre_mint_refusal` and `_write_draft` are the two extractions that keep `_mint` under the complexity ratchet across that change (measured: `_mint` back below the threshold, `_replace_inbound_edges` re-stamped DOWN off the baseline in this same commit). The rest is the two new refusals: `_authored_supersedes_refusal` (a hand-authored `supersedes` string names exactly ONE id - the `;`-split is the WRITER's round-trip and not an authoring spelling) and the `absorbed_ids` arm of `_supersedes_refusal` (a successor may not continue an ALREADY `restructured` row - lineage does not chain, which is the same rule the validator now refuses to record). NOT DECOMPOSABLE further as a unit: it is one field's parse, its three refusals and its one effect, all inside the mint that owns them. Reviewed bump; the reason is this entry and the commit that carries it. Earlier: +19 (1286 -> 1305 SLOC) 2026-09-02, the adversarial review of the `restructured` series (891a5b24..c16182cb): ONE absorbed row SPLIT across SEVERAL successors. `_mint` applies successors one at a time, so the first re-point consumed the dependent's token and every later successor found nothing to rewrite — the dependent waited on a third of the contract. `_replace_inbound_edges` now reads the open specs ONCE (`_open_specs`) and ACCUMULATES: `_repointed_needs` appends a later successor to any row already naming an earlier one. The two extractions are what kept the function under the complexity ratchet rather than earning a second bump there. `supersedes_ids` also splits a `;`-joined string, so the cell `_draft_row` writes round-trips through the reader that refuses it. Reviewed bump; the reason is this entry and the commit that carries it. Earlier: +37 (1249 -> 1286 SLOC) 2026-09-02, the backlog-restructure plan §1.5: `supersedes` accepts a LIST. `supersedes_ids` (the one reader of both spellings — a bare id string, or several ids as a TOML list — returning an ordered de-duplicated list, so the cell, the edge re-point and the refusals each have ONE shape to handle) and `_supersedes_refusal` (two arms: a token that is not a WI-### id, which matches no dependent's `needs` and makes the re-point a silent no-op; and, at the mint where the registry is in hand, an id that is no live row — the failure that would leave one of a consolidation's absorbed rows queued beside its own successor). `_replace_inbound_edges` takes the set and re-points every dependent in ONE pass instead of one pass per absorbed id; `_draft_row` and `_apply_supersede` read through the shared parse. Roughly half the bump is the two docstrings recording why the tolerance is on the frontmatter and not on the COLUMN (widening the cell would re-type a column and every reader of it for a shape most rows never carry) and why liveness cannot be checked where the block is parsed. NOT DECOMPOSABLE as a unit: it is one field's parse, its two refusals and its one effect, all inside the mint that owns them. Reviewed bump, reason in docs/log.d/2026-09-02-restructured-vocabulary.md. Earlier: +2 (1247 -> 1249 SLOC) 2026-09-02, WI-572 REVIEW-A round 4: the first-approval mint writes the act's SCOPE into the new `Adjudicates` cell — the row-id list `_draft_row` serializes and `adjudicate_brief.first_approval_values` intersects its LIVE re-derivation against, so a brief can no longer ask a wider question than the merge asked. Two lines: the `_draft_row` assignment and the `adjudicates` key on the drafted row. Not decomposable — it is one cell on one mint, and the judgement that consumes it already lives in another module. Reviewed bump, reason in docs/log.d/WI-572-approval-act-adjudicator-only.md. Earlier: RE-STAMPED DOWN -8 (1255 -> 1247 SLOC) 2026-09-01, WI-572 REVIEW-A rework: the local `APPROVAL_RUNG` table (which DevStg rung each spine tier is approved into) moved to `agent_common.SPINE_APPROVAL_RUNGS` beside the off-spine sibling it mirrors, `_released_drafted_rows` reads the shared `human_approves_spine` predicate instead of a local rung lookup, and the now-unused `kitlib.ladder` import goes with it (both arms of the guarded block). The mint's BEHAVIOUR is unchanged — this is the half of the fix that removes the second copy, not the half that adds the filter. Recorded DOWN in the same commit rather than left as headroom, per this file's rule. Earlier: +76 (1179 -> 1255 SLOC) 2026-09-01, WI-572 (owner ruling 2026-09-01): TRIGGER (a2), the FIRST-APPROVAL adjudication arm — `APPROVAL_RUNG` (which DevStg rung each spine tier is approved into, so the dial can say whether that tier is loop-held), `_released_drafted_rows` (the `Drafted` rows a merge handed over, minus every tier the dial still holds for the owner), `_first_approval_context` and `_first_approval_drafts`. It is the other side of trigger (a)'s coin and is built as its exact mirror, one section below it: a lane authors `Drafted` rows and its merge is refused if it approves them, so the same merged commit that mints an amendment adjudication for moved approved text mints a first-approval adjudication for text nobody has blessed. Roughly half the bump is the two derived-context line lists, which are source lines by this sensor's rule and are the same shape `_amendment_context` beside them already carries. NOT DECOMPOSABLE as a unit: this module's whole job is the triggers, and a fourth trigger in a sibling file would put half of one mint in another module. Reviewed bump, reason in docs/log.d/WI-572-approval-act-adjudicator-only.md. Earlier: +2 (1177 -> 1179 SLOC) 2026-09-01, WI-571: the CLI surface for the NAMED `--approves` — `_cmd_snapshot` parses the `REGISTRY=REF` value through `baseline_snapshot.parse_approves` (one line, replacing the raw `getattr`) and the success banner renders the copied registries and their refs from the parsed dict (the `"; ".join(...)` over two lines, replacing a one-line `.format(approves)`). The scope logic itself lives in `baseline_snapshot` (ample headroom under THRESHOLD); this is the two-line CLI edge. Reviewed bump, reason in docs/log.d/WI-571-snapshot-copy-scope.md. Earlier: RE-STAMPED DOWN -2 (1179 -> 1177 SLOC) 2026-09-01, WI-552 REVIEW-A rework (009): a later ruff/format pass (blank-line normalization) shrank this WI's own touched file below its stamped baseline; the ratchet compares exact-equality in both directions, so the un-restamped baseline left the smoke bar red. Recorded DOWN in the same commit rather than left as headroom, per this file's rule. Reason in docs/log.d/WI-552-adjudicator-two-exit-close.md. Earlier: +3 (1176 -> 1179 SLOC) 2026-09-01, WI-552 REVIEW-A rework (cancelled-close refusal gap): the refusal invariant now reads the durable `dispose:` TITLE prefix (new `owes_successor` + `_DISPOSITION_TITLE_PREFIX`, single-sourced with the two early-close title builders) instead of the `brief == "disposition"` proxy that missed the brief-LESS cancelled arm — `specref` could not serve, the close clears it before the merge-side guard runs. Reviewed bump, reason in docs/log.d/WI-552-adjudicator-two-exit-close.md. Earlier: +2 (1174 -> 1176 SLOC) 2026-09-01, WI-552 close: ruff format canonicalization (blank-line normalization) of this WI's own touched file; no executable change. Reviewed bump, reason in docs/log.d/WI-552-adjudicator-two-exit-close.md. Earlier: +93 (1081 -> 1174 SLOC) 2026-09-01, WI-552 (OI-70/OI-73): the OI-70/OI-73 mint arms: _mint_open_item/next_oi_id/_inject_open_item (exit B), _replace_inbound_edges/_apply_supersede (inbound-edge replacement), the disposition refusal invariant, and _write_context extracted from _mint. Reviewed bump, reason in docs/log.d/WI-552-adjudicator-two-exit-close.md. Earlier: +5 (1985 -> 1990) 2026-08-30, unattended-run decision 26: a minted successor's Context carries the adjudicator's scope prose after its draft block verbatim (WI-544 review round 2 - the cells alone carry no boundary or exclusion); reviewed bump. Earlier: +1 (1984 -> 1985) 2026-08-30, unattended-run decision 25: `_draft_row` now copies a drafted successor's `supersedes` cell (LLR-161 lineage) - the row schema carried the column and the draft validator accepted the key, but the writer never wrote it, so every minted successor lost its thread; one assignment, reviewed bump. Earlier: +7 (1977 -> 1984) 2026-08-29, WI-533 follow-up (cross-family review F5): `_locate_spine_rows` reads its CSV carrier through `kitlib.spine.csv_body` like every other kit loader — read RAW, a `#` declaration header's first line became `rows[0]`, so the registry had no `Status` column, every staged row read as ABSENT and the brief reported nothing to adjudicate. The lines are the package import (both arms of the existing try/except), the `io` import beside the local `csv` one, and the four-line comment naming the trap. Reviewed bump. Earlier +14 (1963 -> 1977) 2026-08-29, WI-530 (OI-67 slice 3): DOCSTRING ONLY — the `Contract IF-###:` bodies this module owns moved out of the registry cells into its header, the one home the ruling names, and its `Contracts:` marker was trimmed to exactly the rows the registry owns to it. No executable line changed. Reviewed bump, reason in docs/log.d/2026-08-29-wi530-cell-pass.md. Earlier: +4 (1959 -> 1963) 2026-08-29, WI-528 (OI-67 ruled (a)): the seam lines a brief lists beside a touched file read the far side by its DIRECTION — `Requestors` (drawn `<-`) or `Consumers` (`->`) — plus the typed `Channel`/`Data` pair, in place of the retired `Provider`/`Contract`; the four lines are the direction arrow and the two-key far-side read in the filter. Reviewed bump, reason in docs/log.d/2026-08-29-wi528-if-row-shape.md. Earlier: -1 (1960 -> 1959) 2026-08-23, WI-455: the seam lines a brief lists beside a touched file read `Provider`/`Consumers` instead of the three retired cells, and the consumers cell is a list to scan. Reviewed bump, reason in docs/log.d/2026-08-23-wi455-rename-and-shed.md. Earlier: +23 (1937 -> 1960) 2026-08-22, WI-504: terminal history moved to `docs/archive/work/` (OI-55 ruled (a)), so every by-hand terminal-folder read now unions the active workspace and its archive sibling rather than a bare `docs/work/<status_dir>` glob — the `ARCHIVE_WORK` constant, the new `_terminal_hits` helper both `_closed_spec` and `_disposition_drafts` now call through, the `_cmd_sweep` recovery walk re-keyed onto it, and `next_wi_id`'s id-taken sweep widened to both roots (the watermark stays the primary floor; this is belt-and-suspenders against the filename sweep). NOT DECOMPOSABLE as a unit — one helper serving four call sites is the point. Reviewed bump, reason in docs/log.d/2026-08-22-wi504-history-relocation.md. Earlier: RE-STAMPED DOWN -3 (1940 -> 1937) 2026-08-22, WI-483 slice 2: the two deferred `import dispatch` statements and their blank lines went with the census extraction — the module now reads `census.parse_red_tc` / `census.gap_census` off a module-level sibling import, so the mint no longer reaches back up into the scheduling composer. Recorded DOWN in the same commit rather than left as headroom, per this file's rule. Earlier: +39 (1901 -> 1940) 2026-08-21, WI-498 slice 4 (folding WI-497): `_gate_moved` becomes `_stage_moved` — a two-point delta of `docs/stage`'s HEADLINE FIELD across the two git trees, parsed through `kitlib.stage.parse`, replacing a `splitlines()[0]` of `docs/gate` that had been reading the static do-not-hand-edit header and answering False unconditionally since the derived-gate migration. THE EXECUTABLE HALF IS ROUGHLY A WASH (a guarded kitlib import, and a loop that returns early on either side being unreadable instead of comparing two Nones); the bump is the record of a defect that was silent for a whole vocabulary generation — what it read, why line 0 stopped being the value, and why the fix is a FIELD rather than a better line index. A dead mechanism that leaves no note behind is how it gets rebuilt. Reviewed bump, reason in docs/log.d/2026-08-21-wi498-stage-unification.md. Earlier: +37 (1864 -> 1901) 2026-08-21, WI-490: OI-45 rules (b)
