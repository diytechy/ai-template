...
BASELINE = {
    ...
    # line per Drafted SR (ratification owed) and per Modified SR (re-attest
    ...
    # flip warn (--staged): content cells of a Approved spine row changed
    ...
    # `attestation_findings` (the DRIFT rung: a Approved row whose normative
    ...
    # derive_gate.py: ENTRY DELETED 2026-08-21, WI-498 slice 0 — the module
    # fell BELOW THRESHOLD (1531 -> 1424) and this file's own rule is that a
    # baselined entry which drops under 1500 is removed, not re-stamped low.
    # The deleted entry had PREDICTED this exact deletion in its own words:
    # "the honest response to the crossing is this entry, and a re-stamp
    # downward if the stage-axis half is ever extracted". It was extracted:
    # the eight-rung ladder, its order, STAGE_OF, STAGE_DESC and `stage_ord`
    # moved to `kitlib/ladder.py` (one home; the equality pins retired
    # because drift became unrepresentable). The bar axis stayed and dies
    # with slice 2, so a re-crossing here would be new growth, not this one
    # returning. Reason in docs/log.d/2026-08-21-wi498-stage-unification.md.
    #   RE-ENTERED ONE SLICE LATER, and the way it happened is the part worth
    # keeping. WI-498 slice 1 took it back over: `load_spine` (the registry
    # load extracted out of `compute`, so the STAGE derivation in
    # `derive_stage.py` reads the same rows through the same resolution rules
    # instead of re-implementing them) plus the docstring for `spine_stage`'s
    # new `cited_srs` parameter — the seam that makes the rung fall-through
    # callable per PHASE, which is what keeps ONE derivation for two files.
    #   It sat at 1502, then at EXACTLY 1500 after a dead return key was
    # deleted, and the temptation at that point was to call it done. That
    # would have been trimming to fit a threshold, which is the ratchet
    # inverted — so it is recorded here instead, and `ruff format` then
    # settled the question by taking it to 1503 on its own. The honest reading
    # is that the module IS over the line: it still holds the whole BAR axis,
    # which slice 2 deletes. RE-STAMP DOWN THERE, and expect a large drop.
    "derive_gate.py": 1503,  # NEW ENTRY, +79 (1424 -> 1503) 2026-08-21, WI-498 slice 1 — the module crosses THRESHOLD again, one slice after its entry was deleted for falling under. What grew is a SHARED SEAM, not a new feature: `load_spine` lifts the carrier-resolved registry load out of `compute` so `derive_stage.py` reads the identical rows by the identical rules, and `spine_stage` gains `cited_srs`, which scopes the repo-global need-coverage rung so the same fall-through can be run per phase. Both exist so the two axes CANNOT disagree about what a Drafted row is and so slice 3 re-discriminates the ladder by editing one function. Roughly three quarters of the 79 is docstring: why an absent registry and an empty one mean opposite things at the two inserted rungs, and why the coverage rung must not be subsetted with its rows. Deliberate bump, reason in docs/log.d/2026-08-21-wi498-stage-unification.md. The decomposition the ratchet actually wants is already scheduled and is a DELETION: slice 2 removes the bar ordinals, BAR_NAMES, BAR_ORDER, STAGE_BAR and the alias table with the axis they serve — re-stamp down there rather than trimming here.
    "intake.py": 1901,  # +37 (1864 -> 1901) 2026-08-21, WI-490: OI-45 rules (b)
    # RETIRE THE MECHANICAL-RATIFICATION ARM, executed. Docstring/comment-only,
