...
GATE_FILE = (
    ...
    "# basis: SN=1 SR=2 LLR=3 TC=4 drafted=1 computed=DevStg-Tests phase=2 per-phase=1=DevStg-Impl;2=DevStg-Tests\n"
...
def test_status_splices_derived_facts(tmp_path):
    ...
    assert "derived **DevStg-Tests (Release: pending harness driver)**" in block
...
def test_status_legacy_gate_without_basis_falls_back_to_counts(tmp_path):
    ...
    assert gen(tmp_path, "--status").returncode == 0
    block = block_of(tmp_path)
    assert "derived **DevStg-Reqs**" in block
