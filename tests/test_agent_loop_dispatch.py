...
def test_head_reconcile_decision(ihead, dhead, int_ancestor, dev_ancestor, expected):
    assert (
        agent_loop.agent_dispatch._head_reconcile_decision(
            ihead, dhead, int_ancestor, dev_ancestor
        )
        == expected
    )
...
def test_train_evidence_decision(wis, built, blocked, expected):
    assert (
        agent_loop.agent_dispatch._train_evidence_decision(wis, built, blocked)
        == expected
    )
...
def test_dispatch_allowed_decision(paused, blacked_out, ask, expected):
    assert (
        agent_loop.agent_dispatch._dispatch_allowed(paused, blacked_out, ask)
        is expected
    )
...
def test_retry_due_decision(state, now, retry_at, expected):
    assert agent_loop.agent_dispatch._retry_due(state, now, retry_at) is expected
...
def test_worker_exit_decision(code, spine, policy, expected):
    assert (
        agent_loop.agent_dispatch._worker_exit_decision(code, spine, policy) == expected
    )
...
def test_integration_result_decision(result, source, expected):
    assert (
        agent_loop.agent_dispatch._integration_result_decision(result, source)
        == expected
    )
...
def test_idle_decision(paused, ask, blackout, dispatchable, waiting, expected):
    assert (
        agent_loop.agent_dispatch._idle_decision(
            paused, ask, blackout, dispatchable, waiting
        )
        == expected
    )
...
def test_terminal_decision(attention, queued, unpublished, current, blocked, expected):
    assert (
        agent_loop.agent_dispatch._terminal_decision(
            attention, queued, unpublished, current, blocked
        )
        == expected
    )
