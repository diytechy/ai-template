...
# The Gilbert DevStg-Tests finding: unquoted commas inside Permutations sets (set{a,b,c})
...
# DevStg-Impl --strict-schema. Structure is integrity-class: wrong at any stage.
...
def test_triangle_findings_unit_multi_parent_and_empty_parent():
    ...
    assert (
        trace.triangle_findings(
            [{"TC-ID": "TC-1", "Verifies": "SR-002;LLR-009"}], multi
        )
        == []
    )
    # Citing a non-parent SR alongside the multi-parent LLR is still incoherent.
    (f,) = trace.triangle_findings(
        [{"TC-ID": "TC-2", "Verifies": "SR-003;LLR-009"}], multi
    )
    assert "LLR-009" in f
    ...
    assert (
        trace.triangle_findings(
            [{"TC-ID": "TC-3", "Verifies": "SR-001;LLR-010"}], empty
        )
        == []
    )
...
def test_harness_g2_fails_on_unfilled_scaffold(scaffold):
    # The key F3 behavior: you cannot claim DevStg-Tests with placeholder-only registries.
...
def test_strict_schema_requires_evidence_on_automated_yes(scaffold):
    ...
    # cited Evidence is a soft false-green -> a --strict-schema (DevStg-Impl) finding.
...
def test_strict_schema_flags_legacy_tc_header_without_evidence(scaffold):
    ...
    # read as empty and are flagged the same way at DevStg-Impl — the deliberate
    # migration nudge (ADOPTING section 6 adds the column). Below DevStg-Impl (no
...
def test_harness_wires_new_flags():
    ...
    g2 = check.steps(80, "full", "DevStg-Tests")
    allg = check.steps(80, "full", "all")
    # --no-placeholders is on from DevStg-Tests; --strict-schema is DevStg-Impl/all only.
