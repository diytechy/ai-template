...
def test_every_tab_controls_a_labelled_panel(tmp_path):
    ...
    make_repo(tmp_path)
    write_arch_src(tmp_path)
    # The Process tab's render condition. It was a hand-written `docs/gate` bar
    # until WI-498 slice 5 retired that axis; the derived `docs/stage` record is
    # written through the carrier's own renderer (traj_fixtures.write_stage), so
    # this fixture cannot drift from the format the readers parse.
    write_stage(tmp_path, "DevStg-Tests")
...
def test_u3_svg_corner_radii_match_the_declared_scale(tmp_path):
    ...
    # Follow both the facade/shared readers and the HTML package after P9R;
    # moving the emitters must not remove their literals from observation.
    src = "".join(
        p.read_text(encoding="utf-8")
        for p in sorted(SCRIPTS.glob("traj_*.py"))
        + sorted((SCRIPTS / "rendering").glob("*.py"))
