...
def test_disposition_regen_status_map_opts_in_on_open_items_alone(
    ...
    (worktree / "docs" / "open-items.md").write_text("pending\n", encoding="utf-8")
    calls = []
    ...
    ok, detail = agent_loop.agent_dispatch._regenerate_disposition_artifacts(worktree)
    assert ok and detail == ""
    assert calls == [("gen_trajectory.py", ["--status"])]
...
_PENDING_BEGIN = "<!-- BEGIN GENERATED PENDING -->"
_STATUS_BEGIN = "<!-- BEGIN GENERATED STATUS -->"

_OPEN_ITEMS_MD = (
    "# Open items — owner decision briefs\n\n"
    "Hand-authored intro — MUST stay byte-identical.\n\n"
    "## OI-42 — a real pending decision\n\n"
    "- **One-line:** rule the thing.\n\n"
    "---\n\n"
    "## Pending owner actions (generated)\n\n"
    + _PENDING_BEGIN
    + "\nplaceholder — regenerated by gen_trajectory.py --status\n"
...
def test_blocked_disposition_regenerates_pending_projection(tmp_path, with_status_md):
    ...
    # feeds the GENERATED pending-owner-actions projection in docs/open-items.md.
    # The disposition must regenerate it so its own commit passes the SAME
    # status-map floor the pre-commit hook runs. Covered both with and without a
    ...
    repo, ctl, template = _setup(tmp_path, [_wi_row("WI-201"), _wi_row("WI-202")])
    (repo / "docs" / "open-items.md").write_text(_OPEN_ITEMS_MD, encoding="utf-8")
    if with_status_md:
    ...
    # The committed projection now lists the blocked WI — regeneration fired.
    projected = _git(repo, "show", "refs/heads/llm/integration:docs/open-items.md")
    assert "**WI-201** blocked" in projected
    assert "OI-42" in projected

    # ... and the composed tree passes the status-map floor the commit faced —
    # `gen_trajectory.py --status --check` is exactly what the pre-commit hook runs.
    view = tmp_path / "view"
    _git(repo, "worktree", "add", "--detach", str(view), new_head)
    proc = run_py(
        [SCRIPTS / "gen_trajectory.py", "--root", view, "--status", "--check"],
        cwd=view,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
