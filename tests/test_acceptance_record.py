...
def test_no_snapshotted_tier_can_go_unseen_by_the_approval_rung():
    ...
    `lane_approval_refusal` walks `SPINE_CSVS` — SR/LLR/TC — while
    `baseline_snapshot.SNAPSHOTTED` names SEVEN registries whose `Status` a
    snapshot anchors. Both are hand-written literals, in different modules,
    hundreds of lines apart, and NOTHING joined them: a tier could be added to
    the snapshot and reach no approval reader at all, silently. The narrowness
    itself is correct and RULED — the owner's 2026-09-01 ruling scopes the act
    to spine rows ("new SR/LLR/TC rows are written `Drafted`"), and SN's older
    exclusion is design §B7's parked pass — so this does not widen the rung. It
    ...
    refused = {rel for rel, _ in acceptance_record.SPINE_CSVS}
...
def test_an_off_spine_status_flip_is_not_the_act_this_rung_refuses(tmp_path):
    ...
    The review found no test asserting either that the four non-`SPINE_CSVS`
    tiers are refused or that they are out of scope, so the omission read as an
    oversight from the outside. It is the ruling: a lane may move an interface
    row's `status`, and that is not the approval act on a spine row. Driven
