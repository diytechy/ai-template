...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **167,884** as of 2026-08-01/WI-392 (+1,570 on the 166,314 stamp: "Signed measurements" grew its part 3 — the opt-in `fig:` declared-figure marker grammar with the population and derived-figure bars — and its closing paragraph now splits truth (Reviewer) from presence (`check_figures.py`, the opt-in `[step:figures]`) and records rung 2 (re-derivation) as deliberately not built); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
