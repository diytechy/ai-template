...
didn't run. Authority: `docs/process.md` §4 (stages and gates), §5 (verdict
protocol), §7 (the derived gate). The gate's **acceptor** is whoever the `gate_policy` dial in
...
## The derived-gate mechanism (derived, not hand-set)

A repo is **in a stage** and **passes a gate** (`docs/process.md` §4 "Stages and
gates"). The first non-comment line in **`docs/gate`** (`G1|G2|G3`) is the gate
it must next pass, which `scripts/check.py` reads to run exactly that gate's
checks — so CI enforces the bar the project is working toward. `docs/gate` is
**generated by `scripts/derive_gate.py`**, not hand-set (the derived-gate design
note lives in the kit's meta-repo, not shipped; the operative rule is
`docs/process.md` §4): the value is **computed from the artifact states** (SN
section-state + SR/LLR/TC `Status` maturity) as the **min** over every in-scope
