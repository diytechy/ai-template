...
| `project-trajectory/PROCESS.md` | **watched** (baseline **62,267** as of 2026-07-27/WI-328 (+443 on the 61,824 stamp: §3's stand-alone rule corrected to its real scope — every spine registry, not the `SR` alone — and its real severity, gating under `--strict`; plus §4's rule that requirement-text work sequences INTO an open re-attestation window rather than after it); keep flat — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report |
