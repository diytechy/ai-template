...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **161,541** as of 2026-07-25/WI-065 (+561 off the measured 160,980: the ruled home for a seam citation — the TC's own `Verifies` cell, why one cell beats a second column, and the supplements-never-replaces rule. NOTE the previous stamp read 161,007 while the file measured 160,980, so a net −27 had landed un-re-stamped: always `wc -c` before editing rather than trusting this number as the before-size); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
