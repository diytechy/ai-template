...
| `project-trajectory/PROCESS.md` | **watched** (baseline **64,077** as of 2026-07-29/concurrency-restructure Phase 3 (+584 on the 63,493 stamp: §5's verdict/log protocol now states the `docs/log.d/` fragment + trunk-compile contract and WI-scoped review-file names, and the two generated-freshness sentences carry the trunk-lane condition — work branches skip, the trunk regenerates); keep flat — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report |
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **166,953** as of 2026-07-29/concurrency-restructure Phase 3 (+1,342 on the 165,611 stamp: the tracked `docs/work/pause` §5.6 contract replaces the untracked-pause paragraph and blackout re-grounds on it, review verdicts go WI-scoped with the git-time latest rule, session discipline routes a work branch's log record to a `docs/log.d/` fragment, and the dashboard-freshness paragraph states the trunk-only/work-branch-skip rule); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
