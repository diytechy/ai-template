...
| `project-trajectory/PROCESS_OPTIONS.md` | **watched** (baseline **161,117** as of 2026-07-29/concurrency-restructure Phase 5 (−5,836 on the 166,953 stamp: the parallel-dispatch section rewrote onto the §1.2/§2.3 integration-seam model — claims, the serial fail-closed merge queue, forge mode — and the unattended layer's dispatcher/run-state/traincar vocabulary re-grounded on the session engine + tracked pause; a rare shrink, the deletion phase's dividend); growth is allowed but must be *flagged* with a delta + reason — re-stamp this number, every tracked skill copy, when a flagged growth lands) | convention + the WI log's byte-delta report + the doc's own *Applies-when index* note |
